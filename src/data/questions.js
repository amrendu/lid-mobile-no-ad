// Questions data - automatically generated from backend/new.json
// Total questions: 460
// Last updated: 2025-07-26T14:31:18.515Z

export const questionsData = [
  {
    "question_number": 1,
    "bundesland": "General",
    "question": "In Deutschland dürfen Menschen offen etwas gegen die Regierung sagen, weil …",
    "question_en": "In Germany, people can openly say something against the government because ...",
    "options": [
      "hier Religionsfreiheit gilt",
      "die Menschen Steuern zahlen",
      "die Menschen das Wahlrecht haben",
      "hier Meinungsfreiheit gilt"
    ],
    "options_en": [
      "Here applies here religious freedom",
      "People pay taxes",
      "People have the right to vote",
      "Freedom of expression applies here"
    ],
    "answer": "hier Meinungsfreiheit gilt",
    "answer_en": "Freedom of expression applies here",
    "image_paths": []
  },
  {
    "question_number": 2,
    "bundesland": "General",
    "question": "In Deutschland können Eltern bis zum 14. Lebensjahr ihres Kindes entscheiden, ob es in der Schule am …",
    "question_en": "In Germany, parents can decide until their child's 14th year of life whether it takes part in school's ...",
    "options": [
      "Geschichtsunterricht teilnimmt",
      "Religionsunterricht teilnimmt",
      "Politikunterricht teilnimmt",
      "Sprachunterricht teilnimmt"
    ],
    "options_en": [
      "History lessons participate",
      "Religious instruction participates",
      "Political lessons participate",
      "Language lessons participate"
    ],
    "answer": "Religionsunterricht teilnimmt",
    "answer_en": "Religious instruction participates",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "General",
    "question": "Deutschland ist ein Rechtsstaat. Was ist damit gemeint?",
    "question_en": "Germany is a constitutional state. What is meant by that?",
    "options": [
      "Alle Einwohnerinnen/Einwohner und der Staat müssen sich an die Gesetze halten",
      "Der Staat muss sich nicht an die Gesetze halten",
      "Nur Deutsche müssen die Gesetze befolgen",
      "Die Gerichte machen die Gesetze"
    ],
    "options_en": [
      "All residents and the state must adhere to the laws",
      "The state does not have to adhere to the laws",
      "Only Germans have to follow the laws",
      "The courts make the laws"
    ],
    "answer": "Alle Einwohnerinnen/Einwohner und der Staat müssen sich an die Gesetze halten",
    "answer_en": "All residents and the state must adhere to the laws",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "General",
    "question": "Welches Recht gehört zu den Grundrechten in Deutschland?",
    "question_en": "Which right is one of the fundamental rights in Germany?",
    "options": [
      "Waffenbesitz",
      "Faustrecht",
      "Meinungsfreiheit",
      "Selbstjustiz"
    ],
    "options_en": [
      "Gun ownership",
      "Fist right",
      "Freedom of expression",
      "Self -justice"
    ],
    "answer": "Meinungsfreiheit",
    "answer_en": "Freedom of expression",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "General",
    "question": "Wahlen in Deutschland sind frei. Was bedeutet das?",
    "question_en": "Elections in Germany are free. What does that mean?",
    "options": [
      "Man darf Geld annehmen, wenn man dafür eine bestimmte Kandidatin/einen bestimmten Kandidaten wählt",
      "Nur Personen, die noch nie im Gefängnis waren, dürfen wählen",
      "Die Wählerin/der Wähler darf bei der Wahl weder beeinflusst noch zu einer bestimmten Stimmabgabe gezwungen werden und keine Nachteile durch die Wahl haben",
      "Alle wahlberechtigten Personen müssen wählen"
    ],
    "options_en": [
      "You can accept money if you choose a specific candidate for this",
      "Only people who have never been in prison can vote",
      "The voter must neither be influenced in the election nor forced to make a certain voice and have no disadvantages from the choice",
      "All people entitled to vote must choose"
    ],
    "answer": "Die Wählerin/der Wähler darf bei der Wahl weder beeinflusst noch zu einer bestimmten Stimmabgabe gezwungen werden und keine Nachteile durch die Wahl haben",
    "answer_en": "The voter must neither be influenced in the election nor forced to make a certain voice and have no disadvantages from the choice",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "General",
    "question": "Wie heißt die deutsche Verfassung?",
    "question_en": "What is the name of the German constitution?",
    "options": [
      "Volksgesetz",
      "Bundesgesetz",
      "Deutsches Gesetz",
      "Grundgesetz"
    ],
    "options_en": [
      "Public law",
      "Federal law",
      "German law",
      "Basic Law"
    ],
    "answer": "Grundgesetz",
    "answer_en": "Basic Law",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "General",
    "question": "Welches Recht gehört zu den Grundrechten, die nach der deutschen Verfassung garantiert werden? Das Recht auf …",
    "question_en": "Which right is one of the fundamental rights that are guaranteed according to the German constitution? The right to ...",
    "options": [
      "Glaubens- und Gewissensfreiheit",
      "Unterhaltung",
      "Arbeit",
      "Wohnung"
    ],
    "options_en": [
      "Freedom of faith and conscience",
      "Entertainment",
      "Work",
      "Apartment"
    ],
    "answer": "Glaubens- und Gewissensfreiheit",
    "answer_en": "Freedom of faith and conscience",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "General",
    "question": "Was steht nicht im Grundgesetz von Deutschland?",
    "question_en": "What is not in the Basic Law of Germany?",
    "options": [
      "Die Würde des Menschen ist unantastbar",
      "Alle sollen gleich viel Geld haben",
      "Jeder Mensch darf seine Meinung sagen",
      "Alle sind vor dem Gesetz gleich"
    ],
    "options_en": [
      "Human dignity is inviolable",
      "Everyone should have the same amount of money",
      "Everyone can say their opinion",
      "Everyone is the same before the law"
    ],
    "answer": "Alle sollen gleich viel Geld haben",
    "answer_en": "Everyone should have the same amount of money",
    "image_paths": []
  },
  {
    "question_number": 9,
    "bundesland": "General",
    "question": "Welches Grundrecht gilt in Deutschland nur für Ausländerinnen/Ausländer? Das Grundrecht auf …",
    "question_en": "Which fundamental right in Germany only applies to foreigners? The fundamental right to ...",
    "options": [
      "Schutz der Familie",
      "Menschenwürde",
      "Asyl",
      "Meinungsfreiheit"
    ],
    "options_en": [
      "Protection of the family",
      "Human dignity",
      "asylum",
      "Freedom of expression"
    ],
    "answer": "Asyl",
    "answer_en": "asylum",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "General",
    "question": "Was ist mit dem deutschen Grundgesetz vereinbar?",
    "question_en": "What is compatible with the German Basic Law?",
    "options": [
      "die Prügelstrafe",
      "die Folter",
      "die Todesstrafe",
      "die Geldstrafe"
    ],
    "options_en": [
      "The spanking sentence",
      "The torture",
      "The death penalty",
      "The fine"
    ],
    "answer": "die Geldstrafe",
    "answer_en": "The fine",
    "image_paths": []
  },
  {
    "question_number": 11,
    "bundesland": "General",
    "question": "Wie wird die Verfassung der Bundesrepublik Deutschland genannt?",
    "question_en": "What is the constitution of the Federal Republic of Germany called?",
    "options": [
      "Grundgesetz",
      "Bundesverfassung",
      "Gesetzbuch",
      "Verfassungsvertrag"
    ],
    "options_en": [
      "Basic Law",
      "Federal constitution",
      "Code",
      "Constitutional contract"
    ],
    "answer": "Grundgesetz",
    "answer_en": "Basic Law",
    "image_paths": []
  },
  {
    "question_number": 12,
    "bundesland": "General",
    "question": "Eine Partei im Deutschen Bundestag will die Pressefreiheit abschaffen. Ist das möglich?",
    "question_en": "A party in the German Bundestag wants to abolish the freedom of the press. Is that possible?",
    "options": [
      "Ja, wenn mehr als die Hälfte der Abgeordneten im Bundestag dafür sind",
      "Ja, aber dazu müssen zwei Drittel der Abgeordneten im Bundestag dafür sein",
      "Nein, denn die Pressefreiheit ist ein Grundrecht. Sie kann nicht abgeschafft werden",
      "Nein, denn nur der Bundesrat kann die Pressefreiheit abschaffen"
    ],
    "options_en": [
      "Yes, if there are more than half of the MPs in the Bundestag",
      "Yes, but two thirds of the MPs in the Bundestag must be for this",
      "No, because freedom of the press is a fundamental right. It cannot be abolished.",
      "No, because only the Federal Council can abolish the freedom of the press"
    ],
    "answer": "Nein, denn die Pressefreiheit ist ein Grundrecht. Sie kann nicht abgeschafft werden",
    "answer_en": "No, because freedom of the press is a fundamental right. It cannot be abolished.",
    "image_paths": []
  },
  {
    "question_number": 13,
    "bundesland": "General",
    "question": "Im Parlament steht der Begriff \"Opposition\" für …",
    "question_en": "In parliament, the term \"opposition\" stands for ...",
    "options": [
      "die regierenden Parteien",
      "die Fraktion mit den meisten Abgeordneten",
      "alle Parteien, die bei der letzten Wahl die 5%-Hürde erreichen konnten",
      "alle Abgeordneten, die nicht zu der Regierungspartei/den Regierungsparteien gehören"
    ],
    "options_en": [
      "The governing parties",
      "The parliamentary group with most MPs",
      "All parties who could reach the 5%hurdle in the last election",
      "All MPs who do not belong to the government party/government parties"
    ],
    "answer": "alle Abgeordneten, die nicht zu der Regierungspartei/den Regierungsparteien gehören",
    "answer_en": "All MPs who do not belong to the government party/government parties",
    "image_paths": []
  },
  {
    "question_number": 14,
    "bundesland": "General",
    "question": "Meinungsfreiheit in Deutschland heißt, dass ich …",
    "question_en": "Freedom of expression in Germany means that I ...",
    "options": [
      "Passanten auf der Straße beschimpfen darf",
      "meine Meinung im Internet äußern kann",
      "Nazi-, Hamas- oder Islamischer Staat-Symbole öffentlich tragen darf",
      "meine Meinung nur dann äußern darf, solange ich der Regierung nicht widerspreche"
    ],
    "options_en": [
      "Passers -by on the street may insult",
      "can express my opinion on the Internet",
      "Nazi, Hamas or Islamic State symbols may carry public",
      "my opinion can only be expressed as long as I do not contradict the government"
    ],
    "answer": "meine Meinung im Internet äußern kann",
    "answer_en": "can express my opinion on the Internet",
    "image_paths": []
  },
  {
    "question_number": 15,
    "bundesland": "General",
    "question": "Was verbietet das deutsche Grundgesetz?",
    "question_en": "What does the German Basic Law prohibit?",
    "options": [
      "Militärdienst",
      "Zwangsarbeit",
      "freie Berufswahl",
      "Arbeit im Ausland"
    ],
    "options_en": [
      "military service",
      "Forced labor",
      "Free career choice",
      "Work abroad"
    ],
    "answer": "Zwangsarbeit",
    "answer_en": "Forced labor",
    "image_paths": []
  },
  {
    "question_number": 16,
    "bundesland": "General",
    "question": "Wann ist die Meinungsfreiheit in Deutschland eingeschränkt?",
    "question_en": "When is freedom of expression in Germany restricted?",
    "options": [
      "bei der öffentlichen Verbreitung falscher Behauptungen über einzelne Personen",
      "bei Meinungsäußerungen über die Bundesregierung",
      "bei Diskussionen über Religionen",
      "bei Kritik am Staat"
    ],
    "options_en": [
      "in the public distribution of false claims about individuals",
      "in the expressions of opinion about the federal government",
      "In discussions about religions",
      "In criticism of the state"
    ],
    "answer": "bei der öffentlichen Verbreitung falscher Behauptungen über einzelne Personen",
    "answer_en": "in the public distribution of false claims about individuals",
    "image_paths": []
  },
  {
    "question_number": 17,
    "bundesland": "General",
    "question": "Die deutschen Gesetze verbieten …",
    "question_en": "German laws prohibit ...",
    "options": [
      "Meinungsfreiheit der Einwohnerinnen und Einwohner",
      "Petitionen der Bürgerinnen und Bürger",
      "Versammlungsfreiheit der Einwohnerinnen und Einwohner",
      "Ungleichbehandlung der Bürgerinnen und Bürger durch den Staat"
    ],
    "options_en": [
      "Freedom of expression of the residents",
      "Petitions of the citizens",
      "Freedom of assembly of the residents",
      "Unequal treatment of the citizens by the state"
    ],
    "answer": "Ungleichbehandlung der Bürgerinnen und Bürger durch den Staat",
    "answer_en": "Unequal treatment of the citizens by the state",
    "image_paths": []
  },
  {
    "question_number": 18,
    "bundesland": "General",
    "question": "Welches Grundrecht ist in Artikel 1 des Grundgesetzes der Bundesrepublik Deutschland garantiert?",
    "question_en": "Which fundamental right is guaranteed in Article 1 of the Basic Law of the Federal Republic of Germany?",
    "options": [
      "die Unantastbarkeit der Menschenwürde",
      "das Recht auf Leben",
      "Religionsfreiheit",
      "Meinungsfreiheit"
    ],
    "options_en": [
      "the inviolability of human dignity",
      "The right to life",
      "Freedom of religion",
      "Freedom of expression"
    ],
    "answer": "die Unantastbarkeit der Menschenwürde",
    "answer_en": "the inviolability of human dignity",
    "image_paths": []
  },
  {
    "question_number": 19,
    "bundesland": "General",
    "question": "Was versteht man unter dem Recht der \"Freizügigkeit\" in Deutschland?",
    "question_en": "What is meant by the right of \"freedom of movement\" in Germany?",
    "options": [
      "Man darf sich seinen Wohnort selbst aussuchen",
      "Man kann seinen Beruf wechseln",
      "Man darf sich für eine andere Religion entscheiden",
      "Man darf sich in der Öffentlichkeit nur leicht bekleidet bewegen"
    ],
    "options_en": [
      "You can choose your own place of residence",
      "You can change your job",
      "One can choose a different religion",
      "One can only move slightly dressed in public"
    ],
    "answer": "Man darf sich seinen Wohnort selbst aussuchen",
    "answer_en": "You can choose your own place of residence",
    "image_paths": []
  },
  {
    "question_number": 20,
    "bundesland": "General",
    "question": "Eine Partei in Deutschland verfolgt das Ziel, eine Diktatur zu errichten. Sie ist dann …",
    "question_en": "A party in Germany pursues the goal of building a dictatorship. It is then ...",
    "options": [
      "tolerant",
      "rechtsstaatlich orientiert",
      "gesetzestreu",
      "verfassungswidrig"
    ],
    "options_en": [
      "tolerant",
      "under the rule of law",
      "faithful",
      "unconstitutional"
    ],
    "answer": "verfassungswidrig",
    "answer_en": "unconstitutional",
    "image_paths": []
  },
  {
    "question_number": 21,
    "bundesland": "General",
    "question": "Welches ist das Wappen der Bundesrepublik Deutschland?",
    "question_en": "What is the coat of arms of the Federal Republic of Germany?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 1",
    "answer_en": "Image 1",
    "image_paths": [
      "./images/general-21-option-1.png",
      "./images/general-21-option-2.png",
      "./images/general-21-option-3.png",
      "./images/general-21-option-4.jpg"
    ]
  },
  {
    "question_number": 22,
    "bundesland": "General",
    "question": "Was für eine Staatsform hat Deutschland?",
    "question_en": "What kind of government does Germany have?",
    "options": [
      "Monarchie",
      "Diktatur",
      "Republik",
      "Fürstentum"
    ],
    "options_en": [
      "monarchy",
      "dictatorship",
      "republic",
      "principality"
    ],
    "answer": "Republik",
    "answer_en": "republic",
    "image_paths": []
  },
  {
    "question_number": 23,
    "bundesland": "General",
    "question": "In Deutschland sind die meisten Erwerbstätigen …",
    "question_en": "In Germany, most of the workers are ...",
    "options": [
      "in kleinen Familienunternehmen beschäftigt",
      "ehrenamtlich für ein Bundesland tätig",
      "selbstständig mit einer eigenen Firma tätig",
      "bei einer Firma oder Behörde beschäftigt"
    ],
    "options_en": [
      "employed in small family businesses",
      "volunteer for a federal state",
      "work independently with your own company",
      "employed at a company or authority"
    ],
    "answer": "bei einer Firma oder Behörde beschäftigt",
    "answer_en": "employed at a company or authority",
    "image_paths": []
  },
  {
    "question_number": 24,
    "bundesland": "General",
    "question": "Wie viele Bundesländer hat die Bundesrepublik Deutschland?",
    "question_en": "How many federal states does the Federal Republic of Germany have?",
    "options": [
      "14",
      "15",
      "16",
      "17"
    ],
    "options_en": [
      "14",
      "15",
      "16",
      "17"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 25,
    "bundesland": "General",
    "question": "Was ist kein Bundesland der Bundesrepublik Deutschland?",
    "question_en": "What is not a federal state of the Federal Republic of Germany?",
    "options": [
      "Elsass-Lothringen",
      "Nordrhein-Westfalen",
      "Mecklenburg-Vorpommern",
      "Sachsen-Anhalt"
    ],
    "options_en": [
      "Alsace-Lorraine",
      "North Rhine-Westphalia",
      "Mecklenburg-Western Pomerania",
      "Saxony-Anhalt"
    ],
    "answer": "Elsass-Lothringen",
    "answer_en": "Alsace-Lorraine",
    "image_paths": []
  },
  {
    "question_number": 26,
    "bundesland": "General",
    "question": "Deutschland ist …",
    "question_en": "Germany is ...",
    "options": [
      "eine kommunistische Republik",
      "ein demokratischer und sozialer Bundesstaat",
      "eine kapitalistische und soziale Monarchie",
      "ein sozialer und sozialistischer Bundesstaat"
    ],
    "options_en": [
      "a communist republic",
      "A democratic and social federal state",
      "A capitalist and social monarchy",
      "A social and socialist federal state"
    ],
    "answer": "ein demokratischer und sozialer Bundesstaat",
    "answer_en": "A democratic and social federal state",
    "image_paths": []
  },
  {
    "question_number": 27,
    "bundesland": "General",
    "question": "Deutschland ist …",
    "question_en": "Germany is ...",
    "options": [
      "ein sozialistischer Staat",
      "ein Bundesstaat",
      "eine Diktatur",
      "eine Monarchie"
    ],
    "options_en": [
      "A socialist state",
      "a federal state",
      "a dictatorship",
      "A monarchy"
    ],
    "answer": "ein Bundesstaat",
    "answer_en": "a federal state",
    "image_paths": []
  },
  {
    "question_number": 28,
    "bundesland": "General",
    "question": "Wer wählt in Deutschland die Abgeordneten zum Bundestag?",
    "question_en": "Who elects the members of the Bundestag in Germany?",
    "options": [
      "das Militär",
      "die Wirtschaft",
      "das wahlberechtigte Volk",
      "die Verwaltung"
    ],
    "options_en": [
      "The military",
      "the economy",
      "The people entitled to vote",
      "The administration"
    ],
    "answer": "das wahlberechtigte Volk",
    "answer_en": "The people entitled to vote",
    "image_paths": []
  },
  {
    "question_number": 29,
    "bundesland": "General",
    "question": "Welches Tier ist das Wappentier der Bundesrepublik Deutschland?",
    "question_en": "Which animal is the heraldic animal of the Federal Republic of Germany?",
    "options": [
      "Löwe",
      "Adler",
      "Bär",
      "Pferd"
    ],
    "options_en": [
      "Lion",
      "eagle",
      "bear",
      "Horse"
    ],
    "answer": "Adler",
    "answer_en": "eagle",
    "image_paths": []
  },
  {
    "question_number": 30,
    "bundesland": "General",
    "question": "Was ist kein Merkmal unserer Demokratie?",
    "question_en": "What is not a characteristic of our democracy?",
    "options": [
      "regelmäßige Wahlen",
      "Pressezensur",
      "Meinungsfreiheit",
      "verschiedene Parteien"
    ],
    "options_en": [
      "Regular elections",
      "Press censorship",
      "Freedom of expression",
      "different parties"
    ],
    "answer": "Pressezensur",
    "answer_en": "Press censorship",
    "image_paths": []
  },
  {
    "question_number": 31,
    "bundesland": "General",
    "question": "Die Zusammenarbeit von Parteien zur Bildung einer Regierung nennt man in Deutschland …",
    "question_en": "The cooperation of parties to form a government is called in Germany ...",
    "options": [
      "Einheit",
      "Koalition",
      "Ministerium",
      "Fraktion"
    ],
    "options_en": [
      "Unit",
      "coalition",
      "Ministry",
      "fraction"
    ],
    "answer": "Koalition",
    "answer_en": "coalition",
    "image_paths": []
  },
  {
    "question_number": 32,
    "bundesland": "General",
    "question": "Was ist keine staatliche Gewalt in Deutschland?",
    "question_en": "What is not a state power in Germany?",
    "options": [
      "Gesetzgebung",
      "Regierung",
      "Presse",
      "Rechtsprechung"
    ],
    "options_en": [
      "legislation",
      "Government",
      "press",
      "Jurisprudence"
    ],
    "answer": "Presse",
    "answer_en": "press",
    "image_paths": []
  },
  {
    "question_number": 33,
    "bundesland": "General",
    "question": "Welche Aussage ist richtig? In Deutschland …",
    "question_en": "Which statement is correct? In Germany ...",
    "options": [
      "sind Staat und Religionsgemeinschaften voneinander getrennt",
      "bilden die Religionsgemeinschaften den Staat",
      "ist der Staat abhängig von den Religionsgemeinschaften",
      "bilden Staat und Religionsgemeinschaften eine Einheit"
    ],
    "options_en": [
      "the state and religious communities are separated from each other",
      "the religious communities form the state",
      "the state depends on the religious communities",
      "form a unity of the state and religious communities"
    ],
    "answer": "sind Staat und Religionsgemeinschaften voneinander getrennt",
    "answer_en": "the state and religious communities are separated from each other",
    "image_paths": []
  },
  {
    "question_number": 34,
    "bundesland": "General",
    "question": "Was ist Deutschland nicht?",
    "question_en": "What is Germany not?",
    "options": [
      "eine Demokratie",
      "ein Rechtsstaat",
      "eine Monarchie",
      "ein Sozialstaat"
    ],
    "options_en": [
      "a democracy",
      "a constitutional state",
      "A monarchy",
      "A welfare state"
    ],
    "answer": "eine Monarchie",
    "answer_en": "A monarchy",
    "image_paths": []
  },
  {
    "question_number": 35,
    "bundesland": "General",
    "question": "Womit finanziert der deutsche Staat die Sozialversicherung?",
    "question_en": "How does the German state finance social security?",
    "options": [
      "Kirchensteuer",
      "Sozialabgaben",
      "Spendengeldern",
      "Vereinsbeiträgen"
    ],
    "options_en": [
      "Church tax",
      "Social security contributions",
      "Donations",
      "Club contributions"
    ],
    "answer": "Sozialabgaben",
    "answer_en": "Social security contributions",
    "image_paths": []
  },
  {
    "question_number": 36,
    "bundesland": "General",
    "question": "Welche Maßnahme schafft in Deutschland soziale Sicherheit?",
    "question_en": "Which measure creates social security in Germany?",
    "options": [
      "die Krankenversicherung",
      "die Autoversicherung",
      "die Gebäudeversicherung",
      "die Haftpflichtversicherung"
    ],
    "options_en": [
      "health insurance",
      "The car insurance",
      "The building insurance",
      "Liability insurance"
    ],
    "answer": "die Krankenversicherung",
    "answer_en": "health insurance",
    "image_paths": []
  },
  {
    "question_number": 37,
    "bundesland": "General",
    "question": "Wie werden die Regierungschefinnen/Regierungschefs der meisten Bundesländer in Deutschland genannt?",
    "question_en": "What are the heads of government of most federal states in Germany called?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Senatorin/Senator",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Senator",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 38,
    "bundesland": "General",
    "question": "Die Bundesrepublik Deutschland ist ein demokratischer und sozialer …",
    "question_en": "The Federal Republic of Germany is a democratic and social ...",
    "options": [
      "Staatenverbund",
      "Bundesstaat",
      "Staatenbund",
      "Zentralstaat"
    ],
    "options_en": [
      "Confederation of states",
      "Federal state",
      "Federation",
      "Central state"
    ],
    "answer": "Bundesstaat",
    "answer_en": "Federal state",
    "image_paths": []
  },
  {
    "question_number": 39,
    "bundesland": "General",
    "question": "Was hat jedes deutsche Bundesland?",
    "question_en": "What does every German federal state have?",
    "options": [
      "eine eigene Außenministerin/einen eigenen Außenminister",
      "eine eigene Währung",
      "eine eigene Armee",
      "eine eigene Regierung"
    ],
    "options_en": [
      "A separate Foreign Minister",
      "A separate currency",
      "A separate army",
      "A separate government"
    ],
    "answer": "eine eigene Regierung",
    "answer_en": "A separate government",
    "image_paths": []
  },
  {
    "question_number": 40,
    "bundesland": "General",
    "question": "Mit welchen Worten beginnt die deutsche Nationalhymne?",
    "question_en": "With which words does the German national anthem begin?",
    "options": [
      "Völker, hört die Signale …",
      "Einigkeit und Recht und Freiheit …",
      "Freude schöner Götterfunken …",
      "Deutschland einig Vaterland …"
    ],
    "options_en": [
      "Peoples, hear the signals ...",
      "Unity and justice and freedom …",
      "Joy, beautiful spark of divinity …",
      "Germany, united fatherland ..."
    ],
    "answer": "Einigkeit und Recht und Freiheit …",
    "answer_en": "Unity and justice and freedom …",
    "image_paths": []
  },
  {
    "question_number": 41,
    "bundesland": "General",
    "question": "Warum gibt es in einer Demokratie mehr als eine Partei?",
    "question_en": "Why is there more than one party in a democracy?",
    "options": [
      "weil dadurch die unterschiedlichen Meinungen der Bürgerinnen und Bürger vertreten werden",
      "damit Bestechung in der Politik begrenzt wird",
      "um politische Demonstrationen zu verhindern",
      "um wirtschaftlichen Wettbewerb anzuregen"
    ],
    "options_en": [
      "Because this represents the different opinions of the citizens",
      "so that bribery in politics is limited",
      "To prevent political demonstrations",
      "To stimulate economic competition"
    ],
    "answer": "weil dadurch die unterschiedlichen Meinungen der Bürgerinnen und Bürger vertreten werden",
    "answer_en": "Because this represents the different opinions of the citizens",
    "image_paths": []
  },
  {
    "question_number": 42,
    "bundesland": "General",
    "question": "Wer beschließt in Deutschland ein neues Gesetz?",
    "question_en": "Who passes a new law in Germany?",
    "options": [
      "die Regierung",
      "das Parlament",
      "die Gerichte",
      "die Polizei"
    ],
    "options_en": [
      "the government",
      "The parliament",
      "The courts",
      "the police"
    ],
    "answer": "das Parlament",
    "answer_en": "The parliament",
    "image_paths": []
  },
  {
    "question_number": 43,
    "bundesland": "General",
    "question": "Wann kann in Deutschland eine Partei verboten werden?",
    "question_en": "When can a party be banned in Germany?",
    "options": [
      "wenn ihr Wahlkampf zu teuer ist",
      "wenn sie gegen die Verfassung kämpft",
      "wenn sie Kritik am Staatsoberhaupt äußert",
      "wenn ihr Programm eine neue Richtung vorschlägt"
    ],
    "options_en": [
      "If its election campaign is too expensive",
      "When it fights against the constitution",
      "If it expresses criticism of the head of state",
      "If its program suggests a new direction"
    ],
    "answer": "wenn sie gegen die Verfassung kämpft",
    "answer_en": "When it fights against the constitution",
    "image_paths": []
  },
  {
    "question_number": 44,
    "bundesland": "General",
    "question": "Wen kann man als Bürgerin/Bürger in Deutschland nicht direkt wählen?",
    "question_en": "Whom can you not elect directly as a citizen in Germany?",
    "options": [
      "Abgeordnete des EU-Parlaments",
      "die Bundespräsidentin/den Bundespräsidenten",
      "Landtagsabgeordnete",
      "Bundestagsabgeordnete"
    ],
    "options_en": [
      "Members of the EU Parliament",
      "The Federal President",
      "Members of the state parliament",
      "Members of the Bundestag"
    ],
    "answer": "die Bundespräsidentin/den Bundespräsidenten",
    "answer_en": "The Federal President",
    "image_paths": []
  },
  {
    "question_number": 45,
    "bundesland": "General",
    "question": "Zu welcher Versicherung gehört die Pflegeversicherung?",
    "question_en": "To which insurance does the long-term care insurance belong?",
    "options": [
      "Sozialversicherung",
      "Unfallversicherung",
      "Hausratsversicherung",
      "Haftpflicht- und Feuerversicherung"
    ],
    "options_en": [
      "social security",
      "Accident insurance",
      "Household insurance",
      "Liability and fire insurance"
    ],
    "answer": "Sozialversicherung",
    "answer_en": "social security",
    "image_paths": []
  },
  {
    "question_number": 46,
    "bundesland": "General",
    "question": "Der deutsche Staat hat viele Aufgaben. Welche Aufgabe gehört dazu?",
    "question_en": "The German state has many tasks. Which task is one of them?",
    "options": [
      "Er baut Straßen und Schulen",
      "Er verkauft Lebensmittel und Kleidung",
      "Er versorgt alle Einwohnerinnen und Einwohner kostenlos mit Zeitungen",
      "Er produziert Autos und Busse"
    ],
    "options_en": [
      "He builds streets and schools",
      "He sells food and clothing",
      "He provides all residents with newspapers free of charge",
      "He produces cars and buses"
    ],
    "answer": "Er baut Straßen und Schulen",
    "answer_en": "He builds streets and schools",
    "image_paths": []
  },
  {
    "question_number": 47,
    "bundesland": "General",
    "question": "Der deutsche Staat hat viele Aufgaben. Welche Aufgabe gehört nicht dazu?",
    "question_en": "The German state has many tasks. Which task is not one of them?",
    "options": [
      "Er bezahlt für alle Staatsangehörigen Urlaubsreisen",
      "Er zahlt Kindergeld",
      "Er unterstützt Museen",
      "Er fördert Sportlerinnen und Sportler"
    ],
    "options_en": [
      "He pays for all nationals' holiday trips",
      "He pays child benefit",
      "He supports museums",
      "He promotes athletes"
    ],
    "answer": "Er bezahlt für alle Staatsangehörigen Urlaubsreisen",
    "answer_en": "He pays for all nationals' holiday trips",
    "image_paths": []
  },
  {
    "question_number": 48,
    "bundesland": "General",
    "question": "Welches Organ gehört nicht zu den Verfassungsorganen Deutschlands?",
    "question_en": "Which body is not one of the constitutional bodies of Germany?",
    "options": [
      "der Bundesrat",
      "die Bundespräsidentin/der Bundespräsident",
      "die Bürgerversammlung",
      "die Regierung"
    ],
    "options_en": [
      "The Federal Council",
      "The Federal President",
      "The citizens' assembly",
      "the government"
    ],
    "answer": "die Bürgerversammlung",
    "answer_en": "The citizens' assembly",
    "image_paths": []
  },
  {
    "question_number": 49,
    "bundesland": "General",
    "question": "Wer bestimmt in Deutschland die Schulpolitik?",
    "question_en": "Who determines school policy in Germany?",
    "options": [
      "die Lehrer und Lehrerinnen",
      "die Bundesländer",
      "das Familienministerium",
      "die Universitäten"
    ],
    "options_en": [
      "The teachers",
      "The federal states",
      "The Ministry of Family",
      "The universities"
    ],
    "answer": "die Bundesländer",
    "answer_en": "The federal states",
    "image_paths": []
  },
  {
    "question_number": 50,
    "bundesland": "General",
    "question": "Die Wirtschaftsform in Deutschland nennt man …",
    "question_en": "The economic system in Germany is called ...",
    "options": [
      "freie Zentralwirtschaft",
      "soziale Marktwirtschaft",
      "gelenkte Zentralwirtschaft",
      "Planwirtschaft"
    ],
    "options_en": [
      "Free central economy",
      "social market economy",
      "guided central economy",
      "Planned economy"
    ],
    "answer": "soziale Marktwirtschaft",
    "answer_en": "social market economy",
    "image_paths": []
  },
  {
    "question_number": 51,
    "bundesland": "General",
    "question": "Zu einem demokratischen Rechtsstaat gehört es nicht, dass …",
    "question_en": "It is not part of a democratic constitutional state that ...",
    "options": [
      "Menschen sich kritisch über die Regierung äußern können",
      "Bürger friedlich demonstrieren gehen dürfen",
      "Menschen von einer Privatpolizei ohne Grund verhaftet werden",
      "jemand ein Verbrechen begeht und deshalb verhaftet wird"
    ],
    "options_en": [
      "People can comment critically about the government",
      "Citizens can demonstrate peacefully",
      "People are arrested by a private police for no reason",
      "someone commits a crime and is therefore arrested"
    ],
    "answer": "Menschen von einer Privatpolizei ohne Grund verhaftet werden",
    "answer_en": "People are arrested by a private police for no reason",
    "image_paths": []
  },
  {
    "question_number": 52,
    "bundesland": "General",
    "question": "Was bedeutet \"Volkssouveränität\"? Alle Staatsgewalt geht vom ...",
    "question_en": "What does \"popular sovereignty\" mean? All state power emanates from the ...",
    "options": [
      "Volke aus",
      "Bundestag aus",
      "preußischen König aus",
      "Bundesverfassungsgericht aus"
    ],
    "options_en": [
      "people",
      "Bundestag",
      "Prussian king",
      "Federal Constitutional Court"
    ],
    "answer": "Volke aus",
    "answer_en": "people",
    "image_paths": []
  },
  {
    "question_number": 53,
    "bundesland": "General",
    "question": "Was bedeutet \"Rechtsstaat\" in Deutschland?",
    "question_en": "What does \"rule of law\" mean in Germany?",
    "options": [
      "Der Staat hat Recht",
      "Es gibt nur rechte Parteien",
      "Die Bürgerinnen und Bürger entscheiden über Gesetze",
      "Der Staat muss die Gesetze einhalten"
    ],
    "options_en": [
      "The state is right",
      "There are only right-wing parties",
      "The citizens decide on laws",
      "The state must comply with the laws"
    ],
    "answer": "Der Staat muss die Gesetze einhalten",
    "answer_en": "The state must comply with the laws",
    "image_paths": []
  },
  {
    "question_number": 54,
    "bundesland": "General",
    "question": "Was ist keine staatliche Gewalt in Deutschland?",
    "question_en": "What is not a branch of government in Germany?",
    "options": [
      "Legislative",
      "Judikative",
      "Exekutive",
      "Direktive"
    ],
    "options_en": [
      "Legislative",
      "Judiciary",
      "Executive",
      "Directive"
    ],
    "answer": "Direktive",
    "answer_en": "Directive",
    "image_paths": []
  },
  {
    "question_number": 55,
    "bundesland": "General",
    "question": "Was zeigt dieses Bild?",
    "question_en": "What does this picture show?",
    "options": [
      "den Bundestagssitz in Berlin",
      "das Bundesverfassungsgericht in Karlsruhe",
      "das Bundesratsgebäude in Berlin",
      "das Bundeskanzleramt in Berlin"
    ],
    "options_en": [
      "The Bundestag headquarters in Berlin",
      "The Federal Constitutional Court in Karlsruhe",
      "The Federal Council building in Berlin",
      "The Federal Chancellery in Berlin"
    ],
    "answer": "den Bundestagssitz in Berlin",
    "answer_en": "The Bundestag headquarters in Berlin",
    "image_paths": [
      "./images/general-55-question.jpg"
    ]
  },
  {
    "question_number": 56,
    "bundesland": "General",
    "question": "Welches Amt gehört in Deutschland zur Gemeindeverwaltung?",
    "question_en": "Which office belongs to the municipal administration in Germany?",
    "options": [
      "Pfarramt",
      "Ordnungsamt",
      "Finanzamt",
      "Auswärtiges Amt"
    ],
    "options_en": [
      "Parish office",
      "Regulatory office",
      "Tax office",
      "Foreign Office"
    ],
    "answer": "Ordnungsamt",
    "answer_en": "Regulatory office",
    "image_paths": []
  },
  {
    "question_number": 57,
    "bundesland": "General",
    "question": "Wer wird meistens zur Präsidentin/zum Präsidenten des Deutschen Bundestages gewählt?",
    "question_en": "Who is usually elected President of the German Bundestag?",
    "options": [
      "die/der älteste Abgeordnete im Parlament",
      "die Ministerpräsidentin/der Ministerpräsident des größten Bundeslandes",
      "eine ehemalige Bundeskanzlerin/ein ehemaliger Bundeskanzler",
      "eine Abgeordnete/ein Abgeordneter der stärksten Fraktion"
    ],
    "options_en": [
      "The oldest member of parliament",
      "The Prime Minister of the largest federal state",
      "A former Chancellor",
      "A member of the strongest parliamentary group"
    ],
    "answer": "eine Abgeordnete/ein Abgeordneter der stärksten Fraktion",
    "answer_en": "A member of the strongest parliamentary group",
    "image_paths": []
  },
  {
    "question_number": 58,
    "bundesland": "General",
    "question": "Wer ernennt in Deutschland die Ministerinnen/die Minister der Bundesregierung?",
    "question_en": "Who appoints the ministers of the federal government in Germany?",
    "options": [
      "die Präsidentin/der Präsident des Bundesverfassungsgerichtes",
      "die Bundespräsidentin/der Bundespräsident",
      "die Bundesratspräsidentin/der Bundesratspräsident",
      "die Bundestagspräsidentin/der Bundestagspräsident"
    ],
    "options_en": [
      "The President of the Federal Constitutional Court",
      "The Federal President",
      "The President of the Federal Council",
      "The President of the Bundestag"
    ],
    "answer": "die Bundespräsidentin/der Bundespräsident",
    "answer_en": "The Federal President",
    "image_paths": []
  },
  {
    "question_number": 59,
    "bundesland": "General",
    "question": "Vor wie vielen Jahren gab es erstmals eine jüdische Gemeinde auf dem Gebiet des heutigen Deutschlands?",
    "question_en": "How many years ago was there a Jewish community in the territory of today's Germany for the first time?",
    "options": [
      "vor etwa 300 Jahren",
      "vor etwa 700 Jahren",
      "vor etwa 1150 Jahren",
      "vor etwa 1700 Jahren"
    ],
    "options_en": [
      "about 300 years ago",
      "about 700 years ago",
      "about 1150 years ago",
      "about 1700 years ago"
    ],
    "answer": "vor etwa 1700 Jahren",
    "answer_en": "about 1700 years ago",
    "image_paths": []
  },
  {
    "question_number": 60,
    "bundesland": "General",
    "question": "In Deutschland gehören der Bundestag und der Bundesrat zur …",
    "question_en": "In Germany, the Bundestag and the Bundesrat belong to the ...",
    "options": [
      "Exekutive",
      "Legislative",
      "Direktive",
      "Judikative"
    ],
    "options_en": [
      "executive",
      "Legislative",
      "directive",
      "Judiciary"
    ],
    "answer": "Legislative",
    "answer_en": "Legislative",
    "image_paths": []
  },
  {
    "question_number": 61,
    "bundesland": "General",
    "question": "Was bedeutet \"Volkssouveränität\"?",
    "question_en": "What does \"popular sovereignty\" mean?",
    "options": [
      "Die Königin/der König herrscht über das Volk",
      "Das Bundesverfassungsgericht steht über der Verfassung",
      "Die Interessenverbände üben die Souveränität zusammen mit der Regierung aus",
      "Die Staatsgewalt geht vom Volke aus"
    ],
    "options_en": [
      "The queen/king rules over the people",
      "The Federal Constitutional Court is above the constitution",
      "The interest groups exercise sovereignty together with the government",
      "The state power emanates from the people"
    ],
    "answer": "Die Staatsgewalt geht vom Volke aus",
    "answer_en": "The state power emanates from the people",
    "image_paths": []
  },
  {
    "question_number": 62,
    "bundesland": "General",
    "question": "Wenn das Parlament eines deutschen Bundeslandes gewählt wird, nennt man das …",
    "question_en": "When the parliament of a German federal state is elected, it is called ...",
    "options": [
      "Kommunalwahl",
      "Landtagswahl",
      "Europawahl",
      "Bundestagswahl"
    ],
    "options_en": [
      "Local election",
      "State election",
      "European election",
      "Federal election"
    ],
    "answer": "Landtagswahl",
    "answer_en": "State election",
    "image_paths": []
  },
  {
    "question_number": 63,
    "bundesland": "General",
    "question": "Was gehört in Deutschland nicht zur Exekutive?",
    "question_en": "What is not part of the executive branch in Germany?",
    "options": [
      "die Polizei",
      "die Gerichte",
      "das Finanzamt",
      "die Ministerien"
    ],
    "options_en": [
      "the police",
      "The courts",
      "The tax office",
      "The ministries"
    ],
    "answer": "die Gerichte",
    "answer_en": "The courts",
    "image_paths": []
  },
  {
    "question_number": 64,
    "bundesland": "General",
    "question": "Die Bundesrepublik Deutschland ist heute gegliedert in …",
    "question_en": "The Federal Republic of Germany is structured today into ...",
    "options": [
      "vier Besatzungszonen",
      "einen Oststaat und einen Weststaat",
      "16 Kantone",
      "Bund, Länder und Kommunen"
    ],
    "options_en": [
      "Four occupation zones",
      "an eastern state and a western state",
      "16 cantons",
      "Federation, states and municipalities"
    ],
    "answer": "Bund, Länder und Kommunen",
    "answer_en": "Federation, states and municipalities",
    "image_paths": []
  },
  {
    "question_number": 65,
    "bundesland": "General",
    "question": "Es gehört nicht zu den Aufgaben des Deutschen Bundestages, …",
    "question_en": "It is not one of the tasks of the German Bundestag to ...",
    "options": [
      "Gesetze zu entwerfen",
      "die Bundesregierung zu kontrollieren",
      "die Bundeskanzlerin/den Bundeskanzler zu wählen",
      "das Bundeskabinett zu bilden"
    ],
    "options_en": [
      "Draft laws",
      "to control the federal government",
      "to elect the Chancellor",
      "to form the federal cabinet"
    ],
    "answer": "das Bundeskabinett zu bilden",
    "answer_en": "to form the federal cabinet",
    "image_paths": []
  },
  {
    "question_number": 66,
    "bundesland": "General",
    "question": "Welche Städte haben die größten jüdischen Gemeinden in Deutschland?",
    "question_en": "Which cities have the largest Jewish communities in Germany?",
    "options": [
      "Berlin und München",
      "Hamburg und Essen",
      "Nürnberg und Stuttgart",
      "Worms und Speyer"
    ],
    "options_en": [
      "Berlin and Munich",
      "Hamburg and Essen",
      "Nuremberg and Stuttgart",
      "Worms and Speyer"
    ],
    "answer": "Berlin und München",
    "answer_en": "Berlin and Munich",
    "image_paths": []
  },
  {
    "question_number": 67,
    "bundesland": "General",
    "question": "Was ist in Deutschland vor allem eine Aufgabe der Bundesländer?",
    "question_en": "What is primarily a task of the federal states in Germany?",
    "options": [
      "Verteidigungspolitik",
      "Außenpolitik",
      "Wirtschaftspolitik",
      "Schulpolitik"
    ],
    "options_en": [
      "Defense policy",
      "Foreign policy",
      "Economic policy",
      "School policy"
    ],
    "answer": "Schulpolitik",
    "answer_en": "School policy",
    "image_paths": []
  },
  {
    "question_number": 68,
    "bundesland": "General",
    "question": "Warum kontrolliert der Staat in Deutschland das Schulwesen?",
    "question_en": "Why does the state in Germany control the school system?",
    "options": [
      "weil es in Deutschland nur staatliche Schulen gibt",
      "weil alle Schülerinnen und Schüler einen Schulabschluss haben müssen",
      "weil es in den Bundesländern verschiedene Schulen gibt",
      "weil es nach dem Grundgesetz seine Aufgabe ist"
    ],
    "options_en": [
      "Because there are only state schools in Germany",
      "Because all students must have a school leaving certificate",
      "Because there are different schools in the federal states",
      "Because it is its task according to the Basic Law"
    ],
    "answer": "weil es nach dem Grundgesetz seine Aufgabe ist",
    "answer_en": "Because it is its task according to the Basic Law",
    "image_paths": []
  },
  {
    "question_number": 69,
    "bundesland": "General",
    "question": "Die Bundesrepublik Deutschland hat einen dreistufigen Verwaltungsaufbau. Wie heißt die unterste politische Stufe?",
    "question_en": "The Federal Republic of Germany has a three-tier administrative structure. What is the name of the lowest political level?",
    "options": [
      "Stadträte",
      "Landräte",
      "Gemeinden",
      "Bezirksämter"
    ],
    "options_en": [
      "City councils",
      "District administrators",
      "Municipalities",
      "District offices"
    ],
    "answer": "Gemeinden",
    "answer_en": "Municipalities",
    "image_paths": []
  },
  {
    "question_number": 70,
    "bundesland": "General",
    "question": "Der deutsche Bundespräsident Gustav Heinemann gibt Helmut Schmidt 1974 die Ernennungsurkunde zum deutschen Bundeskanzler. Was gehört zu den Aufgaben der deutschen Bundespräsidentin/des deutschen Bundespräsidenten?",
    "question_en": "The German Federal President Gustav Heinemann gives Helmut Schmidt the certificate of appointment as German Federal Chancellor in 1974. What are the tasks of the German Federal President?",
    "options": [
      "Sie/Er führt die Regierungsgeschäfte",
      "Sie/Er kontrolliert die Regierungspartei",
      "Sie/Er wählt die Ministerinnen/Minister aus",
      "Sie/Er schlägt die Kanzlerin/den Kanzler zur Wahl vor"
    ],
    "options_en": [
      "She/He leads the government business",
      "She/He controls the government party",
      "She/He selects the ministers",
      "She/He proposes the Chancellor for election"
    ],
    "answer": "Sie/Er schlägt die Kanzlerin/den Kanzler zur Wahl vor",
    "answer_en": "She/He proposes the Chancellor for election",
    "image_paths": ["./images/general-70-question.png"]
  },
  {
    "question_number": 71,
    "bundesland": "General",
    "question": "Wo hält sich die deutsche Bundeskanzlerin/der deutsche Bundeskanzler am häufigsten auf? Am häufigsten ist sie/er …",
    "question_en": "Where does the German Chancellor usually stay? Most often she/he is ...",
    "options": [
      "in Bonn, weil sich dort das Bundeskanzleramt und der Bundestag befinden",
      "auf Schloss Meseberg, dem Gästehaus der Bundesregierung, um Staatsgäste zu empfangen",
      "auf Schloss Bellevue, dem Amtssitz der Bundespräsidentin/des Bundespräsidenten, um Staatsgäste zu empfangen",
      "in Berlin, weil sich dort das Bundeskanzleramt und der Bundestag befinden"
    ],
    "options_en": [
      "in Bonn because the Federal Chancellery and the Bundestag are there",
      "At Meseberg Castle, the Federal Government's guest house to receive state guests",
      "At Bellevue Castle, the official residence of the Federal President, to receive state guests",
      "in Berlin because the Federal Chancellery and the Bundestag are there"
    ],
    "answer": "in Berlin, weil sich dort das Bundeskanzleramt und der Bundestag befinden",
    "answer_en": "in Berlin because the Federal Chancellery and the Bundestag are there",
    "image_paths": []
  },
  {
    "question_number": 72,
    "bundesland": "General",
    "question": "Wie heißt der jetzige Bundeskanzler von Deutschland?",
    "question_en": "What is the name of the current Chancellor of Germany?",
    "options": [
      "Gerhard Schröder",
      "Angela Merkel",
      "Olaf Scholz",
      "Friedrich Merz"
    ],
    "options_en": [
      "Gerhard Schröder",
      "Angela Merkel",
      "Olaf Scholz",
      "Friedrich Merz"
    ],
    "answer": "Friedrich Merz",
    "answer_en": "Friedrich Merz",
    "image_paths": []
  },
  {
    "question_number": 73,
    "bundesland": "General",
    "question": "Die beiden größten Fraktionen im Deutschen Bundestag heißen zurzeit …",
    "question_en": "The two largest parliamentary groups in the German Bundestag are currently ...",
    "options": [
      "CDU/CSU und SPD",
      "Die Linke und Bündnis 90/Die Grünen",
      "FDP und SPD",
      "Die Linke und CDU/CSU"
    ],
    "options_en": [
      "CDU/CSU and SPD",
      "The Left and Alliance 90/The Greens",
      "FDP and SPD",
      "The Left and CDU/CSU"
    ],
    "answer": "CDU/CSU und SPD",
    "answer_en": "CDU/CSU and SPD",
    "image_paths": []
  },
  {
    "question_number": 74,
    "bundesland": "General",
    "question": "Wie heißt das Parlament für ganz Deutschland?",
    "question_en": "What is the name of the parliament for all of Germany?",
    "options": [
      "Bundesversammlung",
      "Volkskammer",
      "Bundestag",
      "Bundesgerichtshof"
    ],
    "options_en": [
      "Federal Assembly",
      "People's Chamber",
      "Bundestag",
      "Federal Court of Justice"
    ],
    "answer": "Bundestag",
    "answer_en": "Bundestag",
    "image_paths": []
  },
  {
    "question_number": 75,
    "bundesland": "General",
    "question": "Wie heißt Deutschlands heutiges Staatsoberhaupt?",
    "question_en": "What is the name of Germany's current head of state?",
    "options": [
      "Frank-Walter Steinmeier",
      "Bärbel Bas",
      "Bodo Ramelow",
      "Joachim Gauck"
    ],
    "options_en": [
      "Frank-Walter Steinmeier",
      "Bärbel Bas",
      "Bodo Ramelow",
      "Joachim Gauck"
    ],
    "answer": "Frank-Walter Steinmeier",
    "answer_en": "Frank-Walter Steinmeier",
    "image_paths": []
  },
  {
    "question_number": 76,
    "bundesland": "General",
    "question": "Was bedeutet die Abkürzung CDU in Deutschland?",
    "question_en": "What does the abbreviation CDU mean in Germany?",
    "options": [
      "Christliche Deutsche Union",
      "Club Deutscher Unternehmer",
      "Christlicher Deutscher Umweltschutz",
      "Christlich Demokratische Union"
    ],
    "options_en": [
      "Christian German Union",
      "Club of German entrepreneurs",
      "Christian German environmental protection",
      "Christian Democratic Union"
    ],
    "answer": "Christlich Demokratische Union",
    "answer_en": "Christian Democratic Union",
    "image_paths": []
  },
  {
    "question_number": 77,
    "bundesland": "General",
    "question": "Was ist die Bundeswehr?",
    "question_en": "What is the Bundeswehr?",
    "options": [
      "die deutsche Polizei",
      "ein deutscher Hafen",
      "eine deutsche Bürgerinitiative",
      "die deutsche Armee"
    ],
    "options_en": [
      "The German police",
      "A German port",
      "A German citizens' initiative",
      "The German army"
    ],
    "answer": "die deutsche Armee",
    "answer_en": "The German army",
    "image_paths": []
  },
  {
    "question_number": 78,
    "bundesland": "General",
    "question": "Was bedeutet die Abkürzung SPD?",
    "question_en": "What does the abbreviation SPD mean?",
    "options": [
      "Sozialistische Partei Deutschlands",
      "Sozialpolitische Partei Deutschlands",
      "Sozialdemokratische Partei Deutschlands",
      "Sozialgerechte Partei Deutschlands"
    ],
    "options_en": [
      "Socialist Party of Germany",
      "Social Political Party of Germany",
      "Social Democratic Party of Germany",
      "Socially Just Party of Germany"
    ],
    "answer": "Sozialdemokratische Partei Deutschlands",
    "answer_en": "Social Democratic Party of Germany",
    "image_paths": []
  },
  {
    "question_number": 79,
    "bundesland": "General",
    "question": "Was bedeutet die Abkürzung FDP in Deutschland?",
    "question_en": "What does the abbreviation FDP mean in Germany?",
    "options": [
      "Friedliche Demonstrative Partei",
      "Freie Deutschland Partei",
      "Führende Demokratische Partei",
      "Freie Demokratische Partei"
    ],
    "options_en": [
      "Peaceful Demonstrative Party",
      "Free Germany Party",
      "Leading Democratic Party",
      "Free Democratic Party"
    ],
    "answer": "Freie Demokratische Partei",
    "answer_en": "Free Democratic Party",
    "image_paths": []
  },
  {
    "question_number": 80,
    "bundesland": "General",
    "question": "Welches Gericht in Deutschland ist zuständig für die Auslegung des Grundgesetzes?",
    "question_en": "Which court in Germany is responsible for the interpretation of the Basic Law?",
    "options": [
      "Oberlandesgericht",
      "Amtsgericht",
      "Bundesverfassungsgericht",
      "Verwaltungsgericht"
    ],
    "options_en": [
      "Higher Regional Court",
      "District court",
      "Federal Constitutional Court",
      "Administrative court"
    ],
    "answer": "Bundesverfassungsgericht",
    "answer_en": "Federal Constitutional Court",
    "image_paths": []
  },
  {
    "question_number": 81,
    "bundesland": "General",
    "question": "Wer wählt die Bundeskanzlerin/den Bundeskanzler in Deutschland?",
    "question_en": "Who elects the Chancellor in Germany?",
    "options": [
      "der Bundesrat",
      "die Bundesversammlung",
      "das Volk",
      "der Bundestag"
    ],
    "options_en": [
      "The Federal Council",
      "The Federal Assembly",
      "The people",
      "The Bundestag"
    ],
    "answer": "der Bundestag",
    "answer_en": "The Bundestag",
    "image_paths": []
  },
  {
    "question_number": 82,
    "bundesland": "General",
    "question": "Wer leitet das deutsche Bundeskabinett?",
    "question_en": "Who leads the German Federal Cabinet?",
    "options": [
      "die Bundestagspräsidentin/der Bundestagspräsident",
      "die Bundespräsidentin/der Bundespräsident",
      "die Bundesratspräsidentin/der Bundesratspräsident",
      "die Bundeskanzlerin/der Bundeskanzler"
    ],
    "options_en": [
      "The President of the Bundestag",
      "The Federal President",
      "The President of the Federal Council",
      "The Chancellor"
    ],
    "answer": "die Bundeskanzlerin/der Bundeskanzler",
    "answer_en": "The Chancellor",
    "image_paths": []
  },
  {
    "question_number": 83,
    "bundesland": "General",
    "question": "Wer wählt die deutsche Bundeskanzlerin/den deutschen Bundeskanzler?",
    "question_en": "Who elects the German Chancellor?",
    "options": [
      "das Volk",
      "die Bundesversammlung",
      "der Bundestag",
      "die Bundesregierung"
    ],
    "options_en": [
      "The people",
      "The Federal Assembly",
      "The Bundestag",
      "The federal government"
    ],
    "answer": "der Bundestag",
    "answer_en": "The Bundestag",
    "image_paths": []
  },
  {
    "question_number": 84,
    "bundesland": "General",
    "question": "Welche Hauptaufgabe hat die deutsche Bundespräsidentin/der deutsche Bundespräsident? Sie/Er …",
    "question_en": "What is the main task of the German Federal President? She/He ...",
    "options": [
      "regiert das Land",
      "entwirft die Gesetze",
      "repräsentiert das Land",
      "überwacht die Einhaltung der Gesetze"
    ],
    "options_en": [
      "rules the country",
      "drafts the laws",
      "represents the country",
      "monitors compliance with the laws"
    ],
    "answer": "repräsentiert das Land",
    "answer_en": "represents the country",
    "image_paths": []
  },
  {
    "question_number": 85,
    "bundesland": "General",
    "question": "Wer bildet den deutschen Bundesrat?",
    "question_en": "Who forms the German Federal Council?",
    "options": [
      "die Abgeordneten des Bundestages",
      "die Ministerinnen und Minister der Bundesregierung",
      "die Regierungsvertreter der Bundesländer",
      "die Parteimitglieder"
    ],
    "options_en": [
      "The members of the Bundestag",
      "The ministers of the federal government",
      "The government representatives of the federal states",
      "The party members"
    ],
    "answer": "die Regierungsvertreter der Bundesländer",
    "answer_en": "The government representatives of the federal states",
    "image_paths": []
  },
  {
    "question_number": 86,
    "bundesland": "General",
    "question": "Wer wählt in Deutschland die Bundespräsidentin/den Bundespräsidenten?",
    "question_en": "Who elects the Federal President in Germany?",
    "options": [
      "die Bundesversammlung",
      "der Bundesrat",
      "das Bundesparlament",
      "das Bundesverfassungsgericht"
    ],
    "options_en": [
      "The Federal Assembly",
      "The Federal Council",
      "The Federal Parliament",
      "The Federal Constitutional Court"
    ],
    "answer": "die Bundesversammlung",
    "answer_en": "The Federal Assembly",
    "image_paths": []
  },
  {
    "question_number": 87,
    "bundesland": "General",
    "question": "Wer ist das Staatsoberhaupt der Bundesrepublik Deutschland?",
    "question_en": "Who is the head of state of the Federal Republic of Germany?",
    "options": [
      "die Bundeskanzlerin/der Bundeskanzler",
      "die Bundespräsidentin/der Bundespräsident",
      "die Bundesratspräsidentin/der Bundesratspräsident",
      "die Bundestagspräsidentin/der Bundestagspräsident"
    ],
    "options_en": [
      "The Chancellor",
      "The Federal President",
      "The President of the Federal Council",
      "The President of the Bundestag"
    ],
    "answer": "die Bundespräsidentin/der Bundespräsident",
    "answer_en": "The Federal President",
    "image_paths": []
  },
  {
    "question_number": 88,
    "bundesland": "General",
    "question": "Die parlamentarische Opposition im Deutschen Bundestag …",
    "question_en": "The parliamentary opposition in the German Bundestag ...",
    "options": [
      "kontrolliert die Regierung",
      "entscheidet, wer Bundesministerin/Bundesminister wird",
      "bestimmt, wer im Bundesrat sitzt",
      "schlägt die Regierungschefinnen/Regierungschefs der Länder vor"
    ],
    "options_en": [
      "controls the government",
      "decides who becomes Federal Minister",
      "determines who sits in the Federal Council",
      "proposes the heads of government of the states"
    ],
    "answer": "kontrolliert die Regierung",
    "answer_en": "controls the government",
    "image_paths": []
  },
  {
    "question_number": 89,
    "bundesland": "General",
    "question": "Wie nennt man in Deutschland die Vereinigung von Abgeordneten einer Partei im Parlament?",
    "question_en": "What is the name of the association of members of a party in parliament in Germany?",
    "options": [
      "Verband",
      "Ältestenrat",
      "Fraktion",
      "Opposition"
    ],
    "options_en": [
      "Association",
      "Council of Elders",
      "Parliamentary group",
      "opposition"
    ],
    "answer": "Fraktion",
    "answer_en": "Parliamentary group",
    "image_paths": []
  },
  {
    "question_number": 90,
    "bundesland": "General",
    "question": "Die deutschen Bundesländer wirken an der Gesetzgebung des Bundes mit durch …",
    "question_en": "The German federal states participate in the legislation of the Federation through ...",
    "options": [
      "den Bundesrat",
      "die Bundesversammlung",
      "den Bundestag",
      "die Bundesregierung"
    ],
    "options_en": [
      "the Federal Council",
      "The Federal Assembly",
      "the Bundestag",
      "The federal government"
    ],
    "answer": "den Bundesrat",
    "answer_en": "the Federal Council",
    "image_paths": []
  },
  {
    "question_number": 91,
    "bundesland": "General",
    "question": "In Deutschland kann ein Regierungswechsel in einem Bundesland Auswirkungen auf die Bundespolitik haben. Das Regieren wird …",
    "question_en": "In Germany, a change of government in a federal state can have an impact on federal politics. Governing becomes ...",
    "options": [
      "schwieriger, wenn sich dadurch die Mehrheit im Bundestag ändert",
      "leichter, wenn dadurch neue Parteien in den Bundesrat kommen",
      "schwieriger, wenn dadurch die Mehrheit im Bundesrat verändert wird",
      "leichter, wenn es sich um ein reiches Bundesland handelt"
    ],
    "options_en": [
      "more difficult if this changes the majority in the Bundestag",
      "easier when new parties come to the Federal Council",
      "more difficult if the majority in the Federal Council is changed as a result",
      "easier if it is a rich federal state"
    ],
    "answer": "schwieriger, wenn dadurch die Mehrheit im Bundesrat verändert wird",
    "answer_en": "more difficult if the majority in the Federal Council is changed as a result",
    "image_paths": []
  },
  {
    "question_number": 92,
    "bundesland": "General",
    "question": "Was bedeutet die Abkürzung CSU in Deutschland?",
    "question_en": "What does the abbreviation CSU mean in Germany?",
    "options": [
      "Christlich Sichere Union",
      "Christlich Süddeutsche Union",
      "Christlich Sozialer Unternehmerverband",
      "Christlich Soziale Union"
    ],
    "options_en": [
      "Christian Safe Union",
      "Christian South German Union",
      "Christian Social Entrepreneurs' Association",
      "Christian Social Union"
    ],
    "answer": "Christlich Soziale Union",
    "answer_en": "Christian Social Union",
    "image_paths": []
  },
  {
    "question_number": 93,
    "bundesland": "General",
    "question": "Je mehr \"Zweitstimmen\" eine Partei bei einer Bundestagswahl bekommt, desto …",
    "question_en": "The more \"second votes\" a party receives in a federal election, the ...",
    "options": [
      "weniger Erststimmen kann sie haben",
      "mehr Direktkandidaten der Partei ziehen ins Parlament ein",
      "größer ist das Risiko, eine Koalition bilden zu müssen",
      "mehr Sitze erhält die Partei im Parlament"
    ],
    "options_en": [
      "fewer first votes it can have",
      "more direct candidates of the party move into parliament",
      "greater the risk of having to form a coalition",
      "more seats the party receives in parliament"
    ],
    "answer": "mehr Sitze erhält die Partei im Parlament",
    "answer_en": "more seats the party receives in parliament",
    "image_paths": []
  },
  {
    "question_number": 94,
    "bundesland": "General",
    "question": "Ab welchem Alter darf man in Deutschland an der Wahl zum Deutschen Bundestag teilnehmen?",
    "question_en": "From what age can one participate in the election to the German Bundestag in Germany?",
    "options": [
      "16",
      "18",
      "21",
      "23"
    ],
    "options_en": [
      "16",
      "18",
      "21",
      "23"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 95,
    "bundesland": "General",
    "question": "Was gilt für die meisten Kinder in Deutschland?",
    "question_en": "What applies to most children in Germany?",
    "options": [
      "Wahlpflicht",
      "Schulpflicht",
      "Schweigepflicht",
      "Religionspflicht"
    ],
    "options_en": [
      "Compulsory voting",
      "Compulsory schooling",
      "Duty of confidentiality",
      "Religious obligation"
    ],
    "answer": "Schulpflicht",
    "answer_en": "Compulsory schooling",
    "image_paths": []
  },
  {
    "question_number": 96,
    "bundesland": "General",
    "question": "Wie kann jemand, der den Holocaust leugnet, bestraft werden?",
    "question_en": "How can someone who denies the Holocaust be punished?",
    "options": [
      "Kürzung sozialer Leistungen",
      "bis zu 100 Sozialstunden",
      "gar nicht, Holocaustleugnung ist erlaubt",
      "mit Freiheitsstrafe bis zu fünf Jahren oder mit Geldstrafe"
    ],
    "options_en": [
      "Reduction of social benefits",
      "up to 100 hours of community service",
      "not at all, Holocaust denial is permitted",
      "with imprisonment of up to five years or with a fine"
    ],
    "answer": "mit Freiheitsstrafe bis zu fünf Jahren oder mit Geldstrafe",
    "answer_en": "with imprisonment of up to five years or with a fine",
    "image_paths": []
  },
  {
    "question_number": 97,
    "bundesland": "General",
    "question": "Was bezahlt man in Deutschland automatisch, wenn man fest angestellt ist?",
    "question_en": "What do you automatically pay in Germany when you are permanently employed?",
    "options": [
      "Sozialversicherung",
      "Sozialhilfe",
      "Kindergeld",
      "Wohngeld"
    ],
    "options_en": [
      "social security",
      "Social assistance",
      "Child benefit",
      "Housing benefit"
    ],
    "answer": "Sozialversicherung",
    "answer_en": "social security",
    "image_paths": []
  },
  {
    "question_number": 98,
    "bundesland": "General",
    "question": "Wenn Abgeordnete im Deutschen Bundestag ihre Fraktion wechseln, …",
    "question_en": "When members of the German Bundestag change their parliamentary group, ...",
    "options": [
      "dürfen sie nicht mehr an den Sitzungen des Parlaments teilnehmen",
      "kann die Regierung ihre Mehrheit verlieren",
      "muss die Bundespräsidentin/der Bundespräsident zuvor ihr/sein Einverständnis geben",
      "dürfen die Wählerinnen/Wähler dieser Abgeordneten noch einmal wählen"
    ],
    "options_en": [
      "they are no longer allowed to participate in the sessions of parliament",
      "the government can lose its majority",
      "the Federal President must give his/her consent beforehand",
      "the voters of these members are allowed to vote again"
    ],
    "answer": "kann die Regierung ihre Mehrheit verlieren",
    "answer_en": "the government can lose its majority",
    "image_paths": []
  },
  {
    "question_number": 99,
    "bundesland": "General",
    "question": "Wer bezahlt in Deutschland die Sozialversicherungen?",
    "question_en": "Who pays for social security in Germany?",
    "options": [
      "Arbeitgeberinnen/Arbeitgeber und Arbeitnehmerinnen/Arbeitnehmer",
      "nur Arbeitnehmerinnen/Arbeitnehmer",
      "alle Staatsangehörigen",
      "nur Arbeitgeberinnen/Arbeitgeber"
    ],
    "options_en": [
      "Employers and employees",
      "Only employees",
      "All nationals",
      "Only employers"
    ],
    "answer": "Arbeitgeberinnen/Arbeitgeber und Arbeitnehmerinnen/Arbeitnehmer",
    "answer_en": "Employers and employees",
    "image_paths": []
  },
  {
    "question_number": 100,
    "bundesland": "General",
    "question": "Was gehört nicht zur gesetzlichen Sozialversicherung?",
    "question_en": "What is not part of the statutory social security system?",
    "options": [
      "die Lebensversicherung",
      "die gesetzliche Rentenversicherung",
      "die Arbeitslosenversicherung",
      "die Pflegeversicherung"
    ],
    "options_en": [
      "life insurance",
      "The statutory pension insurance",
      "Unemployment insurance",
      "The long-term care insurance"
    ],
    "answer": "die Lebensversicherung",
    "answer_en": "life insurance",
    "image_paths": []
  },
  {
    "question_number": 101,
    "bundesland": "General",
    "question": "Gewerkschaften sind Interessenverbände der …",
    "question_en": "Trade unions are interest groups of the ...",
    "options": [
      "Jugendlichen",
      "Arbeitnehmerinnen und Arbeitnehmer",
      "Rentnerinnen und Rentner",
      "Arbeitgeberinnen und Arbeitgeber"
    ],
    "options_en": [
      "Youth",
      "Employees",
      "Pensioners",
      "Employers"
    ],
    "answer": "Arbeitnehmerinnen und Arbeitnehmer",
    "answer_en": "Employees",
    "image_paths": []
  },
  {
    "question_number": 102,
    "bundesland": "General",
    "question": "Womit kann man in der Bundesrepublik Deutschland geehrt werden, wenn man auf politischem, wirtschaftlichem, kulturellem, geistigem oder sozialem Gebiet eine besondere Leistung erbracht hat? Mit dem …",
    "question_en": "With what can one be honored in the Federal Republic of Germany if one has rendered a special service in the political, economic, cultural, intellectual or social field? With the ...",
    "options": [
      "Bundesverdienstkreuz",
      "Bundesadler",
      "Vaterländischen Verdienstorden",
      "Ehrentitel \"Held der Deutschen Demokratischen Republik\""
    ],
    "options_en": [
      "Federal Cross of Merit",
      "Federal eagle",
      "Patriotic Order of Merit",
      "Honorary title \"Hero of the German Democratic Republic\""
    ],
    "answer": "Bundesverdienstkreuz",
    "answer_en": "Federal Cross of Merit",
    "image_paths": []
  },
  {
    "question_number": 103,
    "bundesland": "General",
    "question": "Was wird in Deutschland als \"Ampelkoalition\" bezeichnet? Die Zusammenarbeit …",
    "question_en": "What is referred to in Germany as a \"traffic light coalition\"? The cooperation ...",
    "options": [
      "der Bundestagsfraktionen von CDU und CSU",
      "von SPD, FDP und Bündnis 90/Die Grünen in einer Regierung",
      "von CSU, Die LINKE und Bündnis 90/Die Grünen in einer Regierung",
      "der Bundestagsfraktionen von CDU und SPD"
    ],
    "options_en": [
      "of the parliamentary groups of CDU and CSU",
      "of SPD, FDP and Alliance 90/The Greens in a government",
      "of CSU, The Left and Alliance 90/The Greens in a government",
      "of the parliamentary groups of CDU and SPD"
    ],
    "answer": "von SPD, FDP und Bündnis 90/Die Grünen in einer Regierung",
    "answer_en": "of SPD, FDP and Alliance 90/The Greens in a government",
    "image_paths": []
  },
  {
    "question_number": 104,
    "bundesland": "General",
    "question": "Eine Frau in Deutschland verliert ihre Arbeit. Was darf nicht der Grund für diese Entlassung sein?",
    "question_en": "A woman in Germany loses her job. What must not be the reason for this dismissal?",
    "options": [
      "Die Frau ist lange krank und arbeitsunfähig",
      "Die Frau kam oft zu spät zur Arbeit",
      "Die Frau erledigt private Sachen während der Arbeitszeit",
      "Die Frau bekommt ein Kind und ihr Chef weiß das"
    ],
    "options_en": [
      "The woman is ill for a long time and unable to work",
      "The woman was often late for work",
      "The woman does private things during working hours",
      "The woman is having a child and her boss knows it"
    ],
    "answer": "Die Frau bekommt ein Kind und ihr Chef weiß das",
    "answer_en": "The woman is having a child and her boss knows it",
    "image_paths": []
  },
  {
    "question_number": 105,
    "bundesland": "General",
    "question": "Was ist eine Aufgabe von Wahlhelferinnen/Wahlhelfern in Deutschland?",
    "question_en": "What is a task of election helpers in Germany?",
    "options": [
      "Sie helfen alten Menschen bei der Stimmabgabe in der Wahlkabine",
      "Sie schreiben die Wahlbenachrichtigungen vor der Wahl",
      "Sie geben Zwischenergebnisse an die Medien weiter",
      "Sie zählen die Stimmen nach dem Ende der Wahl"
    ],
    "options_en": [
      "They help old people to vote in the polling booth",
      "They write the election notifications before the election",
      "They pass on interim results to the media",
      "They count the votes after the end of the election"
    ],
    "answer": "Sie zählen die Stimmen nach dem Ende der Wahl",
    "answer_en": "They count the votes after the end of the election",
    "image_paths": []
  },
  {
    "question_number": 106,
    "bundesland": "General",
    "question": "In Deutschland helfen ehrenamtliche Wahlhelferinnen und Wahlhelfer bei den Wahlen. Was ist eine Aufgabe von Wahlhelferinnen/Wahlhelfern?",
    "question_en": "In Germany, volunteer election helpers assist in the elections. What is a task of election helpers?",
    "options": [
      "Sie helfen Kindern und alten Menschen beim Wählen",
      "Sie schreiben Karten und Briefe mit der Angabe des Wahllokals",
      "Sie geben Zwischenergebnisse an Journalisten weiter",
      "Sie zählen die Stimmen nach dem Ende der Wahl"
    ],
    "options_en": [
      "They help children and old people to vote",
      "They write cards and letters specifying the polling station",
      "They pass on interim results to journalists",
      "They count the votes after the end of the election"
    ],
    "answer": "Sie zählen die Stimmen nach dem Ende der Wahl",
    "answer_en": "They count the votes after the end of the election",
    "image_paths": []
  },
  {
    "question_number": 107,
    "bundesland": "General",
    "question": "Für wie viele Jahre wird der Bundestag in Deutschland gewählt?",
    "question_en": "For how many years is the Bundestag elected in Germany?",
    "options": [
      "2 Jahre",
      "4 Jahre",
      "6 Jahre",
      "8 Jahre"
    ],
    "options_en": [
      "2 years",
      "4 years",
      "6 years",
      "8 years"
    ],
    "answer": "4 Jahre",
    "answer_en": "4 years",
    "image_paths": []
  },
  {
    "question_number": 108,
    "bundesland": "General",
    "question": "Bei einer Bundestagswahl in Deutschland darf jede/jeder wählen, die/der …",
    "question_en": "In a federal election in Germany, everyone is allowed to vote who ...",
    "options": [
      "in der Bundesrepublik Deutschland wohnt und wählen möchte",
      "Bürgerin/Bürger der Bundesrepublik Deutschland ist und mindestens 18 Jahre alt ist",
      "seit mindestens 3 Jahren in der Bundesrepublik Deutschland lebt",
      "Bürgerin/Bürger der Bundesrepublik Deutschland ist und mindestens 21 Jahre alt ist"
    ],
    "options_en": [
      "lives in the Federal Republic of Germany and wants to vote",
      "is a citizen of the Federal Republic of Germany and is at least 18 years old",
      "has lived in the Federal Republic of Germany for at least 3 years",
      "is a citizen of the Federal Republic of Germany and is at least 21 years old"
    ],
    "answer": "Bürgerin/Bürger der Bundesrepublik Deutschland ist und mindestens 18 Jahre alt ist",
    "answer_en": "is a citizen of the Federal Republic of Germany and is at least 18 years old",
    "image_paths": []
  },
  {
    "question_number": 109,
    "bundesland": "General",
    "question": "Wie oft gibt es normalerweise Bundestagswahlen in Deutschland?",
    "question_en": "How often are there usually federal elections in Germany?",
    "options": [
      "alle drei Jahre",
      "alle vier Jahre",
      "alle fünf Jahre",
      "alle sechs Jahre"
    ],
    "options_en": [
      "every three years",
      "every four years",
      "every five years",
      "every six years"
    ],
    "answer": "alle vier Jahre",
    "answer_en": "every four years",
    "image_paths": []
  },
  {
    "question_number": 110,
    "bundesland": "General",
    "question": "Für wie viele Jahre wird der Bundestag in Deutschland gewählt?",
    "question_en": "For how many years is the Bundestag elected in Germany?",
    "options": [
      "2 Jahre",
      "3 Jahre",
      "4 Jahre",
      "5 Jahre"
    ],
    "options_en": [
      "2 years",
      "3 years",
      "4 years",
      "5 years"
    ],
    "answer": "4 Jahre",
    "answer_en": "4 years",
    "image_paths": []
  },
  {
    "question_number": 111,
    "bundesland": "General",
    "question": "Welche Handlungen mit Bezug auf den Staat Israel sind in Deutschland verboten?",
    "question_en": "What actions related to the State of Israel are forbidden in Germany?",
    "options": [
      "die Politik Israels öffentlich kritisieren",
      "das Aufhängen einer israelischen Flagge auf dem Privatgrundstück",
      "eine Diskussion über die Politik Israels",
      "der öffentliche Aufruf zur Vernichtung Israels"
    ],
    "options_en": [
      "publicly criticizing Israel's policies",
      "hanging an Israeli flag on private property",
      "a discussion about Israel's policies",
      "the public call for the destruction of Israel"
    ],
    "answer": "der öffentliche Aufruf zur Vernichtung Israels",
    "answer_en": "the public call for the destruction of Israel",
    "image_paths": []
  },
  {
    "question_number": 112,
    "bundesland": "General",
    "question": "Die Wahlen in Deutschland sind …",
    "question_en": "The elections in Germany are ...",
    "options": [
      "speziell",
      "geheim",
      "berufsbezogen",
      "geschlechtsabhängig"
    ],
    "options_en": [
      "special",
      "secret",
      "job-related",
      "gender-dependent"
    ],
    "answer": "geheim",
    "answer_en": "secret",
    "image_paths": []
  },
  {
    "question_number": 113,
    "bundesland": "General",
    "question": "Wahlen in Deutschland gewinnt die Partei, die …",
    "question_en": "Elections in Germany are won by the party that ...",
    "options": [
      "die meisten Stimmen bekommt",
      "die meisten Männer mehrheitlich gewählt haben",
      "die meisten Stimmen bei den Arbeiterinnen/Arbeitern bekommen hat",
      "die meisten Erststimmen für ihre Kanzlerkandidatin/ihren Kanzlerkandidaten erhalten hat"
    ],
    "options_en": [
      "gets the most votes",
      "the majority of men have elected",
      "received the most votes from the workers",
      "has received the most first votes for its candidate for chancellor"
    ],
    "answer": "die meisten Stimmen bekommt",
    "answer_en": "gets the most votes",
    "image_paths": []
  },
  {
    "question_number": 114,
    "bundesland": "General",
    "question": "An demokratischen Wahlen in Deutschland teilzunehmen ist …",
    "question_en": "Participating in democratic elections in Germany is ...",
    "options": [
      "eine Pflicht",
      "ein Recht",
      "ein Zwang",
      "eine Last"
    ],
    "options_en": [
      "a duty",
      "a right",
      "a compulsion",
      "a burden"
    ],
    "answer": "ein Recht",
    "answer_en": "a right",
    "image_paths": []
  },
  {
    "question_number": 115,
    "bundesland": "General",
    "question": "Was bedeutet \"aktives Wahlrecht\" in Deutschland?",
    "question_en": "What does \"active right to vote\" mean in Germany?",
    "options": [
      "Man kann gewählt werden",
      "Man muss wählen gehen",
      "Man kann wählen",
      "Man muss zur Auszählung der Stimmen gehen"
    ],
    "options_en": [
      "One can be elected",
      "One must go to vote",
      "One can vote",
      "One must go to the counting of the votes"
    ],
    "answer": "Man kann wählen",
    "answer_en": "One can vote",
    "image_paths": []
  },
  {
    "question_number": 116,
    "bundesland": "General",
    "question": "Wenn Sie bei einer Bundestagswahl in Deutschland wählen dürfen, heißt das …",
    "question_en": "If you are allowed to vote in a federal election in Germany, that means ...",
    "options": [
      "aktive Wahlkampagne",
      "aktives Wahlverfahren",
      "aktiver Wahlkampf",
      "aktives Wahlrecht"
    ],
    "options_en": [
      "Active election campaign",
      "Active election procedure",
      "active election campaign",
      "Active right to vote"
    ],
    "answer": "aktives Wahlrecht",
    "answer_en": "Active right to vote",
    "image_paths": []
  },
  {
    "question_number": 117,
    "bundesland": "General",
    "question": "Wie viel Prozent der Zweitstimmen müssen Parteien mindestens bekommen, um in den Deutschen Bundestag gewählt zu werden?",
    "question_en": "What percentage of the second votes must parties receive at a minimum to be elected to the German Bundestag?",
    "options": [
      "3%",
      "4%",
      "5%",
      "6%"
    ],
    "options_en": [
      "3%",
      "4%",
      "5%",
      "6%"
    ],
    "answer": "5%",
    "answer_en": "5%",
    "image_paths": []
  },
  {
    "question_number": 118,
    "bundesland": "General",
    "question": "Wer darf bei den rund 40 jüdischen Makkabi-Sportvereinen Mitglied werden?",
    "question_en": "Who can become a member of the approximately 40 Jewish Maccabi sports clubs?",
    "options": [
      "nur Deutsche",
      "nur Israelis",
      "nur religiöse Menschen",
      "alle Menschen"
    ],
    "options_en": [
      "only Germans",
      "only Israelis",
      "only religious people",
      "all people"
    ],
    "answer": "alle Menschen",
    "answer_en": "all people",
    "image_paths": []
  },
  {
    "question_number": 119,
    "bundesland": "General",
    "question": "Wahlen in Deutschland sind frei. Was bedeutet das?",
    "question_en": "Elections in Germany are free. What does that mean?",
    "options": [
      "Alle verurteilten Straftäterinnen/Straftäter dürfen nicht wählen",
      "Wenn ich wählen gehen möchte, muss meine Arbeitgeberin/mein Arbeitgeber mir frei geben",
      "Jede Person kann ohne Zwang entscheiden, ob sie wählen möchte und wen sie wählen möchte",
      "Ich kann frei entscheiden, wo ich wählen gehen möchte"
    ],
    "options_en": [
      "All convicted criminals are not allowed to vote",
      "If I want to go to vote, my employer must give me time off",
      "Every person can decide without coercion whether they want to vote and whom they want to vote for",
      "I can freely decide where I want to go to vote"
    ],
    "answer": "Jede Person kann ohne Zwang entscheiden, ob sie wählen möchte und wen sie wählen möchte",
    "answer_en": "Every person can decide without coercion whether they want to vote and whom they want to vote for",
    "image_paths": []
  },
  {
    "question_number": 120,
    "bundesland": "General",
    "question": "Das Wahlsystem in Deutschland ist ein …",
    "question_en": "The electoral system in Germany is a ...",
    "options": [
      "Zensuswahlrecht",
      "Dreiklassenwahlrecht",
      "Mehrheits- und Verhältniswahlrecht",
      "allgemeines Männerwahlrecht"
    ],
    "options_en": [
      "Census suffrage",
      "Three-class franchise",
      "Majority and proportional representation",
      "general male suffrage"
    ],
    "answer": "Mehrheits- und Verhältniswahlrecht",
    "answer_en": "Majority and proportional representation",
    "image_paths": []
  },
  {
    "question_number": 121,
    "bundesland": "General",
    "question": "Eine Partei möchte in den Deutschen Bundestag. Sie muss aber einen Mindestanteil an Wählerstimmen haben. Das heißt …",
    "question_en": "A party wants to get into the German Bundestag. However, it must have a minimum share of the votes. This is called ...",
    "options": [
      "5%-Hürde",
      "Zulassungsgrenze",
      "Basiswert",
      "Richtlinie"
    ],
    "options_en": [
      "5% hurdle",
      "Admission limit",
      "Base value",
      "Guideline"
    ],
    "answer": "5%-Hürde",
    "answer_en": "5% hurdle",
    "image_paths": []
  },
  {
    "question_number": 122,
    "bundesland": "General",
    "question": "Welchem Grundsatz unterliegen Wahlen in Deutschland? Wahlen in Deutschland sind …",
    "question_en": "What principle are elections in Germany subject to? Elections in Germany are ...",
    "options": [
      "frei, gleich, geheim",
      "offen, sicher, frei",
      "geschlossen, gleich, sicher",
      "sicher, offen, freiwillig"
    ],
    "options_en": [
      "free, equal, secret",
      "Open, safe, free",
      "closed, equal, safe",
      "Safe, open, voluntary"
    ],
    "answer": "frei, gleich, geheim",
    "answer_en": "free, equal, secret",
    "image_paths": []
  },
  {
    "question_number": 123,
    "bundesland": "General",
    "question": "Was ist in Deutschland die \"5%-Hürde\"?",
    "question_en": "What is the \"5% hurdle\" in Germany?",
    "options": [
      "Abstimmungsregelung im Bundestag für kleine Parteien",
      "Anwesenheitskontrolle im Bundestag für Abstimmungen",
      "Mindestanteil an Wählerstimmen, um ins Parlament zu kommen",
      "Anwesenheitskontrolle im Bundesrat für Abstimmungen"
    ],
    "options_en": [
      "Voting rules in the Bundestag for small parties",
      "Attendance check in the Bundestag for votes",
      "Minimum share of votes to get into parliament",
      "Attendance check in the Bundesrat for votes"
    ],
    "answer": "Mindestanteil an Wählerstimmen, um ins Parlament zu kommen",
    "answer_en": "Minimum share of votes to get into parliament",
    "image_paths": []
  },
  {
    "question_number": 124,
    "bundesland": "General",
    "question": "Die Bundestagswahl in Deutschland ist die Wahl …",
    "question_en": "The federal election in Germany is the election of ...",
    "options": [
      "der Bundeskanzlerin/des Bundeskanzlers",
      "der Parlamente der Länder",
      "des Parlaments für Deutschland",
      "der Bundespräsidentin/des Bundespräsidenten"
    ],
    "options_en": [
      "the Chancellor",
      "the parliaments of the states",
      "the parliament for Germany",
      "the Federal President"
    ],
    "answer": "des Parlaments für Deutschland",
    "answer_en": "the parliament for Germany",
    "image_paths": []
  },
  {
    "question_number": 125,
    "bundesland": "General",
    "question": "In einer Demokratie ist eine Funktion von regelmäßigen Wahlen, …",
    "question_en": "In a democracy, a function of regular elections is to ...",
    "options": [
      "die Bürgerinnen und Bürger zu zwingen, ihre Stimme abzugeben",
      "nach dem Willen der Wählermehrheit den Wechsel der Regierung zu ermöglichen",
      "im Land bestehende Gesetze beizubehalten",
      "den Armen mehr Macht zu geben"
    ],
    "options_en": [
      "to force citizens to cast their vote",
      "to enable a change of government according to the will of the majority of voters",
      "to maintain existing laws in the country",
      "to give more power to the poor"
    ],
    "answer": "nach dem Willen der Wählermehrheit den Wechsel der Regierung zu ermöglichen",
    "answer_en": "to enable a change of government according to the will of the majority of voters",
    "image_paths": []
  },
  {
    "question_number": 126,
    "bundesland": "General",
    "question": "Was bekommen wahlberechtigte Bürgerinnen und Bürger in Deutschland vor einer Wahl?",
    "question_en": "What do eligible citizens in Germany receive before an election?",
    "options": [
      "eine Wahlbenachrichtigung von der Gemeinde",
      "eine Wahlerlaubnis von der Bundespräsidentin/von dem Bundespräsidenten",
      "eine Benachrichtigung von der Bundesversammlung",
      "eine Benachrichtigung vom Pfarramt"
    ],
    "options_en": [
      "An election notification from the municipality",
      "An election permit from the Federal President",
      "A notification from the Federal Assembly",
      "A notification from the parish office"
    ],
    "answer": "eine Wahlbenachrichtigung von der Gemeinde",
    "answer_en": "An election notification from the municipality",
    "image_paths": []
  },
  {
    "question_number": 127,
    "bundesland": "General",
    "question": "Warum gibt es die 5%-Hürde im Wahlgesetz der Bundesrepublik Deutschland? Es gibt sie, weil …",
    "question_en": "Why does the 5% hurdle exist in the electoral law of the Federal Republic of Germany? It exists because ...",
    "options": [
      "die Programme von vielen kleinen Parteien viele Gemeinsamkeiten haben",
      "die Bürgerinnen und Bürger bei vielen kleinen Parteien die Orientierung verlieren können",
      "viele kleine Parteien die Regierungsbildung erschweren",
      "die kleinen Parteien nicht so viel Geld haben, um die Politikerinnen und Politiker zu bezahlen"
    ],
    "options_en": [
      "the programs of many small parties have many things in common",
      "citizens can lose their orientation with many small parties",
      "many small parties make it difficult to form a government",
      "the small parties do not have enough money to pay the politicians"
    ],
    "answer": "viele kleine Parteien die Regierungsbildung erschweren",
    "answer_en": "many small parties make it difficult to form a government",
    "image_paths": []
  },
  {
    "question_number": 128,
    "bundesland": "General",
    "question": "Parlamentsmitglieder, die von den Bürgerinnen und Bürgern gewählt werden, nennt man …",
    "question_en": "Members of parliament who are elected by the citizens are called ...",
    "options": [
      "Abgeordnete",
      "Kanzlerinnen/Kanzler",
      "Botschafterinnen/Botschafter",
      "Ministerpräsidentinnen/Ministerpräsidenten"
    ],
    "options_en": [
      "Members of Parliament",
      "Chancellors",
      "Ambassadors",
      "Prime Ministers"
    ],
    "answer": "Abgeordnete",
    "answer_en": "Members of Parliament",
    "image_paths": []
  },
  {
    "question_number": 129,
    "bundesland": "General",
    "question": "Vom Volk gewählt wird in Deutschland …",
    "question_en": "In Germany, the ... is elected by the people.",
    "options": [
      "die Bundeskanzlerin/der Bundeskanzler",
      "die Ministerpräsidentin/der Ministerpräsident eines Bundeslandes",
      "der Bundestag",
      "die Bundespräsidentin/der Bundespräsident"
    ],
    "options_en": [
      "The Chancellor",
      "The Prime Minister of a federal state",
      "The Bundestag",
      "The Federal President"
    ],
    "answer": "der Bundestag",
    "answer_en": "The Bundestag",
    "image_paths": []
  },
  {
    "question_number": 130,
    "bundesland": "General",
    "question": "Welcher Stimmzettel wäre bei einer Bundestagswahl gültig?",
    "question_en": "Which ballot paper would be valid in a federal election?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "1",
    "answer_en": "1",
    "image_paths": [
      "./images/general-130-question.png"
    ]
  },
  {
    "question_number": 131,
    "bundesland": "General",
    "question": "In Deutschland ist eine Bürgermeisterin/ein Bürgermeister …",
    "question_en": "In Germany, a mayor is ...",
    "options": [
      "die Leiterin/der Leiter einer Schule",
      "die Chefin/der Chef einer Bank",
      "das Oberhaupt einer Gemeinde",
      "die/der Vorsitzende einer Partei"
    ],
    "options_en": [
      "The head of a school",
      "The boss of a bank",
      "The head of a municipality",
      "The chairman of a party"
    ],
    "answer": "das Oberhaupt einer Gemeinde",
    "answer_en": "The head of a municipality",
    "image_paths": []
  },
  {
    "question_number": 132,
    "bundesland": "General",
    "question": "Viele Menschen in Deutschland arbeiten in ihrer Freizeit ehrenamtlich. Was bedeutet das?",
    "question_en": "Many people in Germany work on a voluntary basis in their free time. What does that mean?",
    "options": [
      "Sie arbeiten als Soldatinnen/Soldaten",
      "Sie arbeiten freiwillig und unbezahlt in Vereinen und Verbänden",
      "Sie arbeiten in der Bundesregierung",
      "Sie arbeiten in einem Krankenhaus und verdienen dabei Geld"
    ],
    "options_en": [
      "They work as soldiers",
      "They work voluntarily and unpaid in clubs and associations",
      "They work in the federal government",
      "They work in a hospital and earn money"
    ],
    "answer": "Sie arbeiten freiwillig und unbezahlt in Vereinen und Verbänden",
    "answer_en": "They work voluntarily and unpaid in clubs and associations",
    "image_paths": []
  },
  {
    "question_number": 133,
    "bundesland": "General",
    "question": "Was ist bei Bundestags- und Landtagswahlen in Deutschland erlaubt?",
    "question_en": "What is allowed in federal and state elections in Germany?",
    "options": [
      "Der Ehemann wählt für seine Frau mit",
      "Man kann durch Briefwahl seine Stimme abgeben",
      "Man kann am Wahltag telefonisch seine Stimme abgeben",
      "Kinder ab dem Alter von 14 Jahren dürfen wählen"
    ],
    "options_en": [
      "The husband votes for his wife",
      "You can cast your vote by postal vote",
      "You can cast your vote by telephone on election day",
      "Children from the age of 14 are allowed to vote"
    ],
    "answer": "Man kann durch Briefwahl seine Stimme abgeben",
    "answer_en": "You can cast your vote by postal vote",
    "image_paths": []
  },
  {
    "question_number": 134,
    "bundesland": "General",
    "question": "Man will die Buslinie abschaffen, mit der Sie immer zur Arbeit fahren. Was können Sie machen, um die Buslinie zu erhalten?",
    "question_en": "They want to abolish the bus line you always take to work. What can you do to keep the bus line?",
    "options": [
      "Ich beteilige mich an einer Bürgerinitiative für die Erhaltung der Buslinie oder gründe selber eine Initiative",
      "Ich werde Mitglied in einem Sportverein und trainiere Radfahren",
      "Ich wende mich an das Finanzamt, weil ich als Steuerzahlerin/Steuerzahler ein Recht auf die Buslinie habe",
      "Ich schreibe einen Brief an das Forstamt der Gemeinde"
    ],
    "options_en": [
      "I participate in a citizens' initiative to maintain the bus line or start one myself",
      "I become a member of a sports club and practice cycling",
      "I turn to the tax office because as a taxpayer I have a right to the bus line",
      "I am writing a letter to the municipal forestry office"
    ],
    "answer": "Ich beteilige mich an einer Bürgerinitiative für die Erhaltung der Buslinie oder gründe selber eine Initiative",
    "answer_en": "I participate in a citizens' initiative to maintain the bus line or start one myself",
    "image_paths": []
  },
  {
    "question_number": 135,
    "bundesland": "General",
    "question": "Wen vertreten die Gewerkschaften in Deutschland?",
    "question_en": "Who do the trade unions represent in Germany?",
    "options": [
      "große Unternehmen",
      "kleine Unternehmen",
      "Selbstständige",
      "Arbeitnehmerinnen und Arbeitnehmer"
    ],
    "options_en": [
      "large companies",
      "small companies",
      "Self-employed persons",
      "Employees"
    ],
    "answer": "Arbeitnehmerinnen und Arbeitnehmer",
    "answer_en": "Employees",
    "image_paths": []
  },
  {
    "question_number": 136,
    "bundesland": "General",
    "question": "Sie gehen in Deutschland zum Arbeitsgericht bei …",
    "question_en": "In Germany, you go to the labor court in case of ...",
    "options": [
      "falscher Nebenkostenabrechnung",
      "ungerechtfertigter Kündigung durch Ihre Chefin/Ihren Chef",
      "Problemen mit den Nachbarinnen/Nachbarn",
      "Schwierigkeiten nach einem Verkehrsunfall"
    ],
    "options_en": [
      "incorrect utility bill",
      "unjustified dismissal by your boss",
      "problems with the neighbors",
      "difficulties after a traffic accident"
    ],
    "answer": "ungerechtfertigter Kündigung durch Ihre Chefin/Ihren Chef",
    "answer_en": "unjustified dismissal by your boss",
    "image_paths": []
  },
  {
    "question_number": 137,
    "bundesland": "General",
    "question": "Welches Gericht ist in Deutschland bei Konflikten in der Arbeitswelt zuständig?",
    "question_en": "Which court is responsible for conflicts in the world of work in Germany?",
    "options": [
      "das Familiengericht",
      "das Strafgericht",
      "das Arbeitsgericht",
      "das Amtsgericht"
    ],
    "options_en": [
      "The family court",
      "The criminal court",
      "The labor court",
      "The district court"
    ],
    "answer": "das Arbeitsgericht",
    "answer_en": "The labor court",
    "image_paths": []
  },
  {
    "question_number": 138,
    "bundesland": "General",
    "question": "Was kann ich in Deutschland machen, wenn mir meine Arbeitgeberin/mein Arbeitgeber zu Unrecht gekündigt hat?",
    "question_en": "What can I do in Germany if my employer has wrongly terminated my employment?",
    "options": [
      "weiterarbeiten und freundlich zur Chefin/zum Chef sein",
      "ein Mahnverfahren gegen die Arbeitgeberin/den Arbeitgeber führen",
      "Kündigungsschutzklage erheben",
      "die Arbeitgeberin/den Arbeitgeber bei der Polizei anzeigen"
    ],
    "options_en": [
      "continue to work and be friendly to the boss",
      "conduct a dunning procedure against the employer",
      "file an action for protection against dismissal",
      "report the employer to the police"
    ],
    "answer": "Kündigungsschutzklage erheben",
    "answer_en": "file an action for protection against dismissal",
    "image_paths": []
  },
  {
    "question_number": 139,
    "bundesland": "General",
    "question": "Wann kommt es in Deutschland zu einem Prozess vor Gericht? Wenn jemand …",
    "question_en": "When does a trial take place in Germany? When someone ...",
    "options": [
      "zu einer anderen Religion übertritt",
      "eine Straftat begangen hat und angeklagt wird",
      "eine andere Meinung als die der Regierung vertritt",
      "sein Auto falsch geparkt hat und es abgeschleppt wird"
    ],
    "options_en": [
      "converts to another religion",
      "has committed a crime and is charged",
      "represents a different opinion than that of the government",
      "has parked his car incorrectly and it is towed away"
    ],
    "answer": "eine Straftat begangen hat und angeklagt wird",
    "answer_en": "has committed a crime and is charged",
    "image_paths": []
  },
  {
    "question_number": 140,
    "bundesland": "General",
    "question": "Was macht eine Schöffin/ein Schöffe in Deutschland? Sie/Er …",
    "question_en": "What does a lay judge do in Germany? She/He ...",
    "options": [
      "entscheidet mit Richterinnen/Richtern über Schuld und Strafe",
      "gibt Bürgerinnen/Bürgern rechtlichen Rat",
      "stellt Urkunden aus",
      "verteidigt die Angeklagte/den Angeklagten"
    ],
    "options_en": [
      "decides with judges on guilt and punishment",
      "gives legal advice to citizens",
      "issues documents",
      "defends the accused"
    ],
    "answer": "entscheidet mit Richterinnen/Richtern über Schuld und Strafe",
    "answer_en": "decides with judges on guilt and punishment",
    "image_paths": []
  },
  {
    "question_number": 141,
    "bundesland": "General",
    "question": "Wer berät in Deutschland Personen bei Rechtsfragen und vertritt sie vor Gericht?",
    "question_en": "Who advises people on legal issues in Germany and represents them in court?",
    "options": [
      "eine Rechtsanwältin/ein Rechtsanwalt",
      "eine Richterin/ein Richter",
      "eine Schöffin/ein Schöffe",
      "eine Staatsanwältin/ein Staatsanwalt"
    ],
    "options_en": [
      "A lawyer",
      "a judge",
      "a lay judge",
      "A prosecutor"
    ],
    "answer": "eine Rechtsanwältin/ein Rechtsanwalt",
    "answer_en": "A lawyer",
    "image_paths": []
  },
  {
    "question_number": 142,
    "bundesland": "General",
    "question": "Was ist die Hauptaufgabe einer Richterin/eines Richters in Deutschland? Eine Richterin/ein Richter …",
    "question_en": "What is the main task of a judge in Germany? A judge ...",
    "options": [
      "vertritt Bürgerinnen und Bürger vor einem Gericht",
      "arbeitet an einem Gericht und spricht Urteile",
      "ändert Gesetze",
      "betreut Jugendliche vor Gericht"
    ],
    "options_en": [
      "represents citizens before a court",
      "works at a court and passes judgments",
      "changes laws",
      "supervises young people in court"
    ],
    "answer": "arbeitet an einem Gericht und spricht Urteile",
    "answer_en": "works at a court and passes judgments",
    "image_paths": []
  },
  {
    "question_number": 143,
    "bundesland": "General",
    "question": "Eine Richterin/ein Richter in Deutschland gehört zur …",
    "question_en": "A judge in Germany belongs to the ...",
    "options": [
      "Judikative",
      "Exekutive",
      "Operative",
      "Legislative"
    ],
    "options_en": [
      "Judiciary",
      "executive",
      "Operative",
      "Legislative"
    ],
    "answer": "Judikative",
    "answer_en": "Judiciary",
    "image_paths": []
  },
  {
    "question_number": 144,
    "bundesland": "General",
    "question": "Eine Richterin/ein Richter gehört in Deutschland zur …",
    "question_en": "A judge in Germany belongs to the ...",
    "options": [
      "vollziehenden Gewalt",
      "rechtsprechenden Gewalt",
      "planenden Gewalt",
      "gesetzgebenden Gewalt"
    ],
    "options_en": [
      "executive power",
      "judicial power",
      "planning power",
      "legislative power"
    ],
    "answer": "rechtsprechenden Gewalt",
    "answer_en": "judicial power",
    "image_paths": []
  },
  {
    "question_number": 145,
    "bundesland": "General",
    "question": "In Deutschland wird die Staatsgewalt geteilt. Für welche Staatsgewalt arbeitet eine Richterin/ein Richter? Für die …",
    "question_en": "In Germany, state power is divided. For which branch of government does a judge work? For the ...",
    "options": [
      "Judikative",
      "Exekutive",
      "Presse",
      "Legislative"
    ],
    "options_en": [
      "Judiciary",
      "executive",
      "press",
      "Legislative"
    ],
    "answer": "Judikative",
    "answer_en": "Judiciary",
    "image_paths": []
  },
  {
    "question_number": 146,
    "bundesland": "General",
    "question": "Wie nennt man in Deutschland ein Verfahren vor einem Gericht?",
    "question_en": "What is a procedure before a court called in Germany?",
    "options": [
      "Programm",
      "Prozedur",
      "Protokoll",
      "Prozess"
    ],
    "options_en": [
      "program",
      "procedure",
      "protocol",
      "process"
    ],
    "answer": "Prozess",
    "answer_en": "process",
    "image_paths": []
  },
  {
    "question_number": 147,
    "bundesland": "General",
    "question": "Was ist die Arbeit einer Richterin/eines Richters in Deutschland?",
    "question_en": "What is the job of a judge in Germany?",
    "options": [
      "Deutschland regieren",
      "Recht sprechen",
      "Pläne erstellen",
      "Gesetze erlassen"
    ],
    "options_en": [
      "Governing Germany",
      "Administering justice",
      "Creating plans",
      "Enacting laws"
    ],
    "answer": "Recht sprechen",
    "answer_en": "Administering justice",
    "image_paths": []
  },
  {
    "question_number": 148,
    "bundesland": "General",
    "question": "Was ist eine Aufgabe der Polizei in Deutschland?",
    "question_en": "What is a task of the police in Germany?",
    "options": [
      "das Land zu verteidigen",
      "die Bürgerinnen und Bürger abzuhören",
      "die Gesetze zu beschließen",
      "die Einhaltung von Gesetzen zu überwachen"
    ],
    "options_en": [
      "to defend the country",
      "to eavesdrop on citizens",
      "to pass the laws",
      "to monitor compliance with laws"
    ],
    "answer": "die Einhaltung von Gesetzen zu überwachen",
    "answer_en": "to monitor compliance with laws",
    "image_paths": []
  },
  {
    "question_number": 149,
    "bundesland": "General",
    "question": "Was ist ein Beispiel für antisemitisches Verhalten?",
    "question_en": "What is an example of anti-Semitic behavior?",
    "options": [
      "ein jüdisches Fest besuchen",
      "die israelische Regierung kritisieren",
      "den Holocaust leugnen",
      "gegen Juden Fußball spielen"
    ],
    "options_en": [
      "attending a Jewish festival",
      "criticizing the Israeli government",
      "denying the Holocaust",
      "playing football against Jews"
    ],
    "answer": "den Holocaust leugnen",
    "answer_en": "denying the Holocaust",
    "image_paths": []
  },
  {
    "question_number": 150,
    "bundesland": "General",
    "question": "Eine Gerichtsschöffin/ein Gerichtsschöffe in Deutschland ist …",
    "question_en": "A lay judge in Germany is ...",
    "options": [
      "die Stellvertreterin/der Stellvertreter des Stadtoberhaupts",
      "eine ehrenamtliche Richterin/ein ehrenamtlicher Richter",
      "ein Mitglied eines Gemeinderats",
      "eine Person, die Jura studiert hat"
    ],
    "options_en": [
      "the deputy of the head of the city",
      "a lay judge",
      "a member of a municipal council",
      "a person who has studied law"
    ],
    "answer": "eine ehrenamtliche Richterin/ein ehrenamtlicher Richter",
    "answer_en": "a lay judge",
    "image_paths": []
  },
  {
    "question_number": 151,
    "bundesland": "General",
    "question": "Wer baute die Mauer in Berlin?",
    "question_en": "Who built the Wall in Berlin?",
    "options": [
      "Großbritannien",
      "die DDR",
      "die Bundesrepublik Deutschland",
      "die USA"
    ],
    "options_en": [
      "Great Britain",
      "The GDR",
      "The Federal Republic of Germany",
      "the USA"
    ],
    "answer": "die DDR",
    "answer_en": "The GDR",
    "image_paths": []
  },
  {
    "question_number": 152,
    "bundesland": "General",
    "question": "Wann waren die Nationalsozialisten mit Adolf Hitler in Deutschland an der Macht?",
    "question_en": "When were the National Socialists with Adolf Hitler in power in Germany?",
    "options": [
      "1918 bis 1923",
      "1932 bis 1950",
      "1933 bis 1945",
      "1945 bis 1989"
    ],
    "options_en": [
      "1918 to 1923",
      "1932 to 1950",
      "1933 to 1945",
      "1945 to 1989"
    ],
    "answer": "1933 bis 1945",
    "answer_en": "1933 to 1945",
    "image_paths": []
  },
  {
    "question_number": 153,
    "bundesland": "General",
    "question": "Was war am 8. Mai 1945?",
    "question_en": "What happened on May 8, 1945?",
    "options": [
      "Tod Adolf Hitlers",
      "Beginn des Berliner Mauerbaus",
      "Wahl von Konrad Adenauer zum Bundeskanzler",
      "Ende des Zweiten Weltkriegs in Europa"
    ],
    "options_en": [
      "Death of Adolf Hitler",
      "Start of the construction of the Berlin Wall",
      "Election of Konrad Adenauer as Federal Chancellor",
      "End of the Second World War in Europe"
    ],
    "answer": "Ende des Zweiten Weltkriegs in Europa",
    "answer_en": "End of the Second World War in Europe",
    "image_paths": []
  },
  {
    "question_number": 154,
    "bundesland": "General",
    "question": "Wann war der Zweite Weltkrieg zu Ende?",
    "question_en": "When did the Second World War end?",
    "options": [
      "1933",
      "1945",
      "1949",
      "1961"
    ],
    "options_en": [
      "1933",
      "1945",
      "1949",
      "1961"
    ],
    "answer": "1945",
    "answer_en": "1945",
    "image_paths": []
  },
  {
    "question_number": 155,
    "bundesland": "General",
    "question": "Wann waren die Nationalsozialisten in Deutschland an der Macht?",
    "question_en": "When were the National Socialists in Germany in Germany?",
    "options": [
      "1888 bis 1918",
      "1921 bis 1934",
      "1933 bis 1945",
      "1949 bis 1963"
    ],
    "options_en": [
      "1888 to 1918",
      "1921 to 1934",
      "1933 to 1945",
      "1949 to 1963"
    ],
    "answer": "1933 bis 1945",
    "answer_en": "1933 to 1945",
    "image_paths": []
  },
  {
    "question_number": 156,
    "bundesland": "General",
    "question": "In welchem Jahr wurde Hitler Reichskanzler?",
    "question_en": "In what year did Hitler become the Reich Chancellor?",
    "options": [
      "1923",
      "1927",
      "1933",
      "1936"
    ],
    "options_en": [
      "1923",
      "1927",
      "1933",
      "1936"
    ],
    "answer": "1933",
    "answer_en": "1933",
    "image_paths": []
  },
  {
    "question_number": 157,
    "bundesland": "General",
    "question": "Die Nationalsozialisten mit Adolf Hitler errichteten 1933 in Deutschland …",
    "question_en": "The National Socialists with Adolf Hitler built in Germany in 1933 ...",
    "options": [
      "eine Diktatur",
      "einen demokratischen Staat",
      "eine Monarchie",
      "ein Fürstentum"
    ],
    "options_en": [
      "a dictatorship",
      "a democratic state",
      "A monarchy",
      "Principality"
    ],
    "answer": "eine Diktatur",
    "answer_en": "a dictatorship",
    "image_paths": []
  },
  {
    "question_number": 158,
    "bundesland": "General",
    "question": "Das \"Dritte Reich\" war eine …",
    "question_en": "The \"Third Reich\" was a ...",
    "options": [
      "Diktatur",
      "Demokratie",
      "Monarchie",
      "Räterepublik"
    ],
    "options_en": [
      "dictatorship",
      "democracy",
      "monarchy",
      "Räter Republic"
    ],
    "answer": "Diktatur",
    "answer_en": "dictatorship",
    "image_paths": []
  },
  {
    "question_number": 159,
    "bundesland": "General",
    "question": "Was gab es in Deutschland nicht während der Zeit des Nationalsozialismus?",
    "question_en": "What did not exist in Germany during the National Socialism period?",
    "options": [
      "freie Wahlen",
      "Pressezensur",
      "willkürliche Verhaftungen",
      "Verfolgung von Juden"
    ],
    "options_en": [
      "free elections",
      "Press zeal",
      "arbitrary arrests",
      "Persecution of Jews"
    ],
    "answer": "freie Wahlen",
    "answer_en": "free elections",
    "image_paths": []
  },
  {
    "question_number": 160,
    "bundesland": "General",
    "question": "Welcher Krieg dauerte von 1939 bis 1945?",
    "question_en": "Which war lasted from 1939 to 1945?",
    "options": [
      "der Erste Weltkrieg",
      "der Zweite Weltkrieg",
      "der Vietnamkrieg",
      "der Golfkrieg"
    ],
    "options_en": [
      "The First World War",
      "The Second World War",
      "The Vietnam War",
      "The Gulf War"
    ],
    "answer": "der Zweite Weltkrieg",
    "answer_en": "The Second World War",
    "image_paths": []
  },
  {
    "question_number": 161,
    "bundesland": "General",
    "question": "Was kennzeichnete den NS-Staat? Eine Politik …",
    "question_en": "What characterized the Nazi state?",
    "options": [
      "des staatlichen Rassismus",
      "der Meinungsfreiheit",
      "der allgemeinen Religionsfreiheit",
      "der Entwicklung der Demokratie"
    ],
    "options_en": [
      "of state racism",
      "freedom of expression",
      "general religious freedom",
      "the development of democracy"
    ],
    "answer": "des staatlichen Rassismus",
    "answer_en": "of state racism",
    "image_paths": []
  },
  {
    "question_number": 162,
    "bundesland": "General",
    "question": "Claus Schenk Graf von Stauffenberg wurde bekannt durch …",
    "question_en": "Claus Schenk Graf von Stauffenberg was known for ...",
    "options": [
      "eine Goldmedaille bei den Olympischen Spielen 1936",
      "den Bau des Reichstagsgebäudes",
      "den Aufbau der Wehrmacht",
      "das Attentat auf Hitler am 20. Juli 1944"
    ],
    "options_en": [
      "A gold medal at the 1936 Olympics",
      "the construction of the Reichstag building",
      "the construction of the Wehrmacht",
      "The attack on Hitler on July 20, 1944"
    ],
    "answer": "das Attentat auf Hitler am 20. Juli 1944",
    "answer_en": "The attack on Hitler on July 20, 1944",
    "image_paths": []
  },
  {
    "question_number": 163,
    "bundesland": "General",
    "question": "In welchem Jahr zerstörten die Nationalsozialisten Synagogen und jüdische Geschäfte in Deutschland?",
    "question_en": "In which year did the National Socialists destroy synagogues and Jewish business in Germany?",
    "options": [
      "1925",
      "1930",
      "1938",
      "1945"
    ],
    "options_en": [
      "1925",
      "1930",
      "1938",
      "1945"
    ],
    "answer": "1938",
    "answer_en": "1938",
    "image_paths": []
  },
  {
    "question_number": 164,
    "bundesland": "General",
    "question": "Was passierte am 9. November 1938 in Deutschland?",
    "question_en": "What happened in Germany on November 9, 1938?",
    "options": [
      "Mit dem Angriff auf Polen beginnt der Zweite Weltkrieg",
      "Die Nationalsozialisten verlieren eine Wahl und lösen den Reichstag auf",
      "Jüdische Geschäfte und Synagogen werden durch Nationalsozialisten und ihre Anhänger zerstört",
      "Hitler wird Reichspräsident und lässt alle Parteien verbieten"
    ],
    "options_en": [
      "The Second World War begins with the attack on Poland",
      "The National Socialists lose a choice and dissolve the Reichstag",
      "Jewish shops and synagogues are destroyed by National Socialists and their followers",
      "Hitler becomes an Reich President and lets all parties prohibit"
    ],
    "answer": "Jüdische Geschäfte und Synagogen werden durch Nationalsozialisten und ihre Anhänger zerstört",
    "answer_en": "Jewish shops and synagogues are destroyed by National Socialists and their followers",
    "image_paths": []
  },
  {
    "question_number": 165,
    "bundesland": "General",
    "question": "Wie hieß der erste Bundeskanzler der Bundesrepublik Deutschland?",
    "question_en": "What was the name of the first Chancellor of the Federal Republic of Germany?",
    "options": [
      "Konrad Adenauer",
      "Kurt Georg Kiesinger",
      "Helmut Schmidt",
      "Willy Brandt"
    ],
    "options_en": [
      "Konrad Adenauer",
      "Kurt Georg Kiesinger",
      "Helmut Schmidt",
      "Willy Brandt"
    ],
    "answer": "Konrad Adenauer",
    "answer_en": "Konrad Adenauer",
    "image_paths": []
  },
  {
    "question_number": 166,
    "bundesland": "General",
    "question": "Bei welchen Demonstrationen in Deutschland riefen die Menschen \"Wir sind das Volk\"?",
    "question_en": "In which demonstrations in Germany did people call \"we are the people\"?",
    "options": [
      "beim Arbeiteraufstand 1953 in der DDR",
      "bei den Demonstrationen 1968 in der Bundesrepublik Deutschland",
      "bei den Anti-Atomkraft-Demonstrationen 1985 in der Bundesrepublik Deutschland",
      "bei den Montagsdemonstrationen 1989 in der DDR"
    ],
    "options_en": [
      "at the workers' uprising in 1953 in the GDR",
      "in the demonstrations in 1968 in the Federal Republic of Germany",
      "In the 1985 anti-atomic strength demonstrations in the Federal Republic of Germany",
      "In the 1989 Monday demonstrations in the GDR"
    ],
    "answer": "bei den Montagsdemonstrationen 1989 in der DDR",
    "answer_en": "In the 1989 Monday demonstrations in the GDR",
    "image_paths": []
  },
  {
    "question_number": 167,
    "bundesland": "General",
    "question": "Welche Länder wurden nach dem Zweiten Weltkrieg in Deutschland als \"Alliierte Besatzungsmächte\" bezeichnet?",
    "question_en": "Which countries were called \"Allied occupying powers\" in Germany after the Second World War?",
    "options": [
      "Sowjetunion, Großbritannien, Polen, Schweden",
      "Frankreich, Sowjetunion, Italien, Japan",
      "USA, Sowjetunion, Spanien, Portugal",
      "USA, Sowjetunion, Großbritannien, Frankreich"
    ],
    "options_en": [
      "Soviet Union, Great Britain, Poland, Sweden",
      "France, Soviet Union, Italy, Japan",
      "USA, Soviet Union, Spain, Portugal",
      "USA, Soviet Union, Great Britain, France"
    ],
    "answer": "USA, Sowjetunion, Großbritannien, Frankreich",
    "answer_en": "USA, Soviet Union, Great Britain, France",
    "image_paths": []
  },
  {
    "question_number": 168,
    "bundesland": "General",
    "question": "Welches Land war keine \"Alliierte Besatzungsmacht\" in Deutschland?",
    "question_en": "Which country was not a \"Allied occupying power\" in Germany?",
    "options": [
      "USA",
      "Sowjetunion",
      "Frankreich",
      "Japan"
    ],
    "options_en": [
      "USA",
      "Soviet Union",
      "France",
      "Japan"
    ],
    "answer": "Japan",
    "answer_en": "Japan",
    "image_paths": []
  },
  {
    "question_number": 169,
    "bundesland": "General",
    "question": "Wann wurde die Bundesrepublik Deutschland gegründet?",
    "question_en": "When was the Federal Republic of Germany founded?",
    "options": [
      "1939",
      "1945",
      "1949",
      "1951"
    ],
    "options_en": [
      "1939",
      "1945",
      "1949",
      "1951"
    ],
    "answer": "1949",
    "answer_en": "1949",
    "image_paths": []
  },
  {
    "question_number": 170,
    "bundesland": "General",
    "question": "Was gab es während der Zeit des Nationalsozialismus in Deutschland?",
    "question_en": "What was there during National Socialism in Germany?",
    "options": [
      "das Verbot von Parteien",
      "das Recht zur freien Entfaltung der Persönlichkeit",
      "Pressefreiheit",
      "den Schutz der Menschenwürde"
    ],
    "options_en": [
      "The ban on parties",
      "The right to freely unfold the personality",
      "Freedom of the press",
      "the protection of human dignity"
    ],
    "answer": "das Verbot von Parteien",
    "answer_en": "The ban on parties",
    "image_paths": []
  },
  {
    "question_number": 171,
    "bundesland": "General",
    "question": "Soziale Marktwirtschaft bedeutet, die Wirtschaft …",
    "question_en": "Social market economy means the economy ...",
    "options": [
      "steuert sich allein nach Angebot und Nachfrage",
      "wird vom Staat geplant und gesteuert, Angebot und Nachfrage werden nicht berücksichtigt",
      "richtet sich nach der Nachfrage im Ausland",
      "richtet sich nach Angebot und Nachfrage, aber der Staat sorgt für einen sozialen Ausgleich"
    ],
    "options_en": [
      "controls itself solely according to supply and demand",
      "is planned and controlled by the state, supply and demand are not taken into account",
      "is based on demand abroad",
      "is based on supply and demand, but the state ensures a social balance"
    ],
    "answer": "richtet sich nach Angebot und Nachfrage, aber der Staat sorgt für einen sozialen Ausgleich",
    "answer_en": "is based on supply and demand, but the state ensures a social balance",
    "image_paths": []
  },
  {
    "question_number": 172,
    "bundesland": "General",
    "question": "In welcher Besatzungszone wurde die DDR gegründet? In der …",
    "question_en": "In which occupation zone was the GDR founded?",
    "options": [
      "amerikanischen Besatzungszone",
      "französischen Besatzungszone",
      "britischen Besatzungszone",
      "sowjetischen Besatzungszone"
    ],
    "options_en": [
      "American occupation zone",
      "French occupation zone",
      "British occupation zone",
      "Soviet occupation zone"
    ],
    "answer": "sowjetischen Besatzungszone",
    "answer_en": "Soviet occupation zone",
    "image_paths": []
  },
  {
    "question_number": 173,
    "bundesland": "General",
    "question": "Die Bundesrepublik Deutschland ist ein Gründungsmitglied …",
    "question_en": "The Federal Republic of Germany is a founding member ...",
    "options": [
      "des Nordatlantikpakts (NATO)",
      "der Vereinten Nationen (VN)",
      "der Europäischen Union (EU)",
      "des Warschauer Pakts"
    ],
    "options_en": [
      "of the North Atlantic Pact (NATO)",
      "the United Nations (VN)",
      "the European Union (EU)",
      "of the Warsaw Pact"
    ],
    "answer": "der Europäischen Union (EU)",
    "answer_en": "the European Union (EU)",
    "image_paths": []
  },
  {
    "question_number": 174,
    "bundesland": "General",
    "question": "Wann wurde die DDR gegründet?",
    "question_en": "When was the GDR founded?",
    "options": [
      "1947",
      "1949",
      "1953",
      "1956"
    ],
    "options_en": [
      "1947",
      "1949",
      "1953",
      "1956"
    ],
    "answer": "1949",
    "answer_en": "1949",
    "image_paths": []
  },
  {
    "question_number": 175,
    "bundesland": "General",
    "question": "Wie viele Besatzungszonen gab es in Deutschland nach dem Zweiten Weltkrieg?",
    "question_en": "How many occupation zones were there in Germany after the Second World War?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "4",
    "answer_en": "4",
    "image_paths": []
  },
  {
    "question_number": 176,
    "bundesland": "General",
    "question": "Wie waren die Besatzungszonen Deutschlands nach 1945 verteilt?",
    "question_en": "How were the occupation zones of Germany distributed after 1945?",
    "options": [
      "1=Großbritannien, 2=Sowjetunion, 3=Frankreich, 4=USA",
      "1=Sowjetunion, 2=Großbritannien, 3=USA, 4=Frankreich",
      "1=Großbritannien, 2=Sowjetunion, 3=USA, 4=Frankreich",
      "1=Großbritannien, 2=USA, 3=Sowjetunion, 4=Frankreich"
    ],
    "options_en": [
      "1 = Great Britain, 2 = Soviet Union, 3 = France, 4 = USA",
      "1 = Soviet Union, 2 = Great Britain, 3 = USA, 4 = France",
      "1 = Great Britain, 2 = Soviet Union, 3 = USA, 4 = France",
      "1 = Great Britain, 2 = USA, 3 = Soviet Union, 4 = France"
    ],
    "answer": "1=Großbritannien, 2=Sowjetunion, 3=USA, 4=Frankreich",
    "answer_en": "1 = Great Britain, 2 = Soviet Union, 3 = USA, 4 = France",
    "image_paths": [
      "./images/general-176-question.png"
    ]
  },
  {
    "question_number": 177,
    "bundesland": "General",
    "question": "Welche deutsche Stadt wurde nach dem Zweiten Weltkrieg in vier Sektoren aufgeteilt?",
    "question_en": "Which German city was divided into four sectors after the Second World War?",
    "options": [
      "München",
      "Berlin",
      "Dresden",
      "Frankfurt/Oder"
    ],
    "options_en": [
      "Munich",
      "Berlin",
      "Dresden",
      "Frankfurt/Oder"
    ],
    "answer": "Berlin",
    "answer_en": "Berlin",
    "image_paths": []
  },
  {
    "question_number": 178,
    "bundesland": "General",
    "question": "Vom Juni 1948 bis zum Mai 1949 wurden die Bürgerinnen und Bürger von West-Berlin durch eine Luftbrücke versorgt. Welcher Umstand war dafür verantwortlich?Für Frankreich war eine Versorgung der West-Berliner Bevölkerung mit dem Flugzeug kostengünstiger.Die amerikanischen Soldatinnen und Soldaten hatten beim Landtransport Angst vor Überfällen",
    "question_en": "From June 1948 to May 1949, the citizens of West Berlin were cared for by an air bridge.",
    "options": [
      "Für Frankreich war eine Versorgung der West-Berliner Bevölkerung mit dem Flugzeug kostengünstiger",
      "Die amerikanischen Soldatinnen und Soldaten hatten beim Landtransport Angst vor Überfällen",
      "Für Großbritannien war die Versorgung über die Luftbrücke schneller",
      "Die Sowjetunion unterbrach den gesamten Verkehr auf dem Landwege"
    ],
    "options_en": [
      "For France, a supply of the West Berlin population with the aircraft was cheaper",
      "The American soldiers were afraid of raids while transporting land transport",
      "For the UK, the supply over the air bridge was faster",
      "The Soviet Union interrupted all traffic on the land path"
    ],
    "answer": "Die Sowjetunion unterbrach den gesamten Verkehr auf dem Landwege",
    "answer_en": "The Soviet Union interrupted all traffic on the land path",
    "image_paths": []
  },
  {
    "question_number": 179,
    "bundesland": "General",
    "question": "Wie endete der Zweite Weltkrieg in Europa offiziell?",
    "question_en": "How did the Second World War officially end in Europe?",
    "options": [
      "mit dem Tod Adolf Hitlers",
      "durch die bedingungslose Kapitulation Deutschlands",
      "mit dem Rückzug der Deutschen aus den besetzten Gebieten",
      "durch eine Revolution in Deutschland"
    ],
    "options_en": [
      "With the death of Adolf Hitler",
      "Due to the unconditional surrender of Germany",
      "With the withdrawal of the Germans from the occupied areas",
      "Through a revolution in Germany"
    ],
    "answer": "durch die bedingungslose Kapitulation Deutschlands",
    "answer_en": "Due to the unconditional surrender of Germany",
    "image_paths": []
  },
  {
    "question_number": 180,
    "bundesland": "General",
    "question": "Der erste Bundeskanzler der Bundesrepublik Deutschland war …",
    "question_en": "The first Chancellor of the Federal Republic of Germany was ...",
    "options": [
      "Ludwig Erhard",
      "Willy Brandt",
      "Konrad Adenauer",
      "Gerhard Schröder"
    ],
    "options_en": [
      "Ludwig Erhard",
      "Willy Brandt",
      "Konrad Adenauer",
      "Gerhard Schröder"
    ],
    "answer": "Konrad Adenauer",
    "answer_en": "Konrad Adenauer",
    "image_paths": []
  },
  {
    "question_number": 181,
    "bundesland": "General",
    "question": "Was wollte Willy Brandt mit seinem Kniefall 1970 im ehemaligen jüdischen Ghetto in Warschau ausdrücken? © Bundesregierung/Engelbert Reineke",
    "question_en": "What did Willy Brandt want to express with his knee in 1970 in the former Jewish ghetto in Warsaw?",
    "options": [
      "Er hat sich den ehemaligen Alliierten unterworfen",
      "Er bat Polen und die polnischen Juden um Vergebung",
      "Er zeigte seine Demut vor dem Warschauer Pakt",
      "Er sprach ein Gebet am Grab des Unbekannten Soldaten"
    ],
    "options_en": [
      "He has submitted to the former Allies",
      "He asked Poland and the Polish Jews for forgiveness",
      "He showed his humility in front of the Warsaw Pact",
      "He spoke a prayer at the grave of the unknown soldier"
    ],
    "answer": "Er bat Polen und die polnischen Juden um Vergebung",
    "answer_en": "He asked Poland and the Polish Jews for forgiveness",
    "image_paths": [
      "./images/general-181-question.png"
    ]
  },
  {
    "question_number": 182,
    "bundesland": "General",
    "question": "Wie heißt das jüdische Gebetshaus?",
    "question_en": "What is the name of the Jewish prayer house?",
    "options": ["Basilika" , "Moschee", "Synagoge", "Kirche"],
    "options_en": ["Basilica", "Mosque", "Synagogue", "Church"],
    "answer": "Synagoge",
    "answer_en": "Synagogue",
    "image_paths": []
  },
  {
    "question_number": 183,
    "bundesland": "General",
    "question": "Wann war in der Bundesrepublik Deutschland das \"Wirtschaftswunder\"?",
    "question_en": "When was the \"economic miracle\" in the Federal Republic of Germany?",
    "options": [
      "40er Jahre",
      "50er Jahre",
      "70er Jahre",
      "80er Jahre"
    ],
    "options_en": [
      "40s",
      "50s",
      "70s",
      "80s"
    ],
    "answer": "50er Jahre",
    "answer_en": "50s",
    "image_paths": []
  },
  {
    "question_number": 184,
    "bundesland": "General",
    "question": "Auf welcher rechtlichen Grundlage wurde der Staat Israel gegründet?",
    "question_en": "On what legal basis was the state of Israel founded?",
    "options": [
      "eine Resolution der Vereinten Nationen",
      "ein Beschluss des Zionistenkongresses",
      "ein Vorschlag der Bundesregierung",
      "ein Vorschlag der der UdSSR"
    ],
    "options_en": [
      "a Resolution of the United Nations",
      "a decision of the Zionist Congress",
      "a proposal by the federal government",
      "a proposal by the USSR"
    ],
    "answer": "eine Resolution der Vereinten Nationen",
    "answer_en": "a Resolution of the United Nations",
    "image_paths": []
  },
  {
    "question_number": 185,
    "bundesland": "General",
    "question": "Wofür stand der Ausdruck \"Eiserner Vorhang\"? Für die Abschottung …",
    "question_en": "What was the expression \"iron curtain\" for?",
    "options": [
      "des Warschauer Pakts gegen den Westen",
      "Norddeutschlands gegen Süddeutschland",
      "Nazi-Deutschlands gegen die Alliierten",
      "Europas gegen die USA"
    ],
    "options_en": [
      "of the Warsaw Pact against the West",
      "Northern Germany against southern Germany",
      "Nazi Germany against the Allies",
      "Europe against the USA"
    ],
    "answer": "des Warschauer Pakts gegen den Westen",
    "answer_en": "of the Warsaw Pact against the West",
    "image_paths": []
  },
  {
    "question_number": 186,
    "bundesland": "General",
    "question": "Im Jahr 1953 gab es in der DDR einen Aufstand, an den lange Zeit in der Bundesrepublik Deutschland ein Feiertag erinnerte. Wann war das?",
    "question_en": "In 1953 there was an uprising in the GDR that a holiday reminded in the Federal Republic of Germany for a long time.",
    "options": [
      "1. Mai",
      "17. Juni",
      "20. Juli",
      "9. November"
    ],
    "options_en": [
      "May 1st",
      "June 17th",
      "July 20th",
      "November 9th"
    ],
    "answer": "17. Juni",
    "answer_en": "June 17th",
    "image_paths": []
  },
  {
    "question_number": 187,
    "bundesland": "General",
    "question": "Welcher deutsche Staat hatte eine schwarz-rot-goldene Flagge mit Hammer, Zirkel und Ährenkranz?",
    "question_en": "Which German state had a black, red and gold flag with a hammer, circle and ear wreath?",
    "options": [
      "Preußen",
      "Bundesrepublik Deutschland",
      "\"Drittes Reich\"",
      "DDR"
    ],
    "options_en": [
      "Prussia",
      "Federal Republic of Germany",
      "\"Third Reich\"",
      "GDR"
    ],
    "answer": "DDR",
    "answer_en": "GDR",
    "image_paths": [
      "./images/general-187-question.png"
    ]
  },
  {
    "question_number": 188,
    "bundesland": "General",
    "question": "In welchem Jahr wurde die Mauer in Berlin gebaut?",
    "question_en": "In which year was the wall built in Berlin?",
    "options": [
      "1953",
      "1956",
      "1959",
      "1961"
    ],
    "options_en": [
      "1953",
      "1956",
      "1959",
      "1961"
    ],
    "answer": "1961",
    "answer_en": "1961",
    "image_paths": []
  },
  {
    "question_number": 189,
    "bundesland": "General",
    "question": "Wann baute die DDR die Mauer in Berlin?",
    "question_en": "When did the GDR build the wall in Berlin?",
    "options": [
      "1919",
      "1933",
      "1961",
      "1990"
    ],
    "options_en": [
      "1919",
      "1933",
      "1961",
      "1990"
    ],
    "answer": "1961",
    "answer_en": "1961",
    "image_paths": []
  },
  {
    "question_number": 190,
    "bundesland": "General",
    "question": "Was bedeutet die Abkürzung DDR?",
    "question_en": "What does the abbreviation DDR mean?",
    "options": [
      "Dritter Deutscher Rundfunk",
      "Die Deutsche Republik",
      "Dritte Deutsche Republik",
      "Deutsche Demokratische Republik"
    ],
    "options_en": [
      "Third German radio",
      "The German Republic",
      "Third German Republic",
      "German Democratic Republic"
    ],
    "answer": "Deutsche Demokratische Republik",
    "answer_en": "German Democratic Republic",
    "image_paths": []
  },
  {
    "question_number": 191,
    "bundesland": "General",
    "question": "Wann wurde die Mauer in Berlin für alle geöffnet?",
    "question_en": "When was the wall opened for everyone in Berlin?",
    "options": [
      "1987",
      "1989",
      "1992",
      "1995"
    ],
    "options_en": [
      "1987",
      "1989",
      "1992",
      "1995"
    ],
    "answer": "1989",
    "answer_en": "1989",
    "image_paths": []
  },
  {
    "question_number": 192,
    "bundesland": "General",
    "question": "Welches heutige deutsche Bundesland gehörte früher zum Gebiet der DDR?",
    "question_en": "Which today's German state belonged to the area of the GDR?",
    "options": [
      "Brandenburg",
      "Bayern",
      "Saarland",
      "Hessen"
    ],
    "options_en": [
      "Brandenburg",
      "Bavaria",
      "Saarland",
      "Hesse"
    ],
    "answer": "Brandenburg",
    "answer_en": "Brandenburg",
    "image_paths": []
  },
  {
    "question_number": 193,
    "bundesland": "General",
    "question": "Von 1961 bis 1989 war Berlin …",
    "question_en": "From 1961 to 1989 Berlin was ...",
    "options": [
      "ohne Bürgermeister",
      "ein eigener Staat",
      "durch eine Mauer geteilt",
      "nur mit dem Flugzeug erreichbar"
    ],
    "options_en": [
      "without mayor",
      "a separate state",
      "divided by a wall",
      "only accessible by plane"
    ],
    "answer": "durch eine Mauer geteilt",
    "answer_en": "divided by a wall",
    "image_paths": []
  },
  {
    "question_number": 194,
    "bundesland": "General",
    "question": "Am 3. Oktober feiert man in Deutschland den Tag der Deutschen …",
    "question_en": "On October 3rd, the Day of German ... is celebrated in Germany",
    "options": [
      "Einheit",
      "Nation",
      "Bundesländer",
      "Städte"
    ],
    "options_en": [
      "Unit",
      "nation",
      "Federal states",
      "Cities"
    ],
    "answer": "Einheit",
    "answer_en": "Unit",
    "image_paths": []
  },
  {
    "question_number": 195,
    "bundesland": "General",
    "question": "Welches heutige deutsche Bundesland gehörte früher zum Gebiet der DDR?",
    "question_en": "Which today's German state belonged to the area of the GDR?",
    "options": [
      "Hessen",
      "Sachsen-Anhalt",
      "Nordrhein-Westfalen",
      "Saarland"
    ],
    "options_en": [
      "Hesse",
      "Saxony-Anhalt",
      "North Rhine-Westphalia",
      "Saarland"
    ],
    "answer": "Sachsen-Anhalt",
    "answer_en": "Saxony-Anhalt",
    "image_paths": []
  },
  {
    "question_number": 196,
    "bundesland": "General",
    "question": "Warum nennt man die Zeit im Herbst 1989 in der DDR \"Die Wende\"? In dieser Zeit veränderte sich die DDR politisch …",
    "question_en": "Why is the time in autumn 1989 in the GDR called \"Die Wende\"?",
    "options": [
      "von einer Diktatur zur Demokratie",
      "von einer liberalen Marktwirtschaft zum Sozialismus",
      "von einer Monarchie zur Sozialdemokratie",
      "von einem religiösen Staat zu einem kommunistischen Staat"
    ],
    "options_en": [
      "From a dictatorship to democracy",
      "From a liberal market economy to socialism",
      "From a monarchy to social democracy",
      "From a religious state to a communist state"
    ],
    "answer": "von einer Diktatur zur Demokratie",
    "answer_en": "From a dictatorship to democracy",
    "image_paths": []
  },
  {
    "question_number": 197,
    "bundesland": "General",
    "question": "Welches heutige deutsche Bundesland gehörte früher zum Gebiet der DDR?",
    "question_en": "Which today's German state belonged to the area of the GDR?",
    "options": [
      "Thüringen",
      "Hessen",
      "Bayern",
      "Bremen"
    ],
    "options_en": [
      "Thuringia",
      "Hesse",
      "Bavaria",
      "Bremen"
    ],
    "answer": "Thüringen",
    "answer_en": "Thuringia",
    "image_paths": []
  },
  {
    "question_number": 198,
    "bundesland": "General",
    "question": "Welches heutige deutsche Bundesland gehörte früher zum Gebiet der DDR?",
    "question_en": "Which today's German state belonged to the area of the GDR?",
    "options": [
      "Bayern",
      "Niedersachsen",
      "Sachsen",
      "Baden-Württemberg"
    ],
    "options_en": [
      "Bavaria",
      "Lower Saxony",
      "Saxony",
      "Baden-Württemberg"
    ],
    "answer": "Sachsen",
    "answer_en": "Saxony",
    "image_paths": []
  },
  {
    "question_number": 199,
    "bundesland": "General",
    "question": "Mit der Abkürzung \"Stasi\" meinte man in der DDR …",
    "question_en": "The abbreviation \"Stasi\" said in the GDR ...",
    "options": [
      "das Parlament",
      "das Ministerium für Staatssicherheit",
      "eine regierende Partei",
      "das Ministerium für Volksbildung"
    ],
    "options_en": [
      "The parliament",
      "The Ministry of State Security",
      "A governing party",
      "The Ministry of Popular Education"
    ],
    "answer": "das Ministerium für Staatssicherheit",
    "answer_en": "The Ministry of State Security",
    "image_paths": []
  },
  {
    "question_number": 200,
    "bundesland": "General",
    "question": "Welches heutige deutsche Bundesland gehörte früher zum Gebiet der DDR?",
    "question_en": "Which today's German state belonged to the area of the GDR?",
    "options": [
      "Hessen",
      "Schleswig-Holstein",
      "Mecklenburg-Vorpommern",
      "Saarland"
    ],
    "options_en": [
      "Hesse",
      "Schleswig-Holstein",
      "Mecklenburg-Western Pomerania",
      "Saarland"
    ],
    "answer": "Mecklenburg-Vorpommern",
    "answer_en": "Mecklenburg-Western Pomerania",
    "image_paths": []
  },
  {
    "question_number": 201,
    "bundesland": "General",
    "question": "Welche der folgenden Auflistungen enthält nur Bundesländer, die zum Gebiet der früheren DDR gehörten?Niedersachsen, Nordrhein-Westfalen, Hessen, Schleswig-Holstein, BrandenburgMecklenburg-Vorpommern, Brandenburg, Sachsen, Sachsen-Anhalt, Thüringen",
    "question_en": "Which of the following lists contains only federal states that belong to the area of the former GDR? Lower Saxony, North Rhine-Westphalia, Hesse, Schleswig-Holstein, Brandenburgmecklenburg-Western Pomerania, Brandenburg, Saxony, Saxony-Anhalt, Thuringia",
    "options": [
      "Niedersachsen, Nordrhein-Westfalen, Hessen, Schleswig-Holstein, Brandenburg",
      "Mecklenburg-Vorpommern, Brandenburg, Sachsen, Sachsen-Anhalt, Thüringen",
      "Bayern, Baden-Württemberg, Rheinland-Pfalz, Thüringen, Sachsen",
      "Sachsen, Thüringen, Hessen, Niedersachen, Brandenburg"
    ],
    "options_en": [
      "Lower Saxony, North Rhine-Westphalia, Hesse, Schleswig-Holstein, Brandenburg",
      "Mecklenburg-Western Pomerania, Brandenburg, Saxony, Saxony-Anhalt, Thuringia",
      "Bavaria, Baden-Württemberg, Rhineland-Palatinate, Thuringia, Saxony",
      "Saxony, Thuringia, Hesse, Lower Saxony, Brandenburg"
    ],
    "answer": "Mecklenburg-Vorpommern, Brandenburg, Sachsen, Sachsen-Anhalt, Thüringen",
    "answer_en": "Mecklenburg-Western Pomerania, Brandenburg, Saxony, Saxony-Anhalt, Thuringia",
    "image_paths": []
  },
  {
    "question_number": 202,
    "bundesland": "General",
    "question": "Zu wem gehörte die DDR im \"Kalten Krieg\"?",
    "question_en": "Who did the GDR belong to in the \"Cold War\"?",
    "options": [
      "zu den Westmächten",
      "zum Warschauer Pakt",
      "zur NATO",
      "zu den blockfreien Staaten"
    ],
    "options_en": [
      "To the western powers",
      "To the Warsaw Pact",
      "to NATO",
      "To the block -free states"
    ],
    "answer": "zum Warschauer Pakt",
    "answer_en": "To the Warsaw Pact",
    "image_paths": []
  },
  {
    "question_number": 203,
    "bundesland": "General",
    "question": "Wie hieß das Wirtschaftssystem der DDR?",
    "question_en": "What was the name of the GDR's economic system?",
    "options": [
      "Marktwirtschaft",
      "Planwirtschaft",
      "Angebot und Nachfrage",
      "Kapitalismus"
    ],
    "options_en": [
      "Market economy",
      "Planned economy",
      "supply and demand",
      "capitalism"
    ],
    "answer": "Planwirtschaft",
    "answer_en": "Planned economy",
    "image_paths": []
  },
  {
    "question_number": 204,
    "bundesland": "General",
    "question": "Wie wurden die Bundesrepublik Deutschland und die DDR zu einem Staat?",
    "question_en": "How did the Federal Republic of Germany and the GDR become a state?",
    "options": [
      "Die Bundesrepublik hat die DDR besetzt",
      "Die heutigen fünf östlichen Bundesländer sind der Bundesrepublik Deutschland beigetreten",
      "Die westlichen Bundesländer sind der DDR beigetreten",
      "Die DDR hat die Bundesrepublik Deutschland besetzt"
    ],
    "options_en": [
      "The Federal Republic has occupied the GDR",
      "Today's five eastern federal states have joined the Federal Republic of Germany",
      "The western federal states have joined the GDR",
      "The GDR has occupied the Federal Republic of Germany"
    ],
    "answer": "Die heutigen fünf östlichen Bundesländer sind der Bundesrepublik Deutschland beigetreten",
    "answer_en": "Today's five eastern federal states have joined the Federal Republic of Germany",
    "image_paths": []
  },
  {
    "question_number": 205,
    "bundesland": "General",
    "question": "Mit dem Beitritt der DDR zur Bundesrepublik Deutschland gehören die neuen Bundesländer nun auch …",
    "question_en": "When the GDR joins the Federal Republic of Germany, the new federal states are now also ...",
    "options": [
      "zur Europäischen Union",
      "zum Warschauer Pakt",
      "zur OPEC",
      "zur Europäischen Verteidigungsgemeinschaft"
    ],
    "options_en": [
      "To the European Union",
      "To the Warsaw Pact",
      "To the Opec",
      "to the European defense community"
    ],
    "answer": "zur Europäischen Union",
    "answer_en": "To the European Union",
    "image_paths": []
  },
  {
    "question_number": 206,
    "bundesland": "General",
    "question": "Woran erinnern die sogenannten „Stolpersteine“ in Deutschland?",
    "question_en": "What are the so -called \"stumbling blocks\" in Germany?",
    "options": [
      "an berühmte deutsche Politikerinnen und Politiker",
      "an die Opfer des Nationalsozialismus",
      "an Verkehrstote",
      "an bekannte jüdische Musiker"
    ],
    "options_en": [
      "famous German politicians",
      "the victims of National Socialism",
      "traffic fatalities",
      "well-known Jewish musicians"
    ],
    "answer": "an die Opfer des Nationalsozialismus",
    "answer_en": "the victims of National Socialism",
    "image_paths": []
  },
  {
    "question_number": 207,
    "bundesland": "General",
    "question": "In welchem Militärbündnis war die DDR Mitglied?",
    "question_en": "In which military alliance was the GDR member?",
    "options": [
      "in der NATO",
      "im Rheinbund",
      "im Warschauer Pakt",
      "im Europabündnis"
    ],
    "options_en": [
      "In NATO",
      "In the Rheinbund",
      "In the Warsaw Pact",
      "In the European alliance"
    ],
    "answer": "im Warschauer Pakt",
    "answer_en": "In the Warsaw Pact",
    "image_paths": []
  },
  {
    "question_number": 208,
    "bundesland": "General",
    "question": "Was war die \"Stasi\"?",
    "question_en": "What was the \"Stasi\"?",
    "options": [
      "der Geheimdienst im \"Dritten Reich\"",
      "eine berühmte deutsche Gedenkstätte",
      "der Geheimdienst der DDR",
      "ein deutscher Sportverein während des Zweiten Weltkrieges"
    ],
    "options_en": [
      "The secret service in the \"Third Reich\"",
      "A famous German memorial",
      "The GDR secret service",
      "A German sports club during the Second World War"
    ],
    "answer": "der Geheimdienst der DDR",
    "answer_en": "The GDR secret service",
    "image_paths": []
  },
  {
    "question_number": 209,
    "bundesland": "General",
    "question": "Welches war das Wappen der Deutschen Demokratischen Republik?",
    "question_en": "What was the coat of arms of the German Democratic Republic?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 4",
    "answer_en": "Image 4",
    "image_paths": [
      "./images/general-209-option-1.png",
      "./images/general-209-option-2.png",
      "./images/general-209-option-3.png",
      "./images/general-209-option-4.jpg"
    ]
  },
  {
    "question_number": 210,
    "bundesland": "General",
    "question": "Was ereignete sich amJuni 1953 in der DDR?",
    "question_en": "What happened in the GDR in 1953?",
    "options": [
      "der feierliche Beitritt zum Warschauer Pakt",
      "landesweite Streiks und ein Volksaufstand",
      "der 1. SED-Parteitag",
      "der erste Besuch Fidel Castros"
    ],
    "options_en": [
      "The solemn accession to the Warsaw Pact",
      "nationwide strikes and a popular uprising",
      "The 1st SED party conference",
      "The first visit Fidel Castros"
    ],
    "answer": "landesweite Streiks und ein Volksaufstand",
    "answer_en": "nationwide strikes and a popular uprising",
    "image_paths": []
  },
  {
    "question_number": 211,
    "bundesland": "General",
    "question": "Welcher Politiker steht für die \"Ostverträge\"?",
    "question_en": "Which politician stands for the \"east contracts\"?",
    "options": [
      "Helmut Kohl",
      "Willy Brandt",
      "Michail Gorbatschow",
      "Ludwig Erhard"
    ],
    "options_en": [
      "Helmut Kohl",
      "Willy Brandt",
      "Michail Gorbachev",
      "Ludwig Erhard"
    ],
    "answer": "Willy Brandt",
    "answer_en": "Willy Brandt",
    "image_paths": []
  },
  {
    "question_number": 212,
    "bundesland": "General",
    "question": "Wie heißt Deutschland mit vollem Namen?",
    "question_en": "What is Germany called with a full name?",
    "options": [
      "Bundesstaat Deutschland",
      "Bundesländer Deutschland",
      "Bundesrepublik Deutschland",
      "Bundesbezirk Deutschland"
    ],
    "options_en": [
      "State of Germany",
      "Federal states of Germany",
      "Federal Republic of Germany",
      "Federal district of Germany"
    ],
    "answer": "Bundesrepublik Deutschland",
    "answer_en": "Federal Republic of Germany",
    "image_paths": []
  },
  {
    "question_number": 213,
    "bundesland": "General",
    "question": "Wie viele Einwohner hat Deutschland?",
    "question_en": "How many inhabitants does Germany have?",
    "options": [
      "70 Millionen",
      "78 Millionen",
      "84 Millionen",
      "90 Millionen"
    ],
    "options_en": [
      "70 million",
      "78 million",
      "84 million",
      "90 million"
    ],
    "answer": "84 Millionen",
    "answer_en": "84 million",
    "image_paths": []
  },
  {
    "question_number": 214,
    "bundesland": "General",
    "question": "Welche Farben hat die deutsche Flagge?",
    "question_en": "What colors does the German flag have?",
    "options": [
      "schwarz-rot-gold",
      "rot-weiß-schwarz",
      "schwarz-rot-grün",
      "schwarz-gelb-rot"
    ],
    "options_en": [
      "black-red-gold",
      "Red-white black",
      "black-red-green",
      "black and yellow-red"
    ],
    "answer": "schwarz-rot-gold",
    "answer_en": "black-red-gold",
    "image_paths": []
  },
  {
    "question_number": 215,
    "bundesland": "General",
    "question": "Wer wird als \"Kanzler der Deutschen Einheit\" bezeichnet?",
    "question_en": "Who is called the \"Chancellor of German Unity\"?",
    "options": [
      "Gerhard Schröder",
      "Helmut Kohl",
      "Konrad Adenauer",
      "Helmut Schmidt"
    ],
    "options_en": [
      "Gerhard Schröder",
      "Helmut Kohl",
      "Konrad Adenauer",
      "Helmut Schmidt"
    ],
    "answer": "Helmut Kohl",
    "answer_en": "Helmut Kohl",
    "image_paths": []
  },
  {
    "question_number": 216,
    "bundesland": "General",
    "question": "Welches Symbol ist im Plenarsaal des Deutschen Bundestages zu sehen? © Deutscher Bundestag/Janine Schmitz",
    "question_en": "Which symbol can be seen in the plenary hall of the German Bundestag?",
    "options": [
      "der Bundesadler",
      "die Fahne der Stadt Berlin",
      "der Reichsadler",
      "die Reichskrone"
    ],
    "options_en": [
      "The federal eagle",
      "The flag of the city of Berlin",
      "The imperial eagle",
      "The imperial crown"
    ],
    "answer": "der Bundesadler",
    "answer_en": "The federal eagle",
    "image_paths": [
      "./images/general-216-question.png"
    ]
  },
  {
    "question_number": 217,
    "bundesland": "General",
    "question": "In welchem Zeitraum gab es die Deutsche Demokratische Republik (DDR)?",
    "question_en": "In what period was the German Democratic Republic (GDR)?",
    "options": [
      "1919 bis 1927",
      "1933 bis 1945",
      "1945 bis 1961",
      "1949 bis 1990"
    ],
    "options_en": [
      "1919 to 1927",
      "1933 to 1945",
      "1945 to 1961",
      "1949 to 1990"
    ],
    "answer": "1949 bis 1990",
    "answer_en": "1949 to 1990",
    "image_paths": []
  },
  {
    "question_number": 218,
    "bundesland": "General",
    "question": "Wie viele Bundesländer kamen bei der Wiedervereinigung 1990 zur Bundesrepublik Deutschland hinzu?",
    "question_en": "How many federal states were added to the Federal Republic of Germany at the reunification in 1990?",
    "options": [
      "4",
      "5",
      "6",
      "7"
    ],
    "options_en": [
      "4",
      "5",
      "6",
      "7"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 219,
    "bundesland": "General",
    "question": "Die Bundesrepublik Deutschland hat die Grenzen von heute seit …",
    "question_en": "The Federal Republic of Germany has the limits of today ...",
    "options": [
      "1933",
      "1949",
      "1971",
      "1990"
    ],
    "options_en": [
      "1933",
      "1949",
      "1971",
      "1990"
    ],
    "answer": "1990",
    "answer_en": "1990",
    "image_paths": []
  },
  {
    "question_number": 220,
    "bundesland": "General",
    "question": "DerJanuar ist in Deutschland ein offizieller Gedenktag. Woran erinnert dieser Tag?",
    "question_en": "Derjuar is an official day of commemoration in Germany.",
    "options": [
      "an das Ende des Zweiten Weltkrieges",
      "an die Verabschiedung des Grundgesetzes",
      "an die Wiedervereinigung Deutschlands",
      "an die Opfer des Nationalsozialismus (Tag der Befreiung des Vernichtungslagers Auschwitz)"
    ],
    "options_en": [
      "To the end of the Second World War",
      "To the adoption of the Basic Law",
      "To the reunification of Germany",
      "To the victims of National Socialism (day of the exemption of the Auschwitz extermination camp)"
    ],
    "answer": "an die Opfer des Nationalsozialismus (Tag der Befreiung des Vernichtungslagers Auschwitz)",
    "answer_en": "To the victims of National Socialism (day of the exemption of the Auschwitz extermination camp)",
    "image_paths": []
  },
  {
    "question_number": 221,
    "bundesland": "General",
    "question": "Deutschland ist Mitglied des Schengener Abkommens. Was bedeutet das?",
    "question_en": "Germany is a member of the Schengen Agreement.",
    "options": [
      "Deutsche können in viele Länder Europas ohne Passkontrolle reisen",
      "Alle Menschen können ohne Personenkontrolle in Deutschland einreisen",
      "Deutsche können ohne Passkontrolle in jedes Land reisen",
      "Deutsche können in jedem Land mit dem Euro bezahlen"
    ],
    "options_en": [
      "Germans can travel to many countries in Europe without passport control",
      "All people can enter Germany without personal control",
      "Germans can travel to every country without passing control",
      "Germans can pay with the euro in any country"
    ],
    "answer": "Deutsche können in viele Länder Europas ohne Passkontrolle reisen",
    "answer_en": "Germans can travel to many countries in Europe without passport control",
    "image_paths": []
  },
  {
    "question_number": 222,
    "bundesland": "General",
    "question": "Welches Land ist ein Nachbarland von Deutschland?",
    "question_en": "Which country is a neighboring country of Germany?",
    "options": [
      "Ungarn",
      "Portugal",
      "Spanien",
      "Schweiz"
    ],
    "options_en": [
      "Hungary",
      "Portugal",
      "Spain",
      "Switzerland"
    ],
    "answer": "Schweiz",
    "answer_en": "Switzerland",
    "image_paths": []
  },
  {
    "question_number": 223,
    "bundesland": "General",
    "question": "Welches Land ist ein Nachbarland von Deutschland?",
    "question_en": "Which country is a neighboring country of Germany?",
    "options": [
      "Rumänien",
      "Bulgarien",
      "Polen",
      "Griechenland"
    ],
    "options_en": [
      "Romania",
      "Bulgaria",
      "Poland",
      "Greece"
    ],
    "answer": "Polen",
    "answer_en": "Poland",
    "image_paths": []
  },
  {
    "question_number": 224,
    "bundesland": "General",
    "question": "Was bedeutet die Abkürzung EU?",
    "question_en": "What does the abbreviation mean EU?",
    "options": [
      "Europäische Unternehmen",
      "Europäische Union",
      "Einheitliche Union",
      "Euro Union"
    ],
    "options_en": [
      "European companies",
      "European Union",
      "Uniform union",
      "Euro Union"
    ],
    "answer": "Europäische Union",
    "answer_en": "European Union",
    "image_paths": []
  },
  {
    "question_number": 225,
    "bundesland": "General",
    "question": "In welchem anderen Land gibt es eine große deutschsprachige Bevölkerung?",
    "question_en": "In which other country is there a large German -speaking population?",
    "options": [
      "Tschechien",
      "Norwegen",
      "Spanien",
      "Österreich"
    ],
    "options_en": [
      "Czech Republic",
      "Norway",
      "Spain",
      "Austria"
    ],
    "answer": "Österreich",
    "answer_en": "Austria",
    "image_paths": []
  },
  {
    "question_number": 226,
    "bundesland": "General",
    "question": "Welche ist die Flagge der Europäischen Union?",
    "question_en": "What is the flag of the European Union?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 2",
    "answer_en": "Image 2",
    "image_paths": [
      "./images/general-226-option-1.png",
      "./images/general-226-option-2.jpg",
      "./images/general-226-option-3.jpg",
      "./images/general-226-option-4.jpg"
    ]
  },
  {
    "question_number": 227,
    "bundesland": "General",
    "question": "Welches Land ist ein Nachbarland von Deutschland?",
    "question_en": "Which country is a neighboring country of Germany?",
    "options": [
      "Finnland",
      "Dänemark",
      "Norwegen",
      "Schweden"
    ],
    "options_en": [
      "Finland",
      "Denmark",
      "Norway",
      "Sweden"
    ],
    "answer": "Dänemark",
    "answer_en": "Denmark",
    "image_paths": []
  },
  {
    "question_number": 228,
    "bundesland": "General",
    "question": "Wie wird der Beitritt der DDR zur Bundesrepublik Deutschland im Jahr 1990 allgemein genannt?",
    "question_en": "How is the GDR accessible to the Federal Republic of Germany in general in 1990?",
    "options": [
      "NATO-Osterweiterung",
      "EU-Osterweiterung",
      "Deutsche Wiedervereinigung",
      "Europäische Gemeinschaft"
    ],
    "options_en": [
      "NATO east extension",
      "EU east expansion",
      "German reunification",
      "European Community"
    ],
    "answer": "Deutsche Wiedervereinigung",
    "answer_en": "German reunification",
    "image_paths": []
  },
  {
    "question_number": 229,
    "bundesland": "General",
    "question": "Welches Land ist ein Nachbarland von Deutschland?",
    "question_en": "Which country is a neighboring country of Germany?",
    "options": [
      "Spanien",
      "Bulgarien",
      "Norwegen",
      "Luxemburg"
    ],
    "options_en": [
      "Spain",
      "Bulgaria",
      "Norway",
      "Luxembourg"
    ],
    "answer": "Luxemburg",
    "answer_en": "Luxembourg",
    "image_paths": []
  },
  {
    "question_number": 230,
    "bundesland": "General",
    "question": "Das Europäische Parlament wird regelmäßig gewählt, nämlich alle …",
    "question_en": "The European Parliament is chosen regularly, namely all ...",
    "options": [
      "5 Jahre",
      "6 Jahre",
      "7 Jahre",
      "8 Jahre"
    ],
    "options_en": [
      "5 years",
      "6 years",
      "7 years",
      "8 years"
    ],
    "answer": "5 Jahre",
    "answer_en": "5 years",
    "image_paths": []
  },
  {
    "question_number": 231,
    "bundesland": "General",
    "question": "Was bedeutet der Begriff \"europäische Integration\"?Damit sind amerikanische Einwanderinnen und Einwanderer in Europa gemeint",
    "question_en": "What does the term \"European integration\" mean? This means American immigrants in Europe",
    "options": [
      "Damit sind amerikanische Einwanderinnen und Einwanderer in Europa gemeint",
      "Der Begriff meint den Einwanderungsstopp nach Europa",
      "Damit sind europäische Auswanderinnen und Auswanderer in den USA gemeint",
      "Der Begriff meint den Zusammenschluss europäischer Staaten zur EU"
    ],
    "options_en": [
      "This means American immigrants in Europe",
      "The term means the immigration stop to Europe",
      "This means European emigrants in the United States",
      "The term means the merger of European countries to the EU"
    ],
    "answer": "Der Begriff meint den Zusammenschluss europäischer Staaten zur EU",
    "answer_en": "The term means the merger of European countries to the EU",
    "image_paths": []
  },
  {
    "question_number": 232,
    "bundesland": "General",
    "question": "Wer wird bei der Europawahl gewählt?",
    "question_en": "Who is chosen at the European elections?",
    "options": [
      "die Europäische Kommission",
      "die Länder, die in die EU eintreten dürfen",
      "die Abgeordneten des Europäischen Parlaments",
      "die europäische Verfassung"
    ],
    "options_en": [
      "The European Commission",
      "The countries that may enter the EU",
      "The MPs of the European Parliament",
      "The European constitution"
    ],
    "answer": "die Abgeordneten des Europäischen Parlaments",
    "answer_en": "The MPs of the European Parliament",
    "image_paths": []
  },
  {
    "question_number": 233,
    "bundesland": "General",
    "question": "Welches Land ist ein Nachbarland von Deutschland?",
    "question_en": "Which country is a neighboring country of Germany?",
    "options": [
      "Tschechien",
      "Bulgarien",
      "Griechenland",
      "Portugal"
    ],
    "options_en": [
      "Czech Republic",
      "Bulgaria",
      "Greece",
      "Portugal"
    ],
    "answer": "Tschechien",
    "answer_en": "Czech Republic",
    "image_paths": []
  },
  {
    "question_number": 234,
    "bundesland": "General",
    "question": "Wo ist ein Sitz des Europäischen Parlaments?",
    "question_en": "Where is a seat of the European Parliament?",
    "options": [
      "London",
      "Paris",
      "Berlin",
      "Straßburg"
    ],
    "options_en": [
      "London",
      "Paris",
      "Berlin",
      "Strasbourg"
    ],
    "answer": "Straßburg",
    "answer_en": "Strasbourg",
    "image_paths": []
  },
  {
    "question_number": 235,
    "bundesland": "General",
    "question": "Der damalige französische Staatspräsident François Mitterrand und der damalige deutsche Bundeskanzler Helmut Kohl gedenken in Verdun gemeinsam der Toten beider Weltkriege. Welches Ziel der Europäischen Union wird bei diesem Treffen deutlich? © Bundesregierung/Richard Schulze-Vorberg",
    "question_en": "The then French President François Mitterrand and the then German Chancellor Helmut Kohl commemorate the dead of both world wars in Verdun.",
    "options": [
      "Freundschaft zwischen England und Deutschland",
      "Reisefreiheit in alle Länder der EU",
      "Frieden und Sicherheit in den Ländern der EU",
      "einheitliche Feiertage in den Ländern der EU"
    ],
    "options_en": [
      "Friendship between England and Germany",
      "Freedom of travel to all countries of the EU",
      "Peace and security in the countries of the EU",
      "uniform holidays in the countries of the EU"
    ],
    "answer": "Frieden und Sicherheit in den Ländern der EU",
    "answer_en": "Peace and security in the countries of the EU",
    "image_paths": [
      "./images/general-235-question.png"
    ]
  },
  {
    "question_number": 236,
    "bundesland": "General",
    "question": "Wie viele Mitgliedstaaten hat die EU heute?",
    "question_en": "How many member states does the EU have today?",
    "options": [
      "21",
      "23",
      "25",
      "27"
    ],
    "options_en": [
      "21",
      "23",
      "25",
      "27"
    ],
    "answer": "27",
    "answer_en": "27",
    "image_paths": []
  },
  {
    "question_number": 237,
    "bundesland": "General",
    "question": "2007 wurde das 50-jährige Jubiläum der \"Römischen Verträge\" gefeiert. Was war der Inhalt der Verträge?",
    "question_en": "In 2007 the 50th anniversary of the \"Roman contracts\" was celebrated.",
    "options": [
      "Beitritt Deutschlands zur NATO",
      "Gründung der Europäischen Wirtschaftsgemeinschaft (EWG)",
      "Verpflichtung Deutschlands zu Reparationsleistungen",
      "Festlegung der Oder-Neiße-Linie als Ostgrenze"
    ],
    "options_en": [
      "Access to Germany to NATO",
      "Foundation of the European Economic Community (EEC)",
      "Commitment to Germany's reparation services",
      "Definition of the Oder-Neisse line as an eastern border"
    ],
    "answer": "Gründung der Europäischen Wirtschaftsgemeinschaft (EWG)",
    "answer_en": "Foundation of the European Economic Community (EEC)",
    "image_paths": []
  },
  {
    "question_number": 238,
    "bundesland": "General",
    "question": "An welchen Orten arbeitet das Europäische Parlament?",
    "question_en": "In which places does the European Parliament work?",
    "options": [
      "Paris, London und Den Haag",
      "Straßburg, Luxemburg und Brüssel",
      "Rom, Bern und Wien",
      "Bonn, Zürich und Mailand"
    ],
    "options_en": [
      "Paris, London and the Hague",
      "Strasbourg, Luxembourg and Brussels",
      "Rome, Bern and Vienna",
      "Bonn, Zurich and Milan"
    ],
    "answer": "Straßburg, Luxemburg und Brüssel",
    "answer_en": "Strasbourg, Luxembourg and Brussels",
    "image_paths": []
  },
  {
    "question_number": 239,
    "bundesland": "General",
    "question": "Durch welche Verträge schloss sich die Bundesrepublik Deutschland mit anderen Staaten zur Europäischen Wirtschaftsgemeinschaft zusammen?",
    "question_en": "Through what contracts did the Federal Republic of Germany formed the European Economic Community with other countries?",
    "options": [
      "durch die \"Hamburger Verträge\"",
      "durch die \"Römischen Verträge\"",
      "durch die \"Pariser Verträge\"",
      "durch die \"Londoner Verträge\""
    ],
    "options_en": [
      "Through the \"Hamburg Contracts\"",
      "Through the \"Roman contracts\"",
      "Through the \"Paris Contracts\"",
      "Through the \"London contracts\""
    ],
    "answer": "durch die \"Römischen Verträge\"",
    "answer_en": "Through the \"Roman contracts\"",
    "image_paths": []
  },
  {
    "question_number": 240,
    "bundesland": "General",
    "question": "Seit wann bezahlt man in Deutschland mit dem Euro in bar?",
    "question_en": "Since when have you paid for the euro in Germany in Germany?",
    "options": [
      "1995",
      "1998",
      "2002",
      "2005"
    ],
    "options_en": [
      "1995",
      "1998",
      "2002",
      "2005"
    ],
    "answer": "2002",
    "answer_en": "2002",
    "image_paths": []
  },
  {
    "question_number": 241,
    "bundesland": "General",
    "question": "Frau Seger bekommt ein Kind. Was muss sie tun, um Elterngeld zu erhalten?",
    "question_en": "Ms. Seger gets a child.",
    "options": [
      "Sie muss an ihre Krankenkasse schreiben",
      "Sie muss einen Antrag bei der Elterngeldstelle stellen",
      "Sie muss nichts tun, denn sie bekommt automatisch Elterngeld",
      "Sie muss das Arbeitsamt um Erlaubnis bitten"
    ],
    "options_en": [
      "She has to write to her health insurance",
      "She has to make an application to the parental allowance office",
      "She doesn't have to do anything because she automatically receives parental allowance",
      "She must ask the employment office for permission"
    ],
    "answer": "Sie muss einen Antrag bei der Elterngeldstelle stellen",
    "answer_en": "She has to make an application to the parental allowance office",
    "image_paths": []
  },
  {
    "question_number": 242,
    "bundesland": "General",
    "question": "Wer entscheidet, ob ein Kind in Deutschland in den Kindergarten geht?",
    "question_en": "Who decides whether a child in Germany goes to kindergarten?",
    "options": [
      "der Staat",
      "die Bundesländer",
      "die Eltern/die Erziehungsberechtigten",
      "die Schulen"
    ],
    "options_en": [
      "the State",
      "The federal states",
      "The parents/legal guardians",
      "the schools"
    ],
    "answer": "die Eltern/die Erziehungsberechtigten",
    "answer_en": "The parents/legal guardians",
    "image_paths": []
  },
  {
    "question_number": 243,
    "bundesland": "General",
    "question": "Maik und Sybille wollen mit Freunden an ihrem deutschen Wohnort eine Demonstration auf der Straße abhalten. Was müssen sie vorher tun?",
    "question_en": "Maik and Sybille want to hold a demonstration on the street with friends at their German place of residence.",
    "options": [
      "Sie müssen die Demonstration anmelden",
      "Sie müssen nichts tun. Man darf in Deutschland jederzeit überall demonstrieren",
      "Sie können gar nichts tun, denn Demonstrationen sind in Deutschland grundsätzlich verboten",
      "Maik und Sybille müssen einen neuen Verein gründen, weil nur Vereine demonstrieren dürfen"
    ],
    "options_en": [
      "You have to register the demonstration",
      "You don't have to do anything.",
      "You can't do anything, because demonstrations are fundamentally prohibited in Germany",
      "Maik and Sybille have to found a new club because only clubs are allowed to demonstrate"
    ],
    "answer": "Sie müssen die Demonstration anmelden",
    "answer_en": "You have to register the demonstration",
    "image_paths": []
  },
  {
    "question_number": 244,
    "bundesland": "General",
    "question": "Welchen Schulabschluss braucht man normalerweise, um an einer Universität in Deutschland ein Studium zu beginnen?",
    "question_en": "What school graduation do you normally need to start studying at a university in Germany?",
    "options": [
      "das Abitur",
      "ein Diplom",
      "die Prokura",
      "eine Gesellenprüfung"
    ],
    "options_en": [
      "the high school diploma",
      "",
      "The procuration",
      "A journeyman's examination"
    ],
    "answer": "das Abitur",
    "answer_en": "the high school diploma",
    "image_paths": []
  },
  {
    "question_number": 245,
    "bundesland": "General",
    "question": "Wer darf in Deutschland nicht als Paar zusammenleben?",
    "question_en": "Who is not allowed to live together as a couple in Germany?",
    "options": [
      "Hans (20 Jahre) und Marie (19 Jahre)",
      "Tom (20 Jahre) und Klaus (45 Jahre)",
      "Sofie (35 Jahre) und Lisa (40 Jahre)",
      "Anne (13 Jahre) und Tim (25 Jahre)"
    ],
    "options_en": [
      "Hans (20 years) and Marie (19 years)",
      "Tom (20 years) and Klaus (45 years)",
      "Sofie (35 years) and Lisa (40 years)",
      "Anne (13 years) and Tim (25 years)"
    ],
    "answer": "Anne (13 Jahre) und Tim (25 Jahre)",
    "answer_en": "Anne (13 years) and Tim (25 years)",
    "image_paths": []
  },
  {
    "question_number": 246,
    "bundesland": "General",
    "question": "Ab welchem Alter ist man in Deutschland volljährig?",
    "question_en": "From what age is it of legal age in Germany?",
    "options": [
      "16",
      "18",
      "19",
      "21"
    ],
    "options_en": [
      "16",
      "18",
      "19",
      "21"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 247,
    "bundesland": "General",
    "question": "Eine Frau ist schwanger. Sie ist kurz vor und nach der Geburt ihres Kindes vom Gesetz besonders beschützt. Wie heißt dieser Schutz?",
    "question_en": "A woman is pregnant.",
    "options": [
      "Elternzeit",
      "Mutterschutz",
      "Geburtsvorbereitung",
      "Wochenbett"
    ],
    "options_en": [
      "Parental leave",
      "Maternity protection",
      "Birth preparation",
      "Puerperium"
    ],
    "answer": "Mutterschutz",
    "answer_en": "Maternity protection",
    "image_paths": []
  },
  {
    "question_number": 248,
    "bundesland": "General",
    "question": "Die Erziehung der Kinder ist in Deutschland ist vor allem Aufgabe …",
    "question_en": "The upbringing of children is primarily a task in Germany ...",
    "options": [
      "des Staates",
      "der Eltern",
      "der Großeltern",
      "der Schulen"
    ],
    "options_en": [
      "of the state",
      "the parents",
      "The grandparents",
      "the schools"
    ],
    "answer": "der Eltern",
    "answer_en": "the parents",
    "image_paths": []
  },
  {
    "question_number": 249,
    "bundesland": "General",
    "question": "Wer ist in Deutschland hauptsächlich verantwortlich für die Kindererziehung?",
    "question_en": "Who is mainly responsible for raising children in Germany?",
    "options": [
      "der Staat",
      "die Eltern",
      "die Verwandten",
      "die Schulen"
    ],
    "options_en": [
      "the State",
      "the parents",
      "the relatives",
      "the schools"
    ],
    "answer": "die Eltern",
    "answer_en": "the parents",
    "image_paths": []
  },
  {
    "question_number": 250,
    "bundesland": "General",
    "question": "In Deutschland hat man die besten Chancen auf einen gut bezahlten Arbeitsplatz, wenn man …",
    "question_en": "In Germany you have the best chance of a well -paid job if you ...",
    "options": [
      "katholisch ist",
      "gut ausgebildet ist",
      "eine Frau ist",
      "Mitglied einer Partei ist"
    ],
    "options_en": [
      "Catholic is",
      "is well trained",
      "a woman is",
      "Is a member of a party"
    ],
    "answer": "gut ausgebildet ist",
    "answer_en": "is well trained",
    "image_paths": []
  },
  {
    "question_number": 251,
    "bundesland": "General",
    "question": "Wenn man in Deutschland ein Kind schlägt, …",
    "question_en": "If you hit a child in Germany ...",
    "options": [
      "geht das niemanden etwas an",
      "geht das nur die Familie etwas an",
      "kann man dafür nicht bestraft werden",
      "kann man dafür bestraft werden"
    ],
    "options_en": [
      "Doesn't that matter",
      "Only the family is doing something",
      "you can't be punished for it",
      "you can be punished for it"
    ],
    "answer": "kann man dafür bestraft werden",
    "answer_en": "you can be punished for it",
    "image_paths": []
  },
  {
    "question_number": 252,
    "bundesland": "General",
    "question": "In Deutschland …darf man zur gleichen Zeit nur mit einer Partnerin/einem Partner verheiratet sein",
    "question_en": "In Germany ... at the same time you can only be married to a partner",
    "options": [
      "darf man zur gleichen Zeit nur mit einer Partnerin/einem Partner verheiratet sein",
      "kann man mehrere Ehepartnerinnen/Ehepartner gleichzeitig haben",
      "darf man nicht wieder heiraten, wenn man einmal verheiratet war",
      "darf eine Frau nicht wieder heiraten, wenn ihr Mann gestorben ist"
    ],
    "options_en": [
      "At the same time you can only be married to a partner",
      "you can have several spouses at the same time",
      "you shouldn't marry again once you were married",
      "a woman must not marry again if her husband has died"
    ],
    "answer": "darf man zur gleichen Zeit nur mit einer Partnerin/einem Partner verheiratet sein",
    "answer_en": "At the same time you can only be married to a partner",
    "image_paths": []
  },
  {
    "question_number": 253,
    "bundesland": "General",
    "question": "Wo müssen Sie sich anmelden, wenn Sie in Deutschland umziehen?",
    "question_en": "Where do you have to register if you move in Germany?",
    "options": [
      "beim Einwohnermeldeamt",
      "beim Standesamt",
      "beim Ordnungsamt",
      "beim Gewerbeamt"
    ],
    "options_en": [
      "At the residents' registration office",
      "At the registry office",
      "At the regulatory office",
      "At the commercial office"
    ],
    "answer": "beim Einwohnermeldeamt",
    "answer_en": "At the residents' registration office",
    "image_paths": []
  },
  {
    "question_number": 254,
    "bundesland": "General",
    "question": "In Deutschland dürfen Ehepaare sich scheiden lassen. Meistens müssen sie dazu das \"Trennungsjahr\" einhalten. Was bedeutet das?",
    "question_en": "In Germany, married couples are allowed to divorce.",
    "options": [
      "Der Scheidungsprozess dauert ein Jahr",
      "Die Ehegatten sind ein Jahr verheiratet, dann ist die Scheidung möglich",
      "Das Besuchsrecht für die Kinder gilt ein Jahr",
      "Die Ehegatten führen mindestens ein Jahr getrennt ihr eigenes Leben. Danach ist die Scheidung möglich"
    ],
    "options_en": [
      "The divorce process takes a year",
      "The spouses are married for a year, then the divorce is possible",
      "The right to visit the children applies one year",
      "The spouses live their own life for at least a year."
    ],
    "answer": "Die Ehegatten führen mindestens ein Jahr getrennt ihr eigenes Leben. Danach ist die Scheidung möglich",
    "answer_en": "The spouses live their own life for at least a year. After that, divorce is possible",
    "image_paths": []
  },
  {
    "question_number": 255,
    "bundesland": "General",
    "question": "Bei Erziehungsproblemen können Eltern in Deutschland Hilfe erhalten vom …",
    "question_en": "In the event of educational problems, parents in Germany can receive help from ...",
    "options": [
      "Ordnungsamt",
      "Schulamt",
      "Jugendamt",
      "Gesundheitsamt"
    ],
    "options_en": [
      "Regulatory office",
      "School office",
      "Youth welfare office",
      "Health department"
    ],
    "answer": "Jugendamt",
    "answer_en": "Youth welfare office",
    "image_paths": []
  },
  {
    "question_number": 256,
    "bundesland": "General",
    "question": "Ein Ehepaar möchte in Deutschland ein Restaurant eröffnen. Was braucht es dazu unbedingt?",
    "question_en": "A couple would like to open a restaurant in Germany.",
    "options": [
      "eine Erlaubnis der Polizei",
      "eine Genehmigung einer Partei",
      "eine Genehmigung des Einwohnermeldeamts",
      "eine Gaststättenerlaubnis von der zuständigen Behörde"
    ],
    "options_en": [
      "A police permission",
      "A party approval",
      "A approval of the residents' registration office",
      "A restaurant permit from the responsible authority"
    ],
    "answer": "eine Gaststättenerlaubnis von der zuständigen Behörde",
    "answer_en": "A restaurant permit from the responsible authority",
    "image_paths": []
  },
  {
    "question_number": 257,
    "bundesland": "General",
    "question": "Eine erwachsene Frau möchte in Deutschland das Abitur nachholen. Das kann sie an …",
    "question_en": "An adult woman wants to make up for the Abitur in Germany.",
    "options": [
      "einer Hochschule",
      "einem Abendgymnasium",
      "einer Hauptschule",
      "einer Privatuniversität"
    ],
    "options_en": [
      "a university",
      "an evening high school",
      "a secondary school",
      "a private university"
    ],
    "answer": "einem Abendgymnasium",
    "answer_en": "an evening high school",
    "image_paths": []
  },
  {
    "question_number": 258,
    "bundesland": "General",
    "question": "Was darf das Jugendamt in Deutschland?",
    "question_en": "What is the youth welfare office in Germany?",
    "options": [
      "Es entscheidet, welche Schule das Kind besucht",
      "Es kann ein Kind, das geschlagen wird oder hungern muss, aus der Familie nehmen",
      "Es bezahlt das Kindergeld an die Eltern",
      "Es kontrolliert, ob das Kind einen Kindergarten besucht"
    ],
    "options_en": [
      "It decides which school the child attends",
      "A child who is beaten or has to go hungry can take out of the family",
      "It pays child benefit to the parents",
      "It controls whether the child visits a kindergarten"
    ],
    "answer": "Es kann ein Kind, das geschlagen wird oder hungern muss, aus der Familie nehmen",
    "answer_en": "A child who is beaten or has to go hungry can take out of the family",
    "image_paths": []
  },
  {
    "question_number": 259,
    "bundesland": "General",
    "question": "Das Berufsinformationszentrum BIZ bei der Bundesagentur für Arbeit in Deutschland hilft bei der …",
    "question_en": "The BIS professional information center at the Federal Employment Agency in Germany helps with ...",
    "options": [
      "Rentenberechnung",
      "Lehrstellensuche",
      "Steuererklärung",
      "Krankenversicherung"
    ],
    "options_en": [
      "Pension calculation",
      "Search for apprenticeship",
      "Tax return",
      "health insurance"
    ],
    "answer": "Lehrstellensuche",
    "answer_en": "Search for apprenticeship",
    "image_paths": []
  },
  {
    "question_number": 260,
    "bundesland": "General",
    "question": "In Deutschland hat ein Kind in der Schule …",
    "question_en": "In Germany a child has at school ...",
    "options": [
      "Recht auf unbegrenzte Freizeit",
      "Wahlfreiheit für alle Fächer",
      "Anspruch auf Schulgeld",
      "Anwesenheitspflicht"
    ],
    "options_en": [
      "Right to unlimited leisure time",
      "Freedom of choice for all subjects",
      "Right to school fees",
      "Attendance"
    ],
    "answer": "Anwesenheitspflicht",
    "answer_en": "Attendance",
    "image_paths": []
  },
  {
    "question_number": 261,
    "bundesland": "General",
    "question": "Ein Mann möchte mit 30 Jahren in Deutschland sein Abitur nachholen. Wo kann er das tun? An …",
    "question_en": "A man wants to make up for his Abitur in Germany at the age of 30.",
    "options": [
      "einer Hochschule",
      "einem Abendgymnasium",
      "einer Hauptschule",
      "einer Privatuniversität"
    ],
    "options_en": [
      "a university",
      "an evening high school",
      "a secondary school",
      "a private university"
    ],
    "answer": "einem Abendgymnasium",
    "answer_en": "an evening high school",
    "image_paths": []
  },
  {
    "question_number": 262,
    "bundesland": "General",
    "question": "Was bedeutet in Deutschland der Grundsatz der Gleichbehandlung?",
    "question_en": "What does the principle of equal treatment mean in Germany?",
    "options": [
      "Niemand darf z.B. wegen einer Behinderung benachteiligt werden",
      "Man darf andere Personen benachteiligen, wenn ausreichende persönliche Gründe hierfür vorliegen",
      "Niemand darf gegen Personen klagen, wenn sie benachteiligt wurden",
      "Es ist für alle Gesetz, benachteiligten Gruppen jährlich Geld zu spenden"
    ],
    "options_en": [
      "For example, nobody may be disadvantaged because of a disability",
      "You can disadvantage other people if there are sufficient personal reasons for this",
      "Nobody may complain against people if they have been disadvantaged",
      "It is for all law to donate disadvantaged groups every year"
    ],
    "answer": "Niemand darf z.B. wegen einer Behinderung benachteiligt werden",
    "answer_en": "For example, nobody may be disadvantaged because of a disability",
    "image_paths": []
  },
  {
    "question_number": 263,
    "bundesland": "General",
    "question": "In Deutschland sind Jugendliche ab 14 Jahren strafmündig. Das bedeutet: Jugendliche, die 14 Jahre und älter sind und gegen Strafgesetze verstoßen, …",
    "question_en": "In Germany, young people aged 14 and over are legally impossible.",
    "options": [
      "werden bestraft",
      "werden wie Erwachsene behandelt",
      "teilen die Strafe mit ihren Eltern",
      "werden nicht bestraft"
    ],
    "options_en": [
      "be punished",
      "are treated like adults",
      "share the punishment with their parents",
      "are not punished"
    ],
    "answer": "werden bestraft",
    "answer_en": "be punished",
    "image_paths": []
  },
  {
    "question_number": 264,
    "bundesland": "General",
    "question": "Zu welchem Fest tragen Menschen in Deutschland bunte Kostüme und Masken?",
    "question_en": "Which festival do people in Germany wear colorful costumes and masks?",
    "options": [
      "am Rosenmontag",
      "am Maifeiertag",
      "beim Oktoberfest",
      "an Pfingsten"
    ],
    "options_en": [
      "On Rose Monday",
      "On May Day",
      "At the Oktoberfest",
      "at Pentecost"
    ],
    "answer": "am Rosenmontag",
    "answer_en": "On Rose Monday",
    "image_paths": []
  },
  {
    "question_number": 265,
    "bundesland": "General",
    "question": "Wohin muss man in Deutschland zuerst gehen, wenn man heiraten möchte?",
    "question_en": "Where do you have to go in Germany first if you want to get married?",
    "options": [
      "zum Einwohnermeldeamt",
      "zum Ordnungsamt",
      "zur Agentur für Arbeit",
      "zum Standesamt"
    ],
    "options_en": [
      "To the residents' registration office",
      "To the regulatory office",
      "To the employment agency",
      "To the registry office"
    ],
    "answer": "zum Standesamt",
    "answer_en": "To the registry office",
    "image_paths": []
  },
  {
    "question_number": 266,
    "bundesland": "General",
    "question": "Wann beginnt die gesetzliche Nachtruhe in Deutschland?",
    "question_en": "When does the legal night's sleep begin in Germany?",
    "options": [
      "wenn die Sonne untergeht",
      "wenn die Nachbarn schlafen gehen",
      "um 0 Uhr, Mitternacht",
      "um 22 Uhr"
    ],
    "options_en": [
      "When the sun goes down",
      "When the neighbors go to sleep",
      "at 0 a.m., midnight",
      "at 10 p.m."
    ],
    "answer": "um 22 Uhr",
    "answer_en": "at 10 p.m.",
    "image_paths": []
  },
  {
    "question_number": 267,
    "bundesland": "General",
    "question": "Eine junge Frau in Deutschland, 22 Jahre alt, lebt mit ihrem Freund zusammen. Die Eltern der Frau finden das nicht gut, weil ihnen der Freund nicht gefällt. Was können die Eltern tun?",
    "question_en": "A young woman in Germany, 22 years old, lives with her boyfriend.",
    "options": [
      "Sie müssen die Entscheidung der volljährigen Tochter respektieren",
      "Sie haben das Recht, die Tochter in die elterliche Wohnung zurückzuholen",
      "Sie können zur Polizei gehen und die Tochter anzeigen",
      "Sie suchen einen anderen Mann für die Tochter"
    ],
    "options_en": [
      "You have to respect the decision of the adult daughter",
      "You have the right to bring the daughter back to the parental apartment",
      "You can go to the police and display the daughter",
      "You are looking for another man for the daughter"
    ],
    "answer": "Sie müssen die Entscheidung der volljährigen Tochter respektieren",
    "answer_en": "You have to respect the decision of the adult daughter",
    "image_paths": []
  },
  {
    "question_number": 268,
    "bundesland": "General",
    "question": "Eine junge Frau will den Führerschein machen. Sie hat Angst vor der Prüfung, weil ihre Muttersprache nicht Deutsch ist. Was ist richtig?Sie muss mindestens zehn Jahre in Deutschland leben, bevor sie den Führerschein machen kann",
    "question_en": "A young woman wants to get a driver's license.",
    "options": [
      "Sie muss mindestens zehn Jahre in Deutschland leben, bevor sie den Führerschein machen kann",
      "Wenn sie kein Deutsch kann, darf sie keinen Führerschein haben",
      "Sie muss den Führerschein in dem Land machen, in dem man ihre Sprache spricht",
      "Sie kann die Theorie-Prüfung vielleicht in ihrer Muttersprache machen. Es gibt mehr als zehn Sprachen zur Auswahl"
    ],
    "options_en": [
      "She has to live in Germany for at least ten years before she can get the driver's license",
      "If she can't do a German, she must not have a driver's license",
      "She has to get the driver's license in the country in which one speaks her language",
      "She may be able to do the theory test in her mother tongue."
    ],
    "answer": "Sie kann die Theorie-Prüfung vielleicht in ihrer Muttersprache machen. Es gibt mehr als zehn Sprachen zur Auswahl",
    "answer_en": "She may be able to do the theory test in her mother tongue. There are more than ten languages to choose from",
    "image_paths": []
  },
  {
    "question_number": 269,
    "bundesland": "General",
    "question": "In Deutschland haben Kinder ab dem Alter von drei Jahren bis zur Ersteinschulung einen Anspruch auf …",
    "question_en": "In Germany, children are entitled to ...",
    "options": [
      "monatliches Taschengeld",
      "einen Platz in einem Sportverein",
      "einen Kindergartenplatz",
      "einen Ferienpass"
    ],
    "options_en": [
      "monthly pocket money",
      "A place in a sports club",
      "a kindergarten place",
      "a holiday pass"
    ],
    "answer": "einen Kindergartenplatz",
    "answer_en": "a kindergarten place",
    "image_paths": []
  },
  {
    "question_number": 270,
    "bundesland": "General",
    "question": "Die Volkshochschule in Deutschland ist eine Einrichtung …",
    "question_en": "The adult education center in Germany is a facility ...",
    "options": [
      "für den Religionsunterricht",
      "nur für Jugendliche",
      "zur Weiterbildung",
      "nur für Rentnerinnen und Rentner"
    ],
    "options_en": [
      "For religious education",
      "Only for young people",
      "For further training",
      "Only for pensioners"
    ],
    "answer": "zur Weiterbildung",
    "answer_en": "For further training",
    "image_paths": []
  },
  {
    "question_number": 271,
    "bundesland": "General",
    "question": "Was ist in Deutschland ein Brauch zu Weihnachten?",
    "question_en": "What is a custom for Christmas in Germany?",
    "options": [
      "bunte Eier verstecken",
      "einen Tannenbaum schmücken",
      "sich mit Masken und Kostümen verkleiden",
      "Kürbisse vor die Tür stellen"
    ],
    "options_en": [
      "Hide colorful eggs",
      "decorate a fir tree",
      "dress up with masks and costumes",
      "Put pumpkins in front of the door"
    ],
    "answer": "einen Tannenbaum schmücken",
    "answer_en": "decorate a fir tree",
    "image_paths": []
  },
  {
    "question_number": 272,
    "bundesland": "General",
    "question": "Welche Lebensform ist in Deutschland nicht erlaubt?",
    "question_en": "Which way of life is not allowed in Germany?",
    "options": [
      "Mann und Frau sind geschieden und leben mit neuen Partnern zusammen",
      "Zwei Frauen leben zusammen",
      "Ein alleinerziehender Vater lebt mit seinen zwei Kindern zusammen",
      "Ein Mann ist mit zwei Frauen zur selben Zeit verheiratet"
    ],
    "options_en": [
      "Man and woman are divorced and live with new partners",
      "Two women live together",
      "A single father lives with his two children",
      "A man is married to two women at the same time"
    ],
    "answer": "Ein Mann ist mit zwei Frauen zur selben Zeit verheiratet",
    "answer_en": "A man is married to two women at the same time",
    "image_paths": []
  },
  {
    "question_number": 273,
    "bundesland": "General",
    "question": "Bei Erziehungsproblemen gehen Sie in Deutschland …",
    "question_en": "They go in Germany with educational problems ...",
    "options": [
      "zur Ärztin/zum Arzt",
      "zum Gesundheitsamt",
      "zum Einwohnermeldeamt",
      "zum Jugendamt"
    ],
    "options_en": [
      "to the doctor",
      "to the health department",
      "To the residents' registration office",
      "to the youth welfare office"
    ],
    "answer": "zum Jugendamt",
    "answer_en": "to the youth welfare office",
    "image_paths": []
  },
  {
    "question_number": 274,
    "bundesland": "General",
    "question": "Sie haben in Deutschland absichtlich einen Brief geöffnet, der an eine andere Person adressiert ist. Was haben Sie nicht beachtet?",
    "question_en": "You have deliberately opened a letter in Germany that is addressed to another person.",
    "options": [
      "das Schweigerecht",
      "das Briefgeheimnis",
      "die Schweigepflicht",
      "die Meinungsfreiheit"
    ],
    "options_en": [
      "The silent",
      "The letter secret",
      "The duty of confidentiality",
      "freedom of expression"
    ],
    "answer": "das Briefgeheimnis",
    "answer_en": "The letter secret",
    "image_paths": []
  },
  {
    "question_number": 275,
    "bundesland": "General",
    "question": "Was braucht man in Deutschland für eine Ehescheidung?",
    "question_en": "What do you need for divorce in Germany?",
    "options": [
      "die Einwilligung der Eltern",
      "ein Attest einer Ärztin/eines Arztes",
      "die Einwilligung der Kinder",
      "die Unterstützung einer Anwältin/eines Anwalts"
    ],
    "options_en": [
      "The consent of the parents",
      "A certificate of a doctor",
      "The consent of the children",
      "The support of a lawyer"
    ],
    "answer": "die Unterstützung einer Anwältin/eines Anwalts",
    "answer_en": "The support of a lawyer",
    "image_paths": []
  },
  {
    "question_number": 276,
    "bundesland": "General",
    "question": "Was sollten Sie tun, wenn Sie von Ihrer Ansprechpartnerin/Ihrem Ansprechpartner in einer deutschen Behörde schlecht behandelt werden?",
    "question_en": "What should you do if you are treated poorly by your contact person in a German authority?",
    "options": [
      "Ich kann nichts tun",
      "Ich muss mir diese Behandlung gefallen lassen",
      "Ich drohe der Person",
      "Ich kann mich bei der Behördenleiterin/beim Behördenleiter beschweren"
    ],
    "options_en": [
      "I can't do anything",
      "I have to put up with this treatment",
      "I threaten the person",
      "I can complain to the head of the authorities"
    ],
    "answer": "Ich kann mich bei der Behördenleiterin/beim Behördenleiter beschweren",
    "answer_en": "I can complain to the head of the authorities",
    "image_paths": []
  },
  {
    "question_number": 277,
    "bundesland": "General",
    "question": "Eine Frau, die ein zweijähriges Kind hat, bewirbt sich in Deutschland um eine Stelle. Was ist ein Beispiel für Diskriminierung? Sie bekommt die Stelle nur deshalb nicht, weil sie …",
    "question_en": "A woman who has a two -year -old child is applying for a job in Germany.",
    "options": [
      "kein Englisch spricht",
      "zu hohe Gehaltsvorstellungen hat",
      "keine Erfahrungen in diesem Beruf hat",
      "Mutter ist"
    ],
    "options_en": [
      "no English speaks",
      "Has too high salary ideas",
      "has no experience in this profession",
      "Mother is"
    ],
    "answer": "Mutter ist",
    "answer_en": "Mother is",
    "image_paths": []
  },
  {
    "question_number": 278,
    "bundesland": "General",
    "question": "Ein Mann im Rollstuhl hat sich auf eine Stelle als Buchhalter beworben. Was ist ein Beispiel für Diskriminierung? Er bekommt die Stelle nur deshalb nicht, weil er …",
    "question_en": "A man in a wheelchair applied to a job as an accountant.",
    "options": [
      "im Rollstuhl sitzt",
      "keine Erfahrung hat",
      "zu hohe Gehaltsvorstellungen hat",
      "kein Englisch spricht"
    ],
    "options_en": [
      "Sits in a wheelchair",
      "has no experience",
      "Has too high salary ideas",
      "no English speaks"
    ],
    "answer": "im Rollstuhl sitzt",
    "answer_en": "Sits in a wheelchair",
    "image_paths": []
  },
  {
    "question_number": 279,
    "bundesland": "General",
    "question": "In den meisten Mietshäusern in Deutschland gibt es eine \"Hausordnung\". Was steht in einer solchen \"Hausordnung\"? Sie nennt …",
    "question_en": "In most apartment buildings in Germany there is \"house rules\".",
    "options": [
      "Regeln für die Benutzung öffentlicher Verkehrsmittel",
      "alle Mieterinnen und Mieter im Haus",
      "Regeln, an die sich alle Bewohnerinnen und Bewohner halten müssen",
      "die Adresse des nächsten Ordnungsamtes"
    ],
    "options_en": [
      "Rules for the use of public transport",
      "All tenants in the house",
      "Rules to which all residents have to adhere to",
      "The address of the next regulatory office"
    ],
    "answer": "Regeln, an die sich alle Bewohnerinnen und Bewohner halten müssen",
    "answer_en": "Rules to which all residents have to adhere to",
    "image_paths": []
  },
  {
    "question_number": 280,
    "bundesland": "General",
    "question": "Wenn Sie sich in Deutschland gegen einen falschen Steuerbescheid wehren wollen, müssen Sie …",
    "question_en": "If you want to defend yourself against a wrong tax assessment in Germany, you have to ...",
    "options": [
      "nichts machen",
      "den Bescheid wegwerfen",
      "Einspruch einlegen",
      "warten, bis ein anderer Bescheid kommt"
    ],
    "options_en": [
      "do nothing",
      "throw away",
      "raise an objection",
      "Wait until someone else comes"
    ],
    "answer": "Einspruch einlegen",
    "answer_en": "raise an objection",
    "image_paths": []
  },
  {
    "question_number": 281,
    "bundesland": "General",
    "question": "Zwei Freunde wollen in ein öffentliches Schwimmbad in Deutschland. Beide haben eine dunkle Hautfarbe und werden deshalb nicht hineingelassen. Welches Recht wird in dieser Situation verletzt? Das Recht auf …",
    "question_en": "Two friends want to go to a public swimming pool in Germany.",
    "options": [
      "Meinungsfreiheit",
      "Gleichbehandlung",
      "Versammlungsfreiheit",
      "Freizügigkeit"
    ],
    "options_en": [
      "Freedom of expression",
      "Equal treatment",
      "Freedom of assembly",
      "Freedom"
    ],
    "answer": "Gleichbehandlung",
    "answer_en": "Equal treatment",
    "image_paths": []
  },
  {
    "question_number": 282,
    "bundesland": "General",
    "question": "Welches Ehrenamt müssen deutsche Staatsbürgerinnen und Staatsbürger übernehmen, wenn sie dazu aufgefordert werden?",
    "question_en": "What volunteer German citizens have to take over if they are asked to do so?",
    "options": [
      "Vereinstrainerin/Vereinstrainer",
      "Wahlhelferin/Wahlhelfer",
      "Bibliotheksaufsicht",
      "Lehrerin/Lehrer"
    ],
    "options_en": [
      "Club trainer/club trainer",
      "Election worker/election worker",
      "Library supervision",
      "Teacher"
    ],
    "answer": "Wahlhelferin/Wahlhelfer",
    "answer_en": "Election worker/election worker",
    "image_paths": []
  },
  {
    "question_number": 283,
    "bundesland": "General",
    "question": "Was tun Sie, wenn Sie eine falsche Rechnung von einer deutschen Behörde bekommen?",
    "question_en": "What do you do if you get the wrong bill from a German authority?",
    "options": [
      "Ich lasse die Rechnung liegen",
      "Ich lege Widerspruch bei der Behörde ein",
      "Ich schicke die Rechnung an die Behörde zurück",
      "Ich gehe mit der Rechnung zum Finanzamt"
    ],
    "options_en": [
      "I leave the bill",
      "I file an objection to the authority",
      "I send the invoice back to the authority",
      "I go to the tax office with the invoice"
    ],
    "answer": "Ich lege Widerspruch bei der Behörde ein",
    "answer_en": "I file an objection to the authority",
    "image_paths": []
  },
  {
    "question_number": 284,
    "bundesland": "General",
    "question": "Was man für die Arbeit können muss, ändert sich in Zukunft sehr schnell. Was kann man tun?",
    "question_en": "What you have to be able to do for the work will change very quickly in the future.",
    "options": [
      "Es ist egal, was man lernt",
      "Erwachsene müssen auch nach der Ausbildung immer weiter lernen",
      "Kinder lernen in der Schule alles, was im Beruf wichtig ist. Nach der Schule muss man nicht weiter lernen",
      "Alle müssen früher aufhören zu arbeiten, weil sich alles ändert"
    ],
    "options_en": [
      "It doesn't matter what you learn",
      "Adults must continue to learn after their training",
      "Children learn everything that is important at work at school.",
      "Everyone has to stop working earlier because everything changes"
    ],
    "answer": "Erwachsene müssen auch nach der Ausbildung immer weiter lernen",
    "answer_en": "Adults must continue to learn after their training",
    "image_paths": []
  },
  {
    "question_number": 285,
    "bundesland": "General",
    "question": "Frau Frost arbeitet als fest angestellte Mitarbeiterin in einem Büro. Was muss sie nicht von ihrem Gehalt bezahlen?",
    "question_en": "Ms. Frost works as a permanent employee in an office.",
    "options": [
      "Lohnsteuer",
      "Beiträge zur Arbeitslosenversicherung",
      "Beiträge zur Renten- und Krankenversicherung",
      "Umsatzsteuer"
    ],
    "options_en": [
      "Wage tax",
      "Contributions to unemployment insurance",
      "Contributions to pension and health insurance",
      "Sales tax"
    ],
    "answer": "Umsatzsteuer",
    "answer_en": "Sales tax",
    "image_paths": []
  },
  {
    "question_number": 286,
    "bundesland": "General",
    "question": "Welche Organisation in einer Firma hilft den Arbeitnehmerinnen und Arbeitnehmern bei Problemen mit der Arbeitgeberin/dem Arbeitgeber?",
    "question_en": "Which organization in a company helps workers with problems with the employer?",
    "options": [
      "der Betriebsrat",
      "die Betriebsprüferin/der Betriebsprüfer",
      "die Betriebsgruppe",
      "das Betriebsmanagement"
    ],
    "options_en": [
      "The works council",
      "The auditor",
      "The company group",
      "The operating management"
    ],
    "answer": "der Betriebsrat",
    "answer_en": "The works council",
    "image_paths": []
  },
  {
    "question_number": 287,
    "bundesland": "General",
    "question": "Sie möchten bei einer Firma in Deutschland ihr Arbeitsverhältnis beenden. Was müssen Sie beachten?",
    "question_en": "You want to end your employment at a company in Germany.",
    "options": [
      "die Gehaltszahlungen",
      "die Arbeitszeit",
      "die Kündigungsfrist",
      "die Versicherungspflicht"
    ],
    "options_en": [
      "The salary payments",
      "working hours",
      "The notice period",
      "The compulsory insurance"
    ],
    "answer": "die Kündigungsfrist",
    "answer_en": "The notice period",
    "image_paths": []
  },
  {
    "question_number": 288,
    "bundesland": "General",
    "question": "Woraus begründet sich Deutschlands besondere Verantwortung für Israel?",
    "question_en": "What is Germany's special responsibility for Israel based on?",
    "options": [
      "aus der Mitgliedschaft in der Europäischen Union (EU)",
      "aus den nationalsozialistischen Verbrechen gegen Juden",
      "aus dem Grundgesetz der Bundesrepublik Deutschland",
      "aus der christlichen Tradition"
    ],
    "options_en": [
      "from membership in the European Union (EU)",
      "from the National Socialist crimes against Jews",
      "from the Basic Law of the Federal Republic of Germany",
      "from the Christian tradition"
    ],
    "answer": "aus den nationalsozialistischen Verbrechen gegen Juden",
    "answer_en": "from the Nazi crimes against Jews",
    "image_paths": []
  },
  {
    "question_number": 289,
    "bundesland": "General",
    "question": "Ein Mann mit dunkler Hautfarbe bewirbt sich um eine Stelle als Kellner in einem Restaurant in Deutschland. Was ist ein Beispiel für Diskriminierung? Er bekommt die Stelle nur deshalb nicht, weil …",
    "question_en": "A man with a dark skin color is applying for a place as a waiter in a restaurant in Germany.",
    "options": [
      "seine Deutschkenntnisse zu gering sind",
      "er zu hohe Gehaltsvorstellungen hat",
      "er eine dunkle Haut hat",
      "er keine Erfahrungen im Beruf hat"
    ],
    "options_en": [
      "his knowledge of German is too low",
      "he has too high salary ideas",
      "he has dark skin",
      "he has no experience in work"
    ],
    "answer": "er eine dunkle Haut hat",
    "answer_en": "he has dark skin",
    "image_paths": []
  },
  {
    "question_number": 290,
    "bundesland": "General",
    "question": "Sie haben in Deutschland einen Fernseher gekauft. Zu Hause packen Sie den Fernseher aus, doch er funktioniert nicht. Der Fernseher ist kaputt. Was können Sie machen?",
    "question_en": "You bought a television in Germany.",
    "options": [
      "eine Anzeige schreiben",
      "den Fernseher reklamieren",
      "das Gerät ungefragt austauschen",
      "die Garantie verlängern"
    ],
    "options_en": [
      "write a display",
      "reclaim the television",
      "Exchange the device without being asked",
      "extend the guarantee"
    ],
    "answer": "den Fernseher reklamieren",
    "answer_en": "reclaim the television",
    "image_paths": []
  },
  {
    "question_number": 291,
    "bundesland": "General",
    "question": "Warum muss man in Deutschland bei der Steuererklärung aufschreiben, ob man zu einer Kirche gehört oder nicht? Weil …es eine Kirchensteuer gibt, die an die Einkommen- und Lohnsteuer geknüpft ist",
    "question_en": "Why do you have to write down in the tax return in Germany whether you belong to a church or not?",
    "options": [
      "es eine Kirchensteuer gibt, die an die Einkommen- und Lohnsteuer geknüpft ist",
      "das für die Statistik in Deutschland wichtig ist",
      "man mehr Steuern zahlen muss, wenn man nicht zu einer Kirche gehört",
      "die Kirche für die Steuererklärung verantwortlich ist"
    ],
    "options_en": [
      "There is a church tax that is linked to income and wage tax",
      "that is important for statistics in Germany",
      "you have to pay more taxes if you don't belong to a church",
      "the church is responsible for the tax return"
    ],
    "answer": "es eine Kirchensteuer gibt, die an die Einkommen- und Lohnsteuer geknüpft ist",
    "answer_en": "There is a church tax that is linked to income and wage tax",
    "image_paths": []
  },
  {
    "question_number": 292,
    "bundesland": "General",
    "question": "Die Menschen in Deutschland leben nach dem Grundsatz der religiösen Toleranz. Was bedeutet das?",
    "question_en": "People in Germany live according to the principle of religious tolerance.",
    "options": [
      "Es dürfen keine Moscheen gebaut werden",
      "Alle Menschen glauben an Gott",
      "Jeder kann glauben, was er möchte",
      "Der Staat entscheidet, an welchen Gott die Menschen glauben"
    ],
    "options_en": [
      "No mosques may be built",
      "All people believe in God",
      "Everyone can believe what they want",
      "The state decides which God believes in people"
    ],
    "answer": "Jeder kann glauben, was er möchte",
    "answer_en": "Everyone can believe what they want",
    "image_paths": []
  },
  {
    "question_number": 293,
    "bundesland": "General",
    "question": "Was ist in Deutschland ein Brauch zu Ostern?",
    "question_en": "What is a custom in Germany at Easter?",
    "options": [
      "Kürbisse vor die Tür stellen",
      "einen Tannenbaum schmücken",
      "Eier bemalen",
      "Raketen in die Luft schießen"
    ],
    "options_en": [
      "Put pumpkins in front of the door",
      "decorate a fir tree",
      "Paint eggs",
      "Shoot missiles into the air"
    ],
    "answer": "Eier bemalen",
    "answer_en": "Paint eggs",
    "image_paths": []
  },
  {
    "question_number": 294,
    "bundesland": "General",
    "question": "Pfingsten ist ein …",
    "question_en": "Pentecost is a ...",
    "options": [
      "christlicher Feiertag",
      "deutscher Gedenktag",
      "internationaler Trauertag",
      "bayerischer Brauch"
    ],
    "options_en": [
      "Christian holiday",
      "German commemoration day",
      "International day of mourning",
      "Bavarian custom"
    ],
    "answer": "christlicher Feiertag",
    "answer_en": "Christian holiday",
    "image_paths": []
  },
  {
    "question_number": 295,
    "bundesland": "General",
    "question": "Welche Religion hat die europäische und deutsche Kultur geprägt?",
    "question_en": "What religion has European and German culture shaped?",
    "options": [
      "der Hinduismus",
      "das Christentum",
      "der Buddhismus",
      "der Islam"
    ],
    "options_en": [
      "Hinduism",
      "Christianity",
      "Buddhism",
      "Islam"
    ],
    "answer": "das Christentum",
    "answer_en": "Christianity",
    "image_paths": []
  },
  {
    "question_number": 296,
    "bundesland": "General",
    "question": "In Deutschland nennt man die letzten vier Wochen vor Weihnachten …",
    "question_en": "In Germany, the last four weeks before Christmas is called ...",
    "options": [
      "den Buß- und Bettag",
      "das Erntedankfest",
      "die Adventszeit",
      "Allerheiligen"
    ],
    "options_en": [
      "The day of penance and bed",
      "The Thanksgiving Festival",
      "The Advent season",
      "All Saints' Day"
    ],
    "answer": "die Adventszeit",
    "answer_en": "The Advent season",
    "image_paths": []
  },
  {
    "question_number": 297,
    "bundesland": "General",
    "question": "Aus welchem Land sind die meisten Migrantinnen und Migranten nach Deutschland gekommen?",
    "question_en": "Which country did most of the migrants come from?",
    "options": [
      "Italien",
      "Polen",
      "Marokko",
      "Türkei"
    ],
    "options_en": [
      "Italy",
      "Poland",
      "Morocco",
      "Türkiye"
    ],
    "answer": "Türkei",
    "answer_en": "Türkiye",
    "image_paths": []
  },
  {
    "question_number": 298,
    "bundesland": "General",
    "question": "In der DDR lebten vor allem Migrantinnen und Migranten aus …",
    "question_en": "Migrants in particular lived in the GDR ...",
    "options": [
      "Vietnam, Polen, Mosambik",
      "Frankreich, Rumänien, Somalia",
      "Chile, Ungarn, Simbabwe",
      "Nordkorea, Mexiko, Ägypten"
    ],
    "options_en": [
      "Vietnam, Poland, Mozambique",
      "France, Romania, Somalia",
      "Chile, Hungary, Zimbabwe",
      "North Korea, Mexico, Egypt"
    ],
    "answer": "Vietnam, Polen, Mosambik",
    "answer_en": "Vietnam, Poland, Mozambique",
    "image_paths": []
  },
  {
    "question_number": 299,
    "bundesland": "General",
    "question": "Ausländische Arbeitnehmerinnen und Arbeitnehmer, die in den 50er und 60er Jahren von der Bundesrepublik Deutschland angeworben wurden, nannte man …",
    "question_en": "Foreign employees who were recruited by the Federal Republic of Germany in the 1950s and 60s were called ...",
    "options": [
      "Schwarzarbeiterinnen/Schwarzarbeiter",
      "Gastarbeiterinnen/Gastarbeiter",
      "Zeitarbeiterinnen/Zeitarbeiter",
      "Schichtarbeiterinnen/Schichtarbeiter"
    ],
    "options_en": [
      "Blackworkers/illegal workers",
      "Guest workers",
      "Temporary workers",
      "Shift workers"
    ],
    "answer": "Gastarbeiterinnen/Gastarbeiter",
    "answer_en": "Guest workers",
    "image_paths": []
  },
  {
    "question_number": 300,
    "bundesland": "General",
    "question": "Aus welchem Land kamen die ersten Gastarbeiterinnen und Gastarbeiter in die Bundesrepublik Deutschland?",
    "question_en": "Which country did the first guest workers come from to the Federal Republic of Germany?",
    "options": [
      "Italien",
      "Spanien",
      "Portugal",
      "Türkei"
    ],
    "options_en": [
      "Italy",
      "Spain",
      "Portugal",
      "Türkiye"
    ],
    "answer": "Italien",
    "answer_en": "Italy",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Baden-Württemberg",
    "question": "Welches Wappen gehört zum Bundesland Baden-Württemberg?",
    "question_en": "Which coat of arms belongs to the state of Baden-Württemberg?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 1",
    "answer_en": "Image 1",
    "image_paths": [
      "./images/baden-w-rttemberg-1-option-1.jpg",
      "./images/baden-w-rttemberg-1-option-2.png",
      "./images/baden-w-rttemberg-1-option-3.png",
      "./images/baden-w-rttemberg-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Baden-Württemberg",
    "question": "Welches ist ein Landkreis in Baden-Württemberg?",
    "question_en": "Which is a district in Baden-Württemberg?",
    "options": [
      "Mecklenburgische Seenplatte",
      "Neckar-Odenwald-Kreis",
      "Nordfriesland",
      "Altötting"
    ],
    "options_en": [
      "Mecklenburg Lake District",
      "Neckar-Odenwald-Kreis",
      "North Friesland",
      "Altötting"
    ],
    "answer": "Neckar-Odenwald-Kreis",
    "answer_en": "Neckar-Odenwald-Kreis",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Baden-Württemberg",
    "question": "Für wie viele Jahre wird der Landtag in Baden-Württemberg gewählt?",
    "question_en": "How many years is the state parliament in Baden-Württemberg elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Baden-Württemberg",
    "question": "Ab welchem Alter darf man in Baden-Württemberg bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Baden-Württemberg?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Baden-Württemberg",
    "question": "Welche Farben hat die Landesflagge von Baden-Württemberg?",
    "question_en": "What colors does the state flag of Baden-Württemberg have?",
    "options": [
      "blau-weiß-rot",
      "schwarz-gold",
      "weiß-blau",
      "grün-weiß-rot"
    ],
    "options_en": [
      "blue-white-red",
      "black-gold",
      "white-blue",
      "green-white-red"
    ],
    "answer": "schwarz-gold",
    "answer_en": "black-gold",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Baden-Württemberg",
    "question": "Wo können Sie sich in Baden-Württemberg über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in Baden-Württemberg?",
    "options": [
      "beim Ordnungsamt der Gemeinde",
      "bei der Verbraucherzentrale",
      "bei der Landeszentrale für politische Bildung",
      "bei den Kirchen"
    ],
    "options_en": [
      "at the regulatory office of the municipality",
      "At the consumer center",
      "At the State Center for Political Education",
      "In the churches"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Baden-Württemberg",
    "question": "Die Landeshauptstadt von Baden-Württemberg heißt …",
    "question_en": "The state capital of Baden-Württemberg is called ...",
    "options": [
      "Heidelberg",
      "Stuttgart",
      "Karlsruhe",
      "Mannheim"
    ],
    "options_en": [
      "Heidelberg",
      "Stuttgart",
      "Karlsruhe",
      "Mannheim"
    ],
    "answer": "Stuttgart",
    "answer_en": "Stuttgart",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Baden-Württemberg",
    "question": "Welches Bundesland ist Baden-Württemberg?",
    "question_en": "Which federal state is Baden-Württemberg?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "2",
    "answer_en": "2",
    "image_paths": [
      "./images/baden-w-rttemberg-8-question.png"
    ]
  },
  {
    "question_number": 9,
    "bundesland": "Baden-Württemberg",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Baden-Württemberg?",
    "question_en": "What do you call the head of government in Baden-Württemberg?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Baden-Württemberg",
    "question": "Welche Ministerin/welchen Minister hat Baden-Württemberg nicht?",
    "question_en": "Which minister does not have Baden-Württemberg?",
    "options": [
      "Finanzministerin/Finanzminister",
      "Justizministerin/Justizminister",
      "Innenministerin/Innenminister",
      "Außenministerin/Außenminister"
    ],
    "options_en": [
      "Finance Minister",
      "Minister of Justice/Minister of Justice",
      "Minister of the Interior",
      "Foreign Minister/Foreign Minister"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Bayern",
    "question": "Welches Wappen gehört zum Freistaat Bayern?",
    "question_en": "Which coat of arms belongs to the Free State of Bavaria?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 2",
    "answer_en": "Image 2",
    "image_paths": [
      "./images/bayern-1-option-1.jpg",
      "./images/bayern-1-option-2.png",
      "./images/bayern-1-option-3.png",
      "./images/bayern-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Bayern",
    "question": "Welches ist ein Landkreis in Bayern?",
    "question_en": "Which is a district in Bavaria?",
    "options": [
      "Prignitz",
      "Rhein-Sieg-Kreis",
      "Nordfriesland",
      "Altötting"
    ],
    "options_en": [
      "Prignitz",
      "Rhein-Sieg-Kreis",
      "North Friesland",
      "Altötting"
    ],
    "answer": "Altötting",
    "answer_en": "Altötting",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Bayern",
    "question": "Für wie viele Jahre wird der Landtag in Bayern gewählt?",
    "question_en": "How many years is the state parliament elected in Bavaria?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Bayern",
    "question": "Ab welchem Alter darf man in Bayern bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Bavaria?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Bayern",
    "question": "Welche Farben hat die Landesflagge von Bayern?",
    "question_en": "What colors does the state flag of Bavaria have?",
    "options": [
      "blau-weiß-rot",
      "weiß-blau",
      "grün-weiß-rot",
      "schwarz-gelb"
    ],
    "options_en": [
      "blue-white-red",
      "white-blue",
      "green-white-red",
      "black and yellow"
    ],
    "answer": "weiß-blau",
    "answer_en": "white-blue",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Bayern",
    "question": "Wo können Sie sich in Bayern über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Bavaria?",
    "options": [
      "beim Ordnungsamt der Gemeinde",
      "bei der Landeszentrale für politische Bildung",
      "bei der Verbraucherzentrale",
      "bei den Kirchen"
    ],
    "options_en": [
      "at the regulatory office of the municipality",
      "At the State Center for Political Education",
      "At the consumer center",
      "In the churches"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Bayern",
    "question": "Die Landeshauptstadt von Bayern heißt …",
    "question_en": "The state capital of Bavaria is called ...",
    "options": [
      "Ingolstadt",
      "Regensburg",
      "Nürnberg",
      "München"
    ],
    "options_en": [
      "Ingolstadt",
      "regensburg",
      "Nuremberg",
      "Munich"
    ],
    "answer": "München",
    "answer_en": "Munich",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Bayern",
    "question": "Welches Bundesland ist Bayern?",
    "question_en": "Which federal state is Bavaria?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "4",
    "answer_en": "4",
    "image_paths": ["./images/bayern-8-question.png"]
  },
  {
    "question_number": 9,
    "bundesland": "Bayern",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Bayern?",
    "question_en": "What do you call the head of government in Bavaria?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Bayern",
    "question": "Welche Ministerin/welchen Minister hat Bayern nicht?",
    "question_en": "Which minister does not have Bayern?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Berlin",
    "question": "Welches Wappen gehört zum Bundesland Berlin?",
    "question_en": "Which coat of arms belongs to the state of Berlin?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 4",
    "answer_en": "Image 4",
    "image_paths": [
      "./images/berlin-1-option-1.png",
      "./images/berlin-1-option-2.png",
      "./images/berlin-1-option-3.png",
      "./images/berlin-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Berlin",
    "question": "Welches ist ein Bezirk von Berlin?",
    "question_en": "Which is a district of Berlin?",
    "options": [
      "Altona",
      "Prignitz",
      "Pankow",
      "Mecklenburgische Seenplatte"
    ],
    "options_en": [
      "Altona",
      "Prignitz",
      "Pankow",
      "Mecklenburg Lake District"
    ],
    "answer": "Pankow",
    "answer_en": "Pankow",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Berlin",
    "question": "Für wie viele Jahre wird das Landesparlament in Berlin gewählt?",
    "question_en": "How many years is the state parliament in Berlin elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Berlin",
    "question": "Ab welchem Alter darf man in Berlin bei Kommunalwahlen (Wahl der Bezirksverordnetenversammlung) wählen?",
    "question_en": "From what age can you choose in Berlin in local elections (election of the district council)?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Berlin",
    "question": "Welche Farben hat die Landesflagge von Berlin?",
    "question_en": "What colors does the state flag of Berlin have?",
    "options": [
      "blau-weiß-rot",
      "weiß-rot",
      "grün-weiß-rot",
      "schwarz-gold"
    ],
    "options_en": [
      "blue-white-red",
      "white-red",
      "green-white-red",
      "black-gold"
    ],
    "answer": "weiß-rot",
    "answer_en": "white-red",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Berlin",
    "question": "Wo können Sie sich in Berlin über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Berlin?",
    "options": [
      "beim Ordnungsamt der Gemeinde",
      "bei den Kirchen",
      "bei der Verbraucherzentrale",
      "bei der Landeszentrale für politische Bildung"
    ],
    "options_en": [
      "at the regulatory office of the municipality",
      "In the churches",
      "At the consumer center",
      "At the State Center for Political Education"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Berlin",
    "question": "Welches Bundesland ist ein Stadtstaat?",
    "question_en": "Which federal state is a city state?",
    "options": [
      "Berlin",
      "Saarland",
      "Brandenburg",
      "Hessen"
    ],
    "options_en": [
      "Berlin",
      "Saarland",
      "Brandenburg",
      "Hesse"
    ],
    "answer": "Berlin",
    "answer_en": "Berlin",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Berlin",
    "question": "Welches Bundesland ist Berlin?",
    "question_en": "Which federal state is Berlin?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "4",
    "answer_en": "4",
    "image_paths": [
      "./images/berlin-8-question.png"
    ]
  },
  {
    "question_number": 9,
    "bundesland": "Berlin",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef des Stadtstaates Berlin?",
    "question_en": "What do you call the head of government of the city state of Berlin?",
    "options": [
      "Ministerpräsidentin/Ministerpräsident",
      "Oberbürgermeisterin/Oberbürgermeister",
      "Präsidentin/Präsident des Senates",
      "Regierende Bürgermeisterin/Regierender Bürgermeister"
    ],
    "options_en": [
      "Prime Minister/Prime Minister",
      "Mayor/Mayor",
      "President of the Senate",
      "Governing Mayor"
    ],
    "answer": "Regierende Bürgermeisterin/Regierender Bürgermeister",
    "answer_en": "Governing Mayor",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Berlin",
    "question": "Welche Senatorin/welchen Senator hat Berlin nicht?",
    "question_en": "Which senator does not have Berlin?",
    "options": [
      "Finanzsenatorin/Finanzsenator",
      "Innensenatorin/Innensenator",
      "Senatorin/Senator für Außenbeziehungen",
      "Justizsenatorin/Justizsenator"
    ],
    "options_en": [
      "Finance senator",
      "Senator of the interior",
      "Senator for external relationships",
      "Justice Senator"
    ],
    "answer": "Senatorin/Senator für Außenbeziehungen",
    "answer_en": "Senator for external relationships",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Brandenburg",
    "question": "Welches Wappen gehört zum Bundesland Brandenburg?",
    "question_en": "Which coat of arms belongs to the state of Brandenburg?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 1",
    "answer_en": "Image 1",
    "image_paths": [
      "./images/brandenburg-1-option-1.png",
      "./images/brandenburg-1-option-2.png",
      "./images/brandenburg-1-option-3.png",
      "./images/brandenburg-1-option-4.jpg"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Brandenburg",
    "question": "Welches ist ein Landkreis in Brandenburg?",
    "question_en": "Which is a district in Brandenburg?",
    "options": [
      "Prignitz",
      "Rhein-Sieg-Kreis",
      "Vogtlandkreis",
      "Amberg-Sulzbach"
    ],
    "options_en": [
      "Prignitz",
      "Rhein-Sieg-Kreis",
      "Vogtlandkreis",
      "Amberg-Sulzbach"
    ],
    "answer": "Prignitz",
    "answer_en": "Prignitz",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Brandenburg",
    "question": "Für wie viele Jahre wird der Landtag in Brandenburg gewählt?",
    "question_en": "How many years is the state parliament in Brandenburg elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Brandenburg",
    "question": "Ab welchem Alter darf man in Brandenburg bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Brandenburg?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Brandenburg",
    "question": "Welche Farben hat die Landesflagge von Brandenburg?",
    "question_en": "What colors does the state flag of Brandenburg have?",
    "options": [
      "blau-weiß-rot",
      "rot-weiß",
      "grün-weiß-rot",
      "schwarz-gelb"
    ],
    "options_en": [
      "blue-white-red",
      "red-white",
      "green-white-red",
      "black and yellow"
    ],
    "answer": "rot-weiß",
    "answer_en": "red-white",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Brandenburg",
    "question": "Wo können Sie sich in Brandenburg über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Brandenburg?",
    "options": [
      "bei den Kirchen",
      "beim Ordnungsamt der Gemeinde",
      "bei der Landeszentrale für politische Bildung",
      "bei der Verbraucherzentrale"
    ],
    "options_en": [
      "In the churches",
      "at the regulatory office of the municipality",
      "At the State Center for Political Education",
      "At the consumer center"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Brandenburg",
    "question": "Die Landeshauptstadt von Brandenburg heißt …",
    "question_en": "The state capital of Brandenburg is called ...",
    "options": [
      "Potsdam",
      "Cottbus",
      "Brandenburg",
      "Frankfurt/Oder"
    ],
    "options_en": [
      "Potsdam",
      "cottbus",
      "Brandenburg",
      "Frankfurt/Oder"
    ],
    "answer": "Potsdam",
    "answer_en": "Potsdam",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Brandenburg",
    "question": "Welches Bundesland ist Brandenburg?",
    "question_en": "Which federal state is Brandenburg?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "4",
    "answer_en": "4",
    "image_paths": [
      "./images/brandenburg-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Brandenburg",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Brandenburg?",
    "question_en": "What do you call the head of government in Brandenburg?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Brandenburg",
    "question": "Welche Ministerin/welchen Minister hat Brandenburg nicht?",
    "question_en": "Which minister does not have Brandenburg?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Bremen",
    "question": "Welches Wappen gehört zur Freien Hansestadt Bremen?",
    "question_en": "Which coat of arms belongs to the free Hanseatic city of Bremen?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 3",
    "answer_en": "Image 3",
    "image_paths": [
      "./images/bremen-1-option-1.png",
      "./images/bremen-1-option-2.jpg",
      "./images/bremen-1-option-3.png",
      "./images/bremen-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Bremen",
    "question": "Welches ist ein Stadtteil von Bremen?",
    "question_en": "Which is a district of Bremen?",
    "options": [
      "Altona",
      "Hemelingen",
      "Pankow",
      "Babelsberg"
    ],
    "options_en": [
      "Altona",
      "Hemelingen",
      "Pankow",
      "Babelsberg"
    ],
    "answer": "Hemelingen",
    "answer_en": "Hemelingen",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Bremen",
    "question": "Für wie viele Jahre wird das Landesparlament in Bremen gewählt?",
    "question_en": "How many years is the state parliament in Bremen elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "4",
    "answer_en": "4",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Bremen",
    "question": "Ab welchem Alter darf man in Bremen bei den Wahlen zur Bürgerschaft (Landtag) wählen?",
    "question_en": "From what age can you vote in Bremen in the elections to the citizens (state parliament)?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Bremen",
    "question": "Welche Farben hat die Landesflagge von Bremen?",
    "question_en": "What colors does the Bremen state flag have?",
    "options": [
      "blau-weiß-rot",
      "rot-weiß",
      "grün-weiß-rot",
      "schwarz-gold"
    ],
    "options_en": [
      "blue-white-red",
      "red-white",
      "green-white-red",
      "black-gold"
    ],
    "answer": "rot-weiß",
    "answer_en": "red-white",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Bremen",
    "question": "Wo können Sie sich in Bremen über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Bremen?",
    "options": [
      "beim Ordnungsamt der Gemeinde",
      "bei der Landeszentrale für politische Bildung",
      "bei den Kirchen",
      "bei der Verbraucherzentrale"
    ],
    "options_en": [
      "at the regulatory office of the municipality",
      "At the State Center for Political Education",
      "In the churches",
      "At the consumer center"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Bremen",
    "question": "Was ist ein deutscher Stadtstaat?",
    "question_en": "What is a German city state?",
    "options": [
      "Bremen",
      "München",
      "Frankfurt",
      "Erfurt"
    ],
    "options_en": [
      "Bremen",
      "Munich",
      "Frankfurt",
      "Erfurt"
    ],
    "answer": "Bremen",
    "answer_en": "Bremen",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Bremen",
    "question": "Welches Bundesland ist Bremen?",
    "question_en": "Which federal state is Bremen?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "1",
    "answer_en": "1",
    "image_paths": [
      "./images/bremen-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Bremen",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef des Stadtstaates Bremen?",
    "question_en": "What do you call the head of government of the city state of Bremen?",
    "options": [
      "Ministerpräsidentin/Ministerpräsident",
      "Erste Bürgermeisterin/Erster Bürgermeister",
      "Präsidentin/Präsident des Senates",
      "Regierende Bürgermeisterin/Regierender Bürgermeister"
    ],
    "options_en": [
      "Prime Minister/Prime Minister",
      "First mayor",
      "President of the Senate",
      "Governing Mayor"
    ],
    "answer": "Präsidentin/Präsident des Senates",
    "answer_en": "President of the Senate",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Bremen",
    "question": "Welche Senatorin/welchen Senator hat Bremen nicht?",
    "question_en": "Which senator does not have Bremen?",
    "options": [
      "Senatorin/Senator für Außenbeziehungen",
      "Finanzsenatorin/Finanzsenator",
      "Justizsenatorin/Justizsenator",
      "Innensenatorin/Innensenator"
    ],
    "options_en": [
      "Senator for external relationships",
      "Finance senator",
      "Justice Senator",
      "Senator of the interior"
    ],
    "answer": "Senatorin/Senator für Außenbeziehungen",
    "answer_en": "Senator for external relationships",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Hamburg",
    "question": "Welches Wappen gehört zur Freien und Hansestadt Hamburg?",
    "question_en": "Which coat of arms belongs to the Free and Hanseatic City of Hamburg?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 2",
    "answer_en": "Image 2",
    "image_paths": [
      "./images/hamburg-1-option-1.png",
      "./images/hamburg-1-option-2.png",
      "./images/hamburg-1-option-3.png",
      "./images/hamburg-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Hamburg",
    "question": "Welches ist ein Bezirk von Hamburg?",
    "question_en": "Which is a district of Hamburg?",
    "options": [
      "Altona",
      "Hemelingen",
      "Pankow",
      "Mecklenburgische Seenplatte"
    ],
    "options_en": [
      "Altona",
      "Hemelingen",
      "Pankow",
      "Mecklenburg Lake District"
    ],
    "answer": "Altona",
    "answer_en": "Altona",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Hamburg",
    "question": "Für wie viele Jahre wird das Landesparlament in Hamburg gewählt?",
    "question_en": "How many years is the state parliament in Hamburg elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Hamburg",
    "question": "Ab welchem Alter darf man in Hamburg bei Kommunalwahlen (Wahl der Bezirksversammlungen) wählen?",
    "question_en": "From what age can you choose in Hamburg in local elections (election of the district meetings)?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Hamburg",
    "question": "Welche Farben hat die Landesflagge von Hamburg?",
    "question_en": "What colors does the state flag of Hamburg have?",
    "options": [
      "blau-weiß-rot",
      "weiß-rot",
      "grün-weiß-rot",
      "schwarz-gelb"
    ],
    "options_en": [
      "blue-white-red",
      "white-red",
      "green-white-red",
      "black and yellow"
    ],
    "answer": "weiß-rot",
    "answer_en": "white-red",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Hamburg",
    "question": "Wo können Sie sich in Hamburg über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Hamburg?",
    "options": [
      "beim Ordnungsamt der Gemeinde",
      "bei der Verbraucherzentrale",
      "bei den Kirchen",
      "bei der Landeszentrale für politische Bildung"
    ],
    "options_en": [
      "at the regulatory office of the municipality",
      "At the consumer center",
      "In the churches",
      "At the State Center for Political Education"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Hamburg",
    "question": "Welches Bundesland ist ein Stadtstaat?",
    "question_en": "Which federal state is a city state?",
    "options": [
      "Hamburg",
      "Sachsen",
      "Bayern",
      "Thüringen"
    ],
    "options_en": [
      "Hamburg",
      "Saxony",
      "Bavaria",
      "Thuringia"
    ],
    "answer": "Hamburg",
    "answer_en": "Hamburg",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Hamburg",
    "question": "Welches Bundesland ist Hamburg?",
    "question_en": "Which federal state is Hamburg?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "3",
    "answer_en": "3",
    "image_paths": [
      "./images/hamburg-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Hamburg",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef des Stadtstaates Hamburg?",
    "question_en": "What do you call the heads of government of the Hamburg City State?",
    "options": [
      "Ministerpräsidentin/Ministerpräsident",
      "Erste Bürgermeisterin/Erster Bürgermeister",
      "Regierende Senatorin/Regierender Senator",
      "Oberbürgermeisterin/Oberbürgermeister"
    ],
    "options_en": [
      "Prime Minister/Prime Minister",
      "First mayor",
      "Governing Senator/Governing Senator",
      "Mayor/Mayor"
    ],
    "answer": "Regierende Senatorin/Regierender Senator",
    "answer_en": "Governing Senator/Governing Senator",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Hamburg",
    "question": "Welche Senatorin/welchen Senator hat Hamburg nicht?",
    "question_en": "Which senator does not have Hamburg?",
    "options": [
      "Justizsenatorin/Justizsenator",
      "Senatorin/Senator für Außenbeziehungen",
      "Finanzsenatorin/Finanzsenator",
      "Innensenatorin/Innensenator"
    ],
    "options_en": [
      "Justice Senator",
      "Senator for external relationships",
      "Finance senator",
      "Senator of the interior"
    ],
    "answer": "Senatorin/Senator für Außenbeziehungen",
    "answer_en": "Senator for external relationships",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Hessen",
    "question": "Welches Wappen gehört zum Bundesland Hessen?",
    "question_en": "Which coat of arms belongs to the state of Hesse?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 1",
    "answer_en": "Image 1",
    "image_paths": [
      "./images/hessen-1-option-1.png",
      "./images/hessen-1-option-2.png",
      "./images/hessen-1-option-3.png",
      "./images/hessen-1-option-4.jpg"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Hessen",
    "question": "Welches ist ein Landkreis in Hessen?",
    "question_en": "Which is a district in Hesse?",
    "options": [
      "Ammerland",
      "Altötting",
      "Prignitz",
      "Main-Taunus-Kreis"
    ],
    "options_en": [
      "Ammerland",
      "Altötting",
      "Prignitz",
      "Main-Taunus-Kreis"
    ],
    "answer": "Main-Taunus-Kreis",
    "answer_en": "Main-Taunus-Kreis",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Hessen",
    "question": "Für wie viele Jahre wird der Landtag in Hessen gewählt?",
    "question_en": "How many years is the state parliament in Hesse elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Hessen",
    "question": "Ab welchem Alter darf man in Hessen bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Hesse?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Hessen",
    "question": "Welche Farben hat die Landesflagge von Hessen?",
    "question_en": "What colors does the Hesse state flag have?",
    "options": [
      "blau-weiß-rot",
      "rot-weiß",
      "schwarz-gold",
      "grün-weiß-rot"
    ],
    "options_en": [
      "blue-white-red",
      "red-white",
      "black-gold",
      "green-white-red"
    ],
    "answer": "rot-weiß",
    "answer_en": "red-white",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Hessen",
    "question": "Wo können Sie sich in Hessen über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Hesse?",
    "options": [
      "bei der Landeszentrale für politische Bildung",
      "bei der Verbraucherzentrale",
      "beim Ordnungsamt der Gemeinde",
      "bei den Kirchen"
    ],
    "options_en": [
      "At the State Center for Political Education",
      "At the consumer center",
      "at the regulatory office of the municipality",
      "In the churches"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Hessen",
    "question": "Die Landeshauptstadt von Hessen heißt …",
    "question_en": "The state capital of Hesse is called ...",
    "options": [
      "Kassel",
      "Darmstadt",
      "Frankfurt",
      "Wiesbaden"
    ],
    "options_en": [
      "kassel",
      "Darmstadt",
      "Frankfurt",
      "Wiesbaden"
    ],
    "answer": "Wiesbaden",
    "answer_en": "Wiesbaden",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Hessen",
    "question": "Welches Bundesland ist Hessen?",
    "question_en": "Which federal state is Hessen?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "3",
    "answer_en": "3",
    "image_paths": [
      "./images/hessen-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Hessen",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Hessen?",
    "question_en": "What do you call the head of government in Hesse?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Hessen",
    "question": "Welche Ministerin/welchen Minister hat Hessen nicht?",
    "question_en": "Which minister does not have Hessen?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Welches Wappen gehört zum Bundesland Mecklenburg-Vorpommern?",
    "question_en": "Which coat of arms belongs to the state of Mecklenburg-Western Pomerania?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 3",
    "answer_en": "Image 3",
    "image_paths": [
      "./images/mecklenburg-vorpommern-1-option-1.png",
      "./images/mecklenburg-vorpommern-1-option-2.png",
      "./images/mecklenburg-vorpommern-1-option-3.png",
      "./images/mecklenburg-vorpommern-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Welches ist ein Landkreis in Mecklenburg-Vorpommern?",
    "question_en": "Which is a district in Mecklenburg-Western Pomerania?",
    "options": [
      "Prignitz",
      "Mecklenburgische Seenplatte",
      "Vogtlandkreis",
      "Rhein-Sieg-Kreis"
    ],
    "options_en": [
      "Prignitz",
      "Mecklenburg Lake District",
      "Vogtlandkreis",
      "Rhein-Sieg-Kreis"
    ],
    "answer": "Mecklenburgische Seenplatte",
    "answer_en": "Mecklenburg Lake District",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Für wie viele Jahre wird der Landtag in Mecklenburg-Vorpommern gewählt?",
    "question_en": "How many years is the state parliament elected in Mecklenburg-Western Pomerania?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Ab welchem Alter darf man in Mecklenburg-Vorpommern bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Mecklenburg-Western Pomerania?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Welche Farben hat die Landesflagge von Mecklenburg-Vorpommern?",
    "question_en": "What colors does the state flag of Mecklenburg-Western Pomerania have?",
    "options": [
      "schwarz-rot-gold",
      "blau-weiß-gelb-rot",
      "grün-weiß-rot",
      "schwarz-gelb"
    ],
    "options_en": [
      "black-red-gold",
      "Blue and white-yellow-red",
      "green-white-red",
      "black and yellow"
    ],
    "answer": "blau-weiß-gelb-rot",
    "answer_en": "Blue and white-yellow-red",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Wo können Sie sich in Mecklenburg-Vorpommern über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in Mecklenburg-Western Pomerania?",
    "options": [
      "bei der Landeszentrale für politische Bildung",
      "bei den Kirchen",
      "beim Ordnungsamt der Gemeinde",
      "bei der Verbraucherzentrale"
    ],
    "options_en": [
      "At the State Center for Political Education",
      "In the churches",
      "at the regulatory office of the municipality",
      "At the consumer center"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Die Landeshauptstadt von Mecklenburg-Vorpommern heißt …",
    "question_en": "The state capital of Mecklenburg-Western Pomerania is called ...",
    "options": [
      "Greifswald",
      "Schwerin",
      "Rostock",
      "Wismar"
    ],
    "options_en": [
      "Greifswald",
      "Schwerin",
      "Rostock",
      "Wismar"
    ],
    "answer": "Schwerin",
    "answer_en": "Schwerin",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Welches Bundesland ist Mecklenburg-Vorpommern?",
    "question_en": "Which federal state is Mecklenburg-Western Pomerania?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "3",
    "answer_en": "3",
    "image_paths": [
      "./images/mecklenburg-vorpommern-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Mecklenburg-Vorpommern?",
    "question_en": "What do you call the head of government in Mecklenburg-Western Pomerania?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Mecklenburg-Vorpommern",
    "question": "Welche Ministerin/welchen Minister hat Mecklenburg-Vorpommern nicht?",
    "question_en": "Which minister does not have Mecklenburg-Western Pomerania?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Niedersachsen",
    "question": "Welches Wappen gehört zum Bundesland Niedersachsen?",
    "question_en": "Which coat of arms belongs to the state of Lower Saxony?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 3",
    "answer_en": "Image 3",
    "image_paths": [
      "./images/niedersachsen-1-option-1.png",
      "./images/niedersachsen-1-option-2.png",
      "./images/niedersachsen-1-option-3.png",
      "./images/niedersachsen-1-option-4.jpg"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Niedersachsen",
    "question": "Welches ist ein Landkreis in Niedersachsen?",
    "question_en": "Which is a district in Lower Saxony?",
    "options": [
      "Ammerland",
      "Rhein-Sieg-Kreis",
      "Nordfriesland",
      "Vogtlandkreis"
    ],
    "options_en": [
      "Ammerland",
      "Rhein-Sieg-Kreis",
      "North Friesland",
      "Vogtlandkreis"
    ],
    "answer": "Ammerland",
    "answer_en": "Ammerland",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Niedersachsen",
    "question": "Für wie viele Jahre wird der Landtag in Niedersachsen gewählt?",
    "question_en": "How many years is the state parliament in Lower Saxony elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Niedersachsen",
    "question": "Ab welchem Alter darf man in Niedersachsen bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Lower Saxony?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Niedersachsen",
    "question": "Welche Farben hat die Landesflagge von Niedersachsen?",
    "question_en": "What colors does the state flag of Lower Saxony have?",
    "options": [
      "weiß-blau",
      "schwarz-rot-gold",
      "schwarz-gelb",
      "blau-weiß-red"
    ],
    "options_en": [
      "white-blue",
      "black-red-gold",
      "black and yellow",
      "blue-white-red"
    ],
    "answer": "schwarz-rot-gold",
    "answer_en": "black-red-gold",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Niedersachsen",
    "question": "Wo können Sie sich in Niedersachsen über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in Lower Saxony?",
    "options": [
      "bei der Landeszentrale für politische Bildung",
      "beim Ordnungsamt der Gemeinde",
      "bei der Verbraucherzentrale",
      "bei den Kirchen"
    ],
    "options_en": [
      "At the State Center for Political Education",
      "at the regulatory office of the municipality",
      "At the consumer center",
      "In the churches"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Niedersachsen",
    "question": "Die Landeshauptstadt von Niedersachsen heißt …",
    "question_en": "The state capital of Lower Saxony is called ...",
    "options": [
      "Hannover",
      "Braunschweig",
      "Wolfsburg",
      "Osnabrück"
    ],
    "options_en": [
      "Hanover",
      "Braunschweig",
      "Wolfsburg",
      "Osnabrück"
    ],
    "answer": "Hannover",
    "answer_en": "Hanover",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Niedersachsen",
    "question": "Welches Bundesland ist Niedersachsen?",
    "question_en": "Which federal state is Lower Saxony?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "1",
    "answer_en": "1",
    "image_paths": [
      "./images/niedersachsen-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Niedersachsen",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Niedersachsen?",
    "question_en": "What do you call the head of government in Lower Saxony?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Niedersachsen",
    "question": "Welche Ministerin/welchen Minister hat Niedersachsen nicht?",
    "question_en": "Which minister does not have Lower Saxony?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Welches Wappen gehört zum Bundesland Nordrhein-Westfalen?",
    "question_en": "Which coat of arms belongs to the state of North Rhine-Westphalia?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 2",
    "answer_en": "Image 2",
    "image_paths": [
      "./images/nordrhein-westfalen-1-option-1.png",
      "./images/nordrhein-westfalen-1-option-2.png",
      "./images/nordrhein-westfalen-1-option-3.png",
      "./images/nordrhein-westfalen-1-option-4.jpg"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Welches ist ein Landkreis in Nordrhein-Westfalen?",
    "question_en": "What is a district in North Rhine-Westphalia?",
    "options": [
      "Ammerland",
      "Rhein-Sieg-Kreis",
      "Nordfriesland",
      "Vogtlandkreis"
    ],
    "options_en": [
      "Ammerland",
      "Rhein-Sieg-Kreis",
      "North Friesland",
      "Vogtlandkreis"
    ],
    "answer": "Rhein-Sieg-Kreis",
    "answer_en": "Rhein-Sieg-Kreis",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Für wie viele Jahre wird der Landtag in Nordrhein-Westfalen gewählt?",
    "question_en": "How many years is the state parliament elected in North Rhine-Westphalia?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Ab welchem Alter darf man in Nordrhein-Westfalen bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in North Rhine-Westphalia in local elections?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Welche Farben hat die Landesflagge von Nordrhein-Westfalen?",
    "question_en": "What colors does the state flag of North Rhine-Westphalia have?",
    "options": [
      "rot-weiß",
      "grün-weiß-rot",
      "schwarz-gold",
      "blau-weiß-red"
    ],
    "options_en": [
      "red-white",
      "green-white-red",
      "black-gold",
      "blue-white-red"
    ],
    "answer": "grün-weiß-rot",
    "answer_en": "green-white-red",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Wo können Sie sich in Nordrhein-Westfalen über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in North Rhine-Westphalia?",
    "options": [
      "bei den Kirchen",
      "beim Ordnungsamt der Gemeinde",
      "bei der Verbraucherzentrale",
      "bei der Landeszentrale für politische Bildung"
    ],
    "options_en": [
      "In the churches",
      "at the regulatory office of the municipality",
      "At the consumer center",
      "At the State Center for Political Education"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Die Landeshauptstadt von Nordrhein-Westfalen heißt …",
    "question_en": "The state capital of North Rhine-Westphalia is called ...",
    "options": [
      "Köln",
      "Bonn",
      "Düsseldorf",
      "Dortmund"
    ],
    "options_en": [
      "Cologne",
      "Bonn",
      "Düsseldorf",
      "Dortmund"
    ],
    "answer": "Düsseldorf",
    "answer_en": "Düsseldorf",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Welches Bundesland ist Nordrhein-Westfalen?",
    "question_en": "Which state is North Rhine-Westphalia?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "3",
    "answer_en": "3",
    "image_paths": [
      "./images/nordrhein-westfalen-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Nordrhein-Westfalen?",
    "question_en": "What do you call the head of government in North Rhine-Westphalia?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Nordrhein-Westfalen",
    "question": "Welche Ministerin/welchen Minister hat Nordrhein-Westfalen nicht?",
    "question_en": "Which minister does not have North Rhine-Westphalia?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Rheinland-Pfalz",
    "question": "Welches Wappen gehört zum Bundesland Rheinland-Pfalz?",
    "question_en": "Which coat of arms belongs to the state of Rhineland-Palatinate?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 1",
    "answer_en": "Image 1",
    "image_paths": [
      "./images/rheinland-pfalz-1-option-1.png",
      "./images/rheinland-pfalz-1-option-2.png",
      "./images/rheinland-pfalz-1-option-3.png",
      "./images/rheinland-pfalz-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Rheinland-Pfalz",
    "question": "Welches ist ein Landkreis in Rheinland-Pfalz?",
    "question_en": "Which is a district in Rhineland-Palatinate?",
    "options": [
      "Westerwaldkreis",
      "Altötting",
      "Emsland",
      "Prignitz"
    ],
    "options_en": [
      "Westerwaldkreis",
      "Altötting",
      "Emsland",
      "Prignitz"
    ],
    "answer": "Westerwaldkreis",
    "answer_en": "Westerwaldkreis",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Rheinland-Pfalz",
    "question": "Für wie viele Jahre wird der Landtag in Rheinland-Pfalz gewählt?",
    "question_en": "How many years is the state parliament in Rhineland-Palatinate elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Rheinland-Pfalz",
    "question": "Ab welchem Alter darf man in Rheinland-Pfalz bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Rhineland-Palatinate?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Rheinland-Pfalz",
    "question": "Welche Farben hat die Landesflagge von Rheinland-Pfalz?",
    "question_en": "What colors does the state flag of Rhineland-Palatinate have?",
    "options": [
      "weiß-rot",
      "schwarz-rot-gold",
      "schwarz-gelb",
      "grün-weiß-rot"
    ],
    "options_en": [
      "white-red",
      "black-red-gold",
      "black and yellow",
      "green-white-red"
    ],
    "answer": "schwarz-rot-gold",
    "answer_en": "black-red-gold",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Rheinland-Pfalz",
    "question": "Wo können Sie sich in Rheinland-Pfalz über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Rhineland-Palatinate?",
    "options": [
      "bei den Kirchen",
      "bei der Verbraucherzentrale",
      "beim Ordnungsamt der Gemeinde",
      "bei der Landeszentrale für politische Bildung"
    ],
    "options_en": [
      "In the churches",
      "At the consumer center",
      "at the regulatory office of the municipality",
      "At the State Center for Political Education"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Rheinland-Pfalz",
    "question": "Die Landeshauptstadt von Rheinland-Pfalz heißt …",
    "question_en": "The state capital of Rhineland-Palatinate is called ...",
    "options": [
      "Mainz",
      "Kaiserslautern",
      "Ludwigshafen",
      "Koblenz"
    ],
    "options_en": [
      "Mainz",
      "Kaiserslautern",
      "Ludwigshafen",
      "Koblenz"
    ],
    "answer": "Mainz",
    "answer_en": "Mainz",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Rheinland-Pfalz",
    "question": "Welches Bundesland ist Rheinland-Pfalz?",
    "question_en": "Which federal state is Rhineland-Palatinate?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "1",
    "answer_en": "1",
    "image_paths": [
      "./images/rheinland-pfalz-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Rheinland-Pfalz",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Rheinland-Pfalz?",
    "question_en": "What do you call the head of government in Rhineland-Palatinate?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Rheinland-Pfalz",
    "question": "Welche Ministerin/welchen Minister hat Rheinland-Pfalz nicht?",
    "question_en": "Which minister does not have Rhineland-Palatinate?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Saarland",
    "question": "Welches Wappen gehört zum Bundesland Saarland?",
    "question_en": "Which coat of arms belongs to the state of Saarland?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 4",
    "answer_en": "Image 4",
    "image_paths": [
      "./images/saarland-1-option-1.png",
      "./images/saarland-1-option-2.png",
      "./images/saarland-1-option-3.png",
      "./images/saarland-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Saarland",
    "question": "Welches ist ein Landkreis im Saarland?",
    "question_en": "Which is a district in Saarland?",
    "options": [
      "Mecklenburgische Seenplatte",
      "Altötting",
      "Neunkirchen",
      "Rhein-Sieg-Kreis"
    ],
    "options_en": [
      "Mecklenburg Lake District",
      "Altötting",
      "Neunkirchen",
      "Rhein-Sieg-Kreis"
    ],
    "answer": "Neunkirchen",
    "answer_en": "Neunkirchen",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Saarland",
    "question": "Für wie viele Jahre wird der Landtag des Saarlandes gewählt?",
    "question_en": "How many years is the state parliament of Saarland elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Saarland",
    "question": "Ab welchem Alter darf man im Saarland bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Saarland?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Saarland",
    "question": "Welche Farben hat die Landesflagge des Saarlandes?",
    "question_en": "What colors does the state flag of the Saarland have?",
    "options": [
      "weiß-blau",
      "schwarz-rot-gold",
      "schwarz-gelb",
      "grün-weiß-red"
    ],
    "options_en": [
      "white-blue",
      "black-red-gold",
      "black and yellow",
      "green-white-red"
    ],
    "answer": "schwarz-rot-gold",
    "answer_en": "black-red-gold",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Saarland",
    "question": "Wo können Sie sich im Saarland über politische Themen informieren?",
    "question_en": "Where can you find out about political issues in Saarland?",
    "options": [
      "bei den Kirchen",
      "bei der Landeszentrale für politische Bildung",
      "bei der Verbraucherzentrale",
      "beim Ordnungsamt der Gemeinde"
    ],
    "options_en": [
      "In the churches",
      "At the State Center for Political Education",
      "At the consumer center",
      "at the regulatory office of the municipality"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Saarland",
    "question": "Die Landeshauptstadt des Saarlandes heißt …",
    "question_en": "The state capital of the Saarland is called ...",
    "options": [
      "Neunkirchen",
      "Homburg",
      "Saarbrücken",
      "Völklingen"
    ],
    "options_en": [
      "Neunkirchen",
      "Homburg",
      "Saarbrücken",
      "Völklingen"
    ],
    "answer": "Saarbrücken",
    "answer_en": "Saarbrücken",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Saarland",
    "question": "Welches Bundesland ist das Saarland?",
    "question_en": "Which federal state is Saarland?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "2",
    "answer_en": "2",
    "image_paths": [
      "./images/saarland-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Saarland",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef des Saarlandes?",
    "question_en": "What do you call the head of government of Saarland?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Saarland",
    "question": "Welche Ministerin/welchen Minister hat das Saarland nicht?",
    "question_en": "Which minister does not have the Saarland?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Sachsen",
    "question": "Welches Wappen gehört zum Freistaat Sachsen?",
    "question_en": "Which coat of arms belongs to the Free State of Saxony?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 4",
    "answer_en": "Image 4",
    "image_paths": [
      "./images/sachsen-1-option-1.png",
      "./images/sachsen-1-option-2.png",
      "./images/sachsen-1-option-3.png",
      "./images/sachsen-1-option-4.jpg"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Sachsen",
    "question": "Welches ist ein Landkreis in Sachsen?",
    "question_en": "Which is a district in Saxony?",
    "options": [
      "Vogtlandkreis",
      "Altötting",
      "Uckermark",
      "Nordfriesland"
    ],
    "options_en": [
      "Vogtlandkreis",
      "Altötting",
      "Uckermark",
      "North Friesland"
    ],
    "answer": "Vogtlandkreis",
    "answer_en": "Vogtlandkreis",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Sachsen",
    "question": "Für wie viele Jahre wird der Landtag in Sachsen gewählt?",
    "question_en": "How many years is the state parliament in Saxony elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Sachsen",
    "question": "Ab welchem Alter darf man in Sachsen bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Saxony?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Sachsen",
    "question": "Welche Farben hat die Landesflagge von Sachsen?",
    "question_en": "What colors does the state flag of Saxony have?",
    "options": [
      "blau-weiß-rot",
      "weiß-grün",
      "grün-weiß-rot",
      "schwarz-gelb"
    ],
    "options_en": [
      "blue-white-red",
      "white-green",
      "green-white-red",
      "black and yellow"
    ],
    "answer": "weiß-grün",
    "answer_en": "white-green",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Sachsen",
    "question": "Wo können Sie sich in Sachsen über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in Saxony?",
    "options": [
      "bei der Landeszentrale für politische Bildung",
      "beim Ordnungsamt der Gemeinde",
      "bei den Kirchen",
      "bei der Verbraucherzentrale"
    ],
    "options_en": [
      "At the State Center for Political Education",
      "at the regulatory office of the municipality",
      "In the churches",
      "At the consumer center"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Sachsen",
    "question": "Die Landeshauptstadt von Sachsen heißt …",
    "question_en": "The state capital of Saxony is called ...",
    "options": [
      "Leipzig",
      "Dresden",
      "Chemnitz",
      "Zwickau"
    ],
    "options_en": [
      "Leipzig",
      "Dresden",
      "Chemnitz",
      "Zwickau"
    ],
    "answer": "Dresden",
    "answer_en": "Dresden",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Sachsen",
    "question": "Welches Bundesland ist Sachsen?",
    "question_en": "Which federal state is Saxony?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "4",
    "answer_en": "4",
    "image_paths": [
      "./images/sachsen-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Sachsen",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Sachsen?",
    "question_en": "What do you call the head of government in Saxony?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Sachsen",
    "question": "Welche Ministerin/welchen Minister hat Sachsen nicht?",
    "question_en": "Which minister does not have Saxony?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Sachsen-Anhalt",
    "question": "Welches Wappen gehört zum Bundesland Sachsen-Anhalt?",
    "question_en": "Which coat of arms belongs to the state of Saxony-Anhalt?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 4",
    "answer_en": "Image 4",
    "image_paths": [
      "./images/sachsen-anhalt-1-option-1.png",
      "./images/sachsen-anhalt-1-option-2.png",
      "./images/sachsen-anhalt-1-option-3.png",
      "./images/sachsen-anhalt-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Sachsen-Anhalt",
    "question": "Welches ist ein Landkreis in Sachsen-Anhalt?",
    "question_en": "Which is a district in Saxony-Anhalt?",
    "options": [
      "Ammerland",
      "Altötting",
      "Uckermark",
      "Börde"
    ],
    "options_en": [
      "Ammerland",
      "Altötting",
      "Uckermark",
      "Börde"
    ],
    "answer": "Börde",
    "answer_en": "Börde",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Sachsen-Anhalt",
    "question": "Für wie viele Jahre wird der Landtag in Sachsen-Anhalt gewählt?",
    "question_en": "How many years is the state parliament in Saxony-Anhalt elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Sachsen-Anhalt",
    "question": "Ab welchem Alter darf man in Sachsen-Anhalt bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Saxony-Anhalt?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "18",
    "answer_en": "18",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Sachsen-Anhalt",
    "question": "Welche Farben hat die Landesflagge von Sachsen-Anhalt?",
    "question_en": "What colors does the state flag of Saxony-Anhalt have?",
    "options": [
      "blau-weiß-rot",
      "gelb-schwarz",
      "grün-weiß-rot",
      "weiß-rot"
    ],
    "options_en": [
      "blue-white-red",
      "yellow-black",
      "green-white-red",
      "white-red"
    ],
    "answer": "gelb-schwarz",
    "answer_en": "yellow-black",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Sachsen-Anhalt",
    "question": "Wo können Sie sich in Sachsen-Anhalt über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in Saxony-Anhalt?",
    "options": [
      "bei den Kirchen",
      "bei der Landeszentrale für politische Bildung",
      "beim Ordnungsamt der Gemeinde",
      "bei der Verbraucherzentrale"
    ],
    "options_en": [
      "In the churches",
      "At the State Center for Political Education",
      "at the regulatory office of the municipality",
      "At the consumer center"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Sachsen-Anhalt",
    "question": "Die Landeshauptstadt von Sachsen-Anhalt heißt …",
    "question_en": "The state capital of Saxony-Anhalt is called ...",
    "options": [
      "Halle",
      "Dessau",
      "Magdeburg",
      "Wittenberg"
    ],
    "options_en": [
      "hall",
      "Dessau",
      "Magdeburg",
      "Wittenberg"
    ],
    "answer": "Magdeburg",
    "answer_en": "Magdeburg",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Sachsen-Anhalt",
    "question": "Welches Bundesland ist Sachsen-Anhalt?",
    "question_en": "Which federal state is Saxony-Anhalt?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "3",
    "answer_en": "3",
    "image_paths": [
      "./images/sachsen-anhalt-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Sachsen-Anhalt",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Sachsen-Anhalt?",
    "question_en": "What do you call the head of government in Saxony-Anhalt?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Sachsen-Anhalt",
    "question": "Welche Ministerin/welchen Minister hat Sachsen-Anhalt nicht?",
    "question_en": "Which minister does not have Saxony-Anhalt?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Schleswig-Holstein",
    "question": "Welches Wappen gehört zum Bundesland Schleswig-Holstein?",
    "question_en": "Which coat of arms belongs to the state of Schleswig-Holstein?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 3",
    "answer_en": "Image 3",
    "image_paths": [
      "./images/schleswig-holstein-1-option-1.png",
      "./images/schleswig-holstein-1-option-2.png",
      "./images/schleswig-holstein-1-option-3.png",
      "./images/schleswig-holstein-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Schleswig-Holstein",
    "question": "Welches ist ein Landkreis in Schleswig-Holstein?",
    "question_en": "Which is a district in Schleswig-Holstein?",
    "options": [
      "Ammerland",
      "Mecklenburgische Seenplatte",
      "Nordfriesland",
      "Rhein-Sieg-Kreis"
    ],
    "options_en": [
      "Ammerland",
      "Mecklenburg Lake District",
      "North Friesland",
      "Rhein-Sieg-Kreis"
    ],
    "answer": "Nordfriesland",
    "answer_en": "North Friesland",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Schleswig-Holstein",
    "question": "Für wie viele Jahre wird der Landtag in Schleswig-Holstein gewählt?",
    "question_en": "How many years is the state parliament in Schleswig-Holstein elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Schleswig-Holstein",
    "question": "Ab welchem Alter darf man in Schleswig-Holstein bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Schleswig-Holstein?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Schleswig-Holstein",
    "question": "Welche Farben hat die Landesflagge von Schleswig-Holstein?",
    "question_en": "What colors does the state flag of Schleswig-Holstein have?",
    "options": [
      "weiß-blau",
      "blau-weiß-rot",
      "weiß-rot",
      "grün-weiß-rot"
    ],
    "options_en": [
      "white-blue",
      "blue-white-red",
      "white-red",
      "green-white-red"
    ],
    "answer": "blau-weiß-rot",
    "answer_en": "blue-white-red",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Schleswig-Holstein",
    "question": "Wo können Sie sich in Schleswig-Holstein über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in Schleswig-Holstein?",
    "options": [
      "bei der Verbraucherzentrale",
      "beim Ordnungsamt der Gemeinde",
      "beim / bei der Landesbeauftragten für politische Bildung",
      "bei den Kirchen"
    ],
    "options_en": [
      "At the consumer center",
      "at the regulatory office of the municipality",
      "at the state representative for political education",
      "In the churches"
    ],
    "answer": "beim / bei der Landesbeauftragten für politische Bildung",
    "answer_en": "at the state representative for political education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Schleswig-Holstein",
    "question": "Die Landeshauptstadt von Schleswig-Holstein heißt …",
    "question_en": "The state capital of Schleswig-Holstein is called ...",
    "options": [
      "Husum",
      "Flensburg",
      "Lübeck",
      "Kiel"
    ],
    "options_en": [
      "Husum",
      "Flensburg",
      "Lübeck",
      "Keel"
    ],
    "answer": "Kiel",
    "answer_en": "Keel",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Schleswig-Holstein",
    "question": "Welches Bundesland ist Schleswig-Holstein?",
    "question_en": "Which federal state is Schleswig-Holstein?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "1",
    "answer_en": "1",
    "image_paths": [
      "./images/schleswig-holstein-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Schleswig-Holstein",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Schleswig-Holstein?",
    "question_en": "What do you call the head of government in Schleswig-Holstein?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Schleswig-Holstein",
    "question": "Welche Ministerin/welchen Minister hat Schleswig-Holstein nicht?",
    "question_en": "Which minister does not have Schleswig-Holstein?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  },
  {
    "question_number": 1,
    "bundesland": "Thüringen",
    "question": "Welches Wappen gehört zum Freistaat Thüringen?",
    "question_en": "Which coat of arms belongs to the Free State of Thuringia?",
    "options": [
      "Bild 1",
      "Bild 2",
      "Bild 3",
      "Bild 4"
    ],
    "options_en": [
      "Image 1",
      "Image 2",
      "Image 3",
      "Image 4"
    ],
    "answer": "Bild 4",
    "answer_en": "Image 4",
    "image_paths": [
      "./images/th-ringen-1-option-1.jpg",
      "./images/th-ringen-1-option-2.png",
      "./images/th-ringen-1-option-3.jpg",
      "./images/th-ringen-1-option-4.png"
    ]
  },
  {
    "question_number": 2,
    "bundesland": "Thüringen",
    "question": "Welches ist ein Landkreis in Thüringen?",
    "question_en": "Which is a district in Thuringia?",
    "options": [
      "Ammerland",
      "Altötting",
      "Nordfriesland",
      "Wartburgkreis"
    ],
    "options_en": [
      "Ammerland",
      "Altötting",
      "North Friesland",
      "Wartburg district"
    ],
    "answer": "Wartburgkreis",
    "answer_en": "Wartburg district",
    "image_paths": []
  },
  {
    "question_number": 3,
    "bundesland": "Thüringen",
    "question": "Für wie viele Jahre wird der Landtag in Thüringen gewählt?",
    "question_en": "How many years is the state parliament in Thuringia elected?",
    "options": [
      "3",
      "4",
      "5",
      "6"
    ],
    "options_en": [
      "3",
      "4",
      "5",
      "6"
    ],
    "answer": "5",
    "answer_en": "5",
    "image_paths": []
  },
  {
    "question_number": 4,
    "bundesland": "Thüringen",
    "question": "Ab welchem Alter darf man in Thüringen bei Kommunalwahlen wählen?",
    "question_en": "From what age can you choose in local elections in Thuringia?",
    "options": [
      "14",
      "16",
      "18",
      "20"
    ],
    "options_en": [
      "14",
      "16",
      "18",
      "20"
    ],
    "answer": "16",
    "answer_en": "16",
    "image_paths": []
  },
  {
    "question_number": 5,
    "bundesland": "Thüringen",
    "question": "Welche Farben hat die Landesflagge von Thüringen?",
    "question_en": "What colors does the state flag of Thuringia have?",
    "options": [
      "blau-weiß-rot",
      "weiß-rot",
      "grün-weiß-rot",
      "schwarz-gold"
    ],
    "options_en": [
      "blue-white-red",
      "white-red",
      "green-white-red",
      "black-gold"
    ],
    "answer": "weiß-rot",
    "answer_en": "white-red",
    "image_paths": []
  },
  {
    "question_number": 6,
    "bundesland": "Thüringen",
    "question": "Wo können Sie sich in Thüringen über politische Themen informieren?",
    "question_en": "Where can you find out about political topics in Thuringia?",
    "options": [
      "bei den Kirchen",
      "bei der Verbraucherzentrale",
      "bei der Landeszentrale für politische Bildung",
      "beim Ordnungsamt der Gemeinde"
    ],
    "options_en": [
      "In the churches",
      "At the consumer center",
      "At the State Center for Political Education",
      "at the regulatory office of the municipality"
    ],
    "answer": "bei der Landeszentrale für politische Bildung",
    "answer_en": "At the State Center for Political Education",
    "image_paths": []
  },
  {
    "question_number": 7,
    "bundesland": "Thüringen",
    "question": "Die Landeshauptstadt von Thüringen heißt …",
    "question_en": "The state capital of Thuringia is called ...",
    "options": [
      "Eisenach",
      "Erfurt",
      "Gera",
      "Jena"
    ],
    "options_en": [
      "Ironach",
      "Erfurt",
      "Gera",
      "Jena"
    ],
    "answer": "Erfurt",
    "answer_en": "Erfurt",
    "image_paths": []
  },
  {
    "question_number": 8,
    "bundesland": "Thüringen",
    "question": "Welches Bundesland ist Thüringen?",
    "question_en": "Which federal state is Thuringia?",
    "options": [
      "1",
      "2",
      "3",
      "4"
    ],
    "options_en": [
      "1",
      "2",
      "3",
      "4"
    ],
    "answer": "2",
    "answer_en": "2",
    "image_paths": [
      "./images/th-ringen-8-question.png"

    ]
  },
  {
    "question_number": 9,
    "bundesland": "Thüringen",
    "question": "Wie nennt man die Regierungschefin/den Regierungschef in Thüringen?",
    "question_en": "What do you call the head of government in Thuringia?",
    "options": [
      "Erste Ministerin/Erster Minister",
      "Premierministerin/Premierminister",
      "Bürgermeisterin/Bürgermeister",
      "Ministerpräsidentin/Ministerpräsident"
    ],
    "options_en": [
      "First Minister",
      "Prime Minister/Prime Minister",
      "Mayor",
      "Prime Minister/Prime Minister"
    ],
    "answer": "Ministerpräsidentin/Ministerpräsident",
    "answer_en": "Prime Minister/Prime Minister",
    "image_paths": []
  },
  {
    "question_number": 10,
    "bundesland": "Thüringen",
    "question": "Welche Ministerin/welchen Minister hat Thüringen nicht?",
    "question_en": "Which minister does not have Thuringia?",
    "options": [
      "Justizministerin/Justizminister",
      "Außenministerin/Außenminister",
      "Finanzministerin/Finanzminister",
      "Innenministerin/Innenminister"
    ],
    "options_en": [
      "Minister of Justice/Minister of Justice",
      "Foreign Minister/Foreign Minister",
      "Finance Minister",
      "Minister of the Interior"
    ],
    "answer": "Außenministerin/Außenminister",
    "answer_en": "Foreign Minister/Foreign Minister",
    "image_paths": []
  }
];

// Statistics
export const questionsStats = {
  total: 460,
  bundeslaender: [
    "General",
    "Baden-Württemberg",
    "Bayern",
    "Berlin",
    "Brandenburg",
    "Bremen",
    "Hamburg",
    "Hessen",
    "Mecklenburg-Vorpommern",
    "Niedersachsen",
    "Nordrhein-Westfalen",
    "Rheinland-Pfalz",
    "Saarland",
    "Sachsen",
    "Sachsen-Anhalt",
    "Schleswig-Holstein",
    "Thüringen"
  ],
  lastUpdated: "2025-07-26T14:31:18.518Z"
};
